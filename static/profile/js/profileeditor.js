...
//  defaultprofile.startDate.setMilliseconds(0);
  ...
  function profileSubmit(event) {
    ...
    if (record.convertedOnTheFly) {
      var result = window.confirm(translate('Profile is going to be saved in newer format used in Nightscout 0.9.1-dev-20161112 and above and will not be usable in older versions anymore.\nAre you sure?'));
      if (!result) {
        return;
      }
    }
