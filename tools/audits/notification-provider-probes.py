...
parser.add_argument('--port',default='17340')
args=parser.parse_args()
out=Path(args.output).resolve();out.mkdir(parents=True,exist_ok=True)
node=args.node
roots={'baseline':str(Path(args.baseline).resolve()),'candidate':str(Path(args.candidate).resolve())}
probe=Path(__file__).with_name('notification-provider-probe.cjs')
results=[]
for run in range(7):
 for mode in ('disabled','enabled'):
  for label,root in roots.items():
   env={k:os.environ[k] for k in ('PATH','HOME','TMPDIR','LANG') if k in os.environ}
   env.update(API_SECRET='benchmark-fixture-secret',MONGODB_URI=args.mongo_uri,PORT=args.port,HOSTNAME='127.0.0.1',NODE_ENV='production',INSECURE_USE_HTTP='true',ENABLE='careportal',MONGO_POOL_SIZE='5',MONGO_MIN_POOL_SIZE='1')
   ...
   assert data['providerSends'] == (20 if mode == 'enabled' else 0)
   assert data['pushoverSends'] == (20 if mode == 'enabled' else 0)
   assert data['apnLoaded'] == (mode == 'enabled' or label == 'baseline')
   assert data['pushoverLoaded'] == (mode == 'enabled' or label == 'baseline')
   assert data['providerClientsRetained'] == (20 if mode == 'enabled' and label == 'baseline' else 0)
