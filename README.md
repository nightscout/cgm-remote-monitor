...
### Core
  ...
  * `SSL_KEY` - Path to your ssl key file, so that ssl(https) can be enabled directly in node.js. If you are using let's encrypt, make this variable the path to your privkey.pem file (private key)
  * `SSL_CERT` - Path to your ssl cert file, so that ssl(https) can be enabled directly in node.js. If you are using let's encrypt, make this variable the path to the fullchain.pem file (cert + ca).
  * `SSL_CA` - Path to your ssl ca file, so that ssl(https) can be enabled directly in node.js. If you are using let's encrypt, make this variable the path to the chain.pem file (chain)
