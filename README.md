...
### Predefined values for your server settings (optional)
  ...
  * `Clock` - Shows current BG. Grey text on a black background.
  * `Color` - Shows current BG, trend arrow, and time of day. Grey text on a black background.
  * `Simple` - Shows current BG and trend arrow. White text on a background that changes color to indicate current BG threshold (green = in range; blue = below range; yellow = above range; red = urgent below/above).
