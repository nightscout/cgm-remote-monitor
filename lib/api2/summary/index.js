...
function configure (env, ctx) {
  const basalProcessor = require('./basaldataprocessor')
  ...
  function constructState() {
    ...
    const p = ctx?.sbx?.properties;

    const state = {
        iob: Math.round(p?.iob?.iob*100)/100,
        cob: Math.round(p?.cob?.cob),
        bwp: Math.round(p?.bwp?.bolusEstimate*100)/100,
        cage: p?.cage?.age,
        sage: p?.sage?.age,
        iage: p?.iage?.age,
        bage: p?.bage?.age,
        battery: p?.upbat?.level
    }
     return state;
    }
