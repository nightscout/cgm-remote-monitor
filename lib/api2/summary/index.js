...
function configure (env, ctx) {
  ...
  function constructState() {
    ...
    const p = __get(ctx, 'sbx.properties');

    const state = {
        iob: Math.round(__get(p,'iob.iob')*100)/100,
        cob: Math.round(__get(p,'cob.cob')),
        bwp: Math.round(__get(p,'bwp.bolusEstimate')*100)/100,
        cage: __get(p,'cage.age'),
        sage: __get(p,'sage.age'),
        iage: __get(p,'iage.age'),
        bage: __get(p,'bage.age'),
        battery: __get(p,'upbat.level')
    }
     return state;
    }
