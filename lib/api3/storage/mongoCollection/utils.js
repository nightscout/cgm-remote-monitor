 ...
 * identifier: the documents with that identifier, and v1 records without an
 * identifier whose _id is the identifier in any form (a record stored twice,
 * once with the 24-hex string and once with the ObjectId it names). A record
 * with an identifier of its own is addressed by that identifier, so it is not
 * matched by its _id here, even when its _id equals this identifier.
 * @param {string} identifier
 */
function filterForEveryForm (identifier) {
  ...
  if (idForms.isHexId(identifier)) {
    idForms.idForms(identifier).forEach(function (form) {
      filterOpts.push({ identifier: { $exists: false }, _id: { $eq: form } });
    });
  } else if (typeof identifier === 'string') {
    filterOpts.push({ identifier: { $exists: false }, _id: { $eq: identifier } });
  }
