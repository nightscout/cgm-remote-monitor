...
function normalizeDoc (doc) {
  if (!doc.identifier && doc._id != null) {
...
/**
 * Create query filter for every stored form of the document addressed by
 * identifier: the documents with that identifier, and v1 records with an
 * absent, null or empty string identifier whose _id is the identifier in any
 * form (a record stored twice, once with the 24-hex string and once with the
 * ObjectId it names). A record with its own identifier is addressed by it,
 * so it is not matched by its _id here, even when its _id equals this identifier.
 * @param {string} identifier
 */
function filterForEveryForm (identifier) {

  const filterOpts = [ { identifier: { $eq: identifier } } ];

  // normalizeDoc exposes _id for null and empty identifiers too. MongoDB's
  // null match also includes an absent field; a different identifier still
  // prevents a record from being deleted through this _id fallback. Exclude
  // arrays, since $in also matches their elements but normalizeDoc keeps them.
  const emptyIdentifier = { $in: [null, ''], $not: { $type: 'array' } };
  if (idForms.isHexId(identifier)) {
    idForms.idForms(identifier).forEach(function (form) {
      filterOpts.push({ identifier: emptyIdentifier, _id: { $eq: form } });
    });
  } else if (typeof identifier === 'string') {
    filterOpts.push({ identifier: emptyIdentifier, _id: { $eq: identifier } });
  }
