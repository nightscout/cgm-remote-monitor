...
function MongoCollection (ctx, env, colName) {
   ...
   * @param {any} filterDef
   */
  var parseFilter = function parseFilter (filterDef) {
  ...
  /**
   * Parse array of timestamps for history query
   * @param {any} filterHistory
   */
  var parseFilterHistory = function parseFilterHistory (filterHistory) {
    
    if (!filterHistory || !_.isArray(filterHistory) || filterHistory.length === 0)
      throw 'Bad filter history definition';

    var clauses = _.map(filterHistory, function mapFilterHistory (item) {

      var clause = { };
      clause[item.field] = { $gte: item.value };
      return clause;
    });
  ...
  /**
   * Find history of many documents since timestamps in array
   * @param {function} done
   */
  self.findManyFrom = function findManyFrom (filterHistory, sort, limit, skip, projection, done) {

    var filter = parseFilterHistory(filterHistory);
