...
function MongoCollection (ctx, env, colName) {
   ...
   * @param {any} filterDef
   */
  var parseFilter = function parseFilter (filterDef, logicalOperator) {
  ...
  self.findMany = function findMany (filterDef, sort, limit, skip, projection, done) {
    ...
    var filter = parseFilter(filterDef, 'and');
   ...
   * @param {function} done
   */
  self.findManyOr = function findManyOr (filterDef, sort, limit, skip, projection, done) {
  ...
  /**
   * Get timestamp (srvModified) of the last modified document 
   * @param {function} done
   */
  self.getLastModified = function getLastModified (done) {
    
    self.col.find()
      .sort({ srvModified: -1 })
      .limit(1)
      .project({ srvModified: 1 })
