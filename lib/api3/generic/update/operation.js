...
async function update (opCtx) {
  ...
  const { col, req, res } = opCtx;  const doc = req.body;
  ...
  // PUT may resolve to either an insert or a replacement. Require at least
  // one applicable write permission before processing or querying the body;
  // the selected branch demands its exact permission below.
  ...
  // Route parameters are outside req.body, so purify the final document again
  // after copying the route-derived identifier into the persisted shape.
  writePurifier.purifyWritableDocument(opCtx, doc);
