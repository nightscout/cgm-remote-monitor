...
const apiConst = require('../../const.json')
  ...
  , srvDates = require('../../../server/srv-dates')
  ;
...
async function deletePermanently (opCtx) {
  ...
  const identifier = req.params.identifier;
  const result = await col.storage.deleteEveryForm(identifier);
...
async function markAsDeleted (opCtx) {
  ...
  const identifier = req.params.identifier;
  const setFields = { 'isValid': false, 'srvModified': srvDates.next() };

  if (auth && auth.subject && auth.subject.name) {
    setFields.modifiedBy = auth.subject.name;
  }

  const result = await col.storage.updateEveryForm(identifier, setFields);
