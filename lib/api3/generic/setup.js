...
function setupGenericCollections (ctx, env, app) {
  ...
  if (_.includes(enabledCols, 'treatments')) {
    ...
    */
    ...
    if (_.includes(enabledCols, 'remotecommands')) {
      cols.remoteCommands = new Collection({
        ctx, env, app,
        colName: 'remotecommands',
        storageColName: env.remotecommands_collection || 'remoteCommands',
        fallbackGetDate: fallbackCreatedAt, //TODO: What should this be?
        dedupFallbackFields: ['created_at'], //TODO: Should I add the client-specified identifier field?
        fallbackDateField: 'created_at'
      });
    }
  }
