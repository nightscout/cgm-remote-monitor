...
function Collection ({ ctx, env, app, colName, storageColName, fallbackGetDate, dedupFallbackFields,
  ...
  self.parseLimit = function parseLimit (req, res) {
    ...
    if (req.query.limit) {
      // Read the digits the client actually wrote.  `isNaN` accepts `0x10` and
      // `1e2`, which `parseInt(_, 10)` then reads as `0` and `1`; and MongoDB
      // defines a limit of `0` as *no limit*, so `?limit=0x10` was answered
      // with the whole collection, ignoring API3_MAX_LIMIT entirely.
      const requested = /^\s*\d+\s*$/.test(req.query.limit)
        ? Number(req.query.limit)
        : NaN;

      if (Number.isSafeInteger(requested) && requested > 0 && requested <= maxLimit) {
        limit = requested;
      }
      else {
