...
function Collection ({ ctx, env, app, colName, storageColName, fallbackGetDate, dedupFallbackFields,
  ...
  self.parseLimit = function parseLimit(req, res) {
    const maxLimitRaw = app.get("API3_MAX_LIMIT");
    const maxLimit = parseInt(maxLimitRaw) || apiConst.API3_MAX_LIMIT;
    let limit = maxLimit;
    ...
    if (req.query.limit) {
      if (
        ...
        opTools.sendJSONStatus(
          res,
          apiConst.HTTP.BAD_REQUEST,
          apiConst.MSG.HTTP_400_BAD_LIMIT
        );
        return null;
      }
    }
