...
function Collection ({ ctx, env, app, authBuilder, colName, storageColName, fallbackGetDate, dedupFallbackFields,
  ...
  self.authorizers = {
    admin: authBuilder.authorizerFor(`api:${colName}:admin`),
  ...
  /**
    ...
    */
  self.validateCommon = function validateCommon (doc, res) {
    ...
    if (typeof(doc.utcOffset) !== 'number' 
    ...
    if (typeof(doc.app) !== 'string' || stringTools.isNullOrWhitespace(doc.app)) {
