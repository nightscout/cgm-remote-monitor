...
function parseFilter (req, res) {
  ...
  for (let param in req.query) {
    ...
    if (param === 'filter_parameters') {
      // filter_parameters are a space separated list of
      // <param>$<operator>=<value>
      // for the purpose of building the filter, we want
      // to take each <param>$<operator> as the key for
      // the processes query params map
      const filterValues = req.query[param].split(' ');

      for (const filterValue of filterValues) {
        const filterParts = filterValue.split('=');

        processedQueryParams[filterParts[0]] = filterParts[1];
      }
    }
    else {
