...
function parseSort (req, res) {
  ...
  // `_id` is always present and always unique, so it makes the order total.
  // Without it every key in the chain can tie at once - bulk-imported documents
  // have no `identifier`, and an uploader writing in bursts shares one
  // `created_at` and one `date` - and an untotal order makes `skip` return some
  // documents more than once and others not at all.
