...
function configure (env, ctx) {
  ...
  self.setupGenericCollections = function setupGenericCollections () {
    ...
    cols.devicestatus = colBuilder(env.devicestatus_collection, self.fallbackCreatedAt);

    cols.entries = colBuilder(env.entries_collection, self.fallbackDate);

    cols.food = colBuilder(env.food_collection, self.fallbackCreatedAt);

    cols.profile = colBuilder(env.profile_collection, self.fallbackCreatedAt);

    cols.treatments = colBuilder(env.treatments_collection, self.fallbackCreatedAt);
