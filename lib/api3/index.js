...
function configure (env, ctx) {
  ...
  self.setupGenericCollections = function setupGenericCollections () {
    ...
    cols.devicestatus = new Collection(ctx, env, app, authBuilder, env.devicestatus_collection, self.fallbackCreatedAt);

    cols.entries = new Collection(ctx, env, app, authBuilder, env.entries_collection, self.fallbackDate);

    cols.food = new Collection(ctx, env, app, authBuilder, env.food_collection, self.fallbackCreatedAt);

    cols.profile = new Collection(ctx, env, app, authBuilder, env.profile_collection, self.fallbackCreatedAt);

    cols.treatments = new Collection(ctx, env, app, authBuilder, env.treatments_collection, self.fallbackCreatedAt);
