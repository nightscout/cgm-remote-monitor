...
function configure (env, ctx) {
  ...
  self.setupApiRoutes = function setupApiRoutes () {
    ...
    app.get('/history', require('./specific/history')(app, ctx, env, authBuilder));
  }
  ...
  self.setupGenericCollections = function setupGenericCollections () {
    ...
    cols.devicestatus = new Collection(ctx, env, app, authBuilder, env.devicestatus_collection, 
      self.fallbackCreatedAt, ['created_at', 'device']);

    cols.entries = new Collection(ctx, env, app, authBuilder, env.entries_collection, 
      self.fallbackDate, ['date', 'type']);

    cols.food = new Collection(ctx, env, app, authBuilder, env.food_collection, 
      self.fallbackCreatedAt, ['created_at']);

    cols.profile = new Collection(ctx, env, app, authBuilder, env.profile_collection, 
      self.fallbackCreatedAt, ['created_at']);

    cols.treatments = new Collection(ctx, env, app, authBuilder, env.treatments_collection, 
      self.fallbackCreatedAt, ['created_at', 'eventType']);
