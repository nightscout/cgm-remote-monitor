...
const apiConst = require('./const.json')
  ...
  , clientIP = require('../server/client-ip')
  ;
...
function authenticate (opCtx) {
  return new Promise(function promise (resolve, reject) {
    ...
    ctx.authorization.resolve({ token, ip: clientIP.getClientIP(req, clientIP.trustFor(env)) }, function resolveFinish (err, result) {
      if (err) {
        return reject(
...
function checkPermission (auth, permission) {
  if (auth) {
    const found = auth.shiros && Array.isArray(auth.shiros) ?
      auth.shiros.find(function checkEach (shiro) {
