...
const { clientIPFor } = require('../server/client-ip');
...
function AlarmSocket (app, env, ctx) {
  ...
  /**
   * Whether a web `subscribe` message carries any credential. Absent, null and
   * empty values do not count; neither does the string 'null', which
   * `resolve` already treats as no secret. `token` and `accessToken` are
   * counted even though this path does not read them, so that a message
   * carrying one is never answered as anonymous without the wait.
   *
   * @param {Object} message
   * @returns {Boolean}
   */
  function presentsCredential (message) {
    return ['secret', 'jwtToken', 'token', 'accessToken'].some(function carries (field) {
  ...
  self.init = function init (io) {
    ...
    self.namespace.on('connection', function onConnected (socket) {
       ...
       * closed (BF-75). A later `subscribe` re-evaluates with whatever
       * credentials the client offers and can widen or narrow this same
       * decision.
       *
       * The connection carries no credential, so this is answered through
       * `resolveAnonymous`, without the failed-login delay for the address
       * (BF-80): the answer is the same fixed default permissions whatever
       * the delay state, and it neither records nor clears a failure, so an
       * instance cannot throttle itself by accepting connections and a
       * connection cannot wear the delay down. A viewer on an address shared
       * with a misconfigured uploader is therefore admitted at once on a
       * readable site, signed in or not; its credentialed `subscribe` still
       * waits the full delay before the credential is checked, and can only
       * add to what this admission gave.
       *
       * The answer is synchronous, so it lands before any message from the
       * socket can. `hasSubscribed` keeps that safe without depending on it —
       * an explicit `subscribe` is the more specific answer and the anonymous
       * default must never overwrite it, which in this namespace would mean
       * quietly evicting an authorized viewer from the delivery room and
       * dropping their alarms.
       */
      ctx.authorization.resolveAnonymous(function resolvedDefault (err, auth) {
  ...
  self.subscribe = function subscribe (socket, message, returnCallback) {
    ...
    */
    ...
    /* A subscriber presenting no credential at all -- the web page before
     * anyone signs in sends `{ secret: null }` -- gets the deployment's
     * anonymous default permissions without waiting on the failed-login delay
     * for its address (BF-80). It has nothing to guess with, so the delay
     * would protect nothing, and holding it would keep an alarm from a viewer
     * on an address shared with a misconfigured uploader. Any value in any
     * credential field, valid or not, still goes through `resolve` and waits
     * exactly as before.
     */
    const resolveFor = presentsCredential(message)
      ? callback => ctx.authorization.resolve({ api_secret: message.secret, token: message.jwtToken, ip: getRemoteIP(socket.request) }, callback)
      : callback => ctx.authorization.resolveAnonymous(callback);

    if (message && shouldTry) {
      return resolveFor(function resolveFinish (err, auth) {
