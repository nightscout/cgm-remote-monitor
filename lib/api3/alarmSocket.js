...
function AlarmSocket (app, env, ctx) {
  ...
  self.init = function init (io) {
    ...
    self.namespace.on('connection', function onConnected (socket) {
       ...
       * lands after later messages from the same socket. A `subscribe` that
       * carries a credential waits on the same deadline and resolves after
       * it; one that carries none is answered at once (BF-80) and so resolves
       * BEFORE it. `hasSubscribed` makes both orders safe — an explicit
       * `subscribe` is the more specific answer and the anonymous default must
       * never overwrite it, which in this namespace would mean quietly
       * evicting an authorized viewer from the delivery room and dropping
       * their alarms.
       */
