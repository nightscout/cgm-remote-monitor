...
pentagon.prepareHtml = function pentagonPrepareHtml (timeSpanToShow) {
  $('#pentagoncharts').html('');
  ...
  '<br>' +
  'Target high glucose was hardcoded to 180 [mg/dl] <br>' +
  'Target low glucose was hardcoded to 70 [mg/dl] <br>' +
  '</span></div>';
...
pentagon.report = function report_pentagon (datastorage, sorteddaystoshow) {
  ...
  pentagon.prepareHtml(timeSpanToShow);
  ...
  timeSpanToShow.forEach(function eachTimeSpan (timeSpan) {
    ...
    drawPentagon(timeSpan, sgvData);
  });

  function drawPentagon (week, sgvData) {
    let charts, context;
    ...
    function drawAxis(name, axis){
      ...
      for(let x of axis.ticks){
        // Workaround to keep short tick names on x1000 scales
        let y = axis.formula(x * (axis.label.includes("1000 x mg/dl") ? 1000 : 1));
    ...
    let patientResult = new function(){
      ...
      for(let i = 0; i < dayIndices.length-1; ++i){
        ...
        dayResults.get("ToR").push(1440-integralFunction(sgvDataSlice, x => x.sgv >= rangeLow && x.sgv <= rangeHigh).totalTimeMin);
        dayResults.get("CV").push(coefficientOfVariation(sgvDataSlice));
        dayResults.get("Hypo").push(normalizedAreaUnderCurve(sgvDataSlice, x => x.sgv < rangeLow));
        dayResults.get("Hyper").push(normalizedAreaUnderCurve(sgvDataSlice, x => x.sgv > rangeHigh));
