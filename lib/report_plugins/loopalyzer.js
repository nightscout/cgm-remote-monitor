...
loopalyzer.css =
  ...
  '}' +
  '#loopalyzer-help, #loopalyzer-notenoughdata, #loopalyzer-buttons, #loopalyzer-dateinfo {' +
  '  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;' +
  '  color: #2b2f33;' +
  '}' +
  '#loopalyzer-profiles-table {' +
  '  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;' +
  '  color: #2b2f33;' +
  '  -webkit-print-color-adjust: exact;' +
  '  print-color-adjust: exact;' +
  '}' +
  '#loopalyzer-profiles-table table { margin: 0 10px; border-collapse: separate; border-spacing: 0; border: 0px; }' +
  ...
  '}' +
  '#loopalyzer-profiles-table td caption { text-align: left; font-weight: 600; padding: 2px 4px 6px; color: #4a5057; }' +
  ...
  '  print-color-adjust: exact;' +
  '}' +
  '#loopalyzer-profiles-table td td { text-align: right; vertical-align: top; padding: 0 1px; border: 0; }' +
  '#loopalyzer-profiles-table td td td { padding: 1px 8px; }' +
  '#loopalyzer-profiles-table td tr:nth-child(even) td td { background-color: #f7f9fa; }';
...
loopalyzer.generateReport = function(datastorage, daysToShow, options) {
  ...
  var tickColor = '#e0e3e7';
  var basalColor = '#1f6fc4'; // canonical app blue
  var glucoseColor = '#4caf50'; // in-range green, legible over the range band
  var predictionsColor = '#8E5FD0'; // purple
  var glucoseRangeColor = 'rgba(76,175,80,0.12)'; // soft in-range green band (was #D6FFD6)
  var tempBasalColor = '#0097a7'; // distinct teal for temp-basal delta
  var iobColor = '#1f6fc4'; // blue for insulin on board
  var insulinColor = iobColor; // back-compat alias (zero-line marking + IOB)
  var carbColor = '#ffb74d'; // amber/orange for carbs on board
  var timeShiftBackgroundColor = "#f2f4f6"; // very light neutral overlay
  ...
  if (doTimeShift)
    markings.push({ xaxis: { from: timeShiftStartTime.toDate(), to: timeShiftStopTime.toDate() }, color: timeShiftBackgroundColor });
  markings.push({ yaxis: { from: 0, to: 0 }, color: tempBasalColor, lineWidth: 2 });

  var chartTempBasalData = [{
    data: tempBasalsAvg
    , label: translate('Temp basal delta')
    , id: 'tempBasals'
    , color: tempBasalColor
    , points: { show: false }
