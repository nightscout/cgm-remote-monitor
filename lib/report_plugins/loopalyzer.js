...
loopalyzer.html = function html(client) {
  ...
  var ret = ''
  ret += '<h2>' + translate('Loopalyzer') + '&nbsp;&nbsp;<span id="loopalyzer-dateinfo"></span></h2>'
...
loopalyzer.css =
    '#loopalyzer-charts, #loopalyzer-profiles { padding: 5px 20px; }'
...
loopalyzer.renderProfilesTable = function (datastoreProfiles, daysToShow, client) {
  ...
  // Now we're up for some profile juggling. We want to display only the profiles relevant to the days we are showing.
  // This includes the last profile changed before the first display date, and the profiles created on the display dates.
  ...
  // these on ascending startDate (create a clone array so we don't modify the Store array)
  var profilesArray1 = [];
  datastoreProfiles.forEach(function (entry) {
    var newEntry = {};
    newEntry.startDate = entry.startDate;
    newEntry.basal = entry.store.Default.basal;
    newEntry.carbratio = entry.store.Default.carbratio;
    newEntry.sens = entry.store.Default.sens;
    if (moment(entry.startDate).isBefore(endOfLastDay))
  ...
  // Second, find the latest profile with a startDate before beginningOfFirstDay
  var latestProfile = profilesArray1[0]; // This is the oldest one
  profilesArray1.forEach(function (entry) {
    if (laDebug)
  ...
  // Now create an array with the latest profile found above and add all the other profiles
  // with a startDate between beginningOfFirstDay and endOfLastDay
  var profilesArray2 = [];
  profilesArray2.push(latestProfile); // Add the latest one
  profilesArray1.forEach(function (entry) {
    if (laDebug)
    ...
    if (moment(entry.startDate).isAfter(beginningOfFirstDay))
      profilesArray2.push(entry); // Add the profile if it's between beginning and end of show dates
  });
  ...
  if (laDebug) {
    profilesArray2.forEach(function (entry) {
      console.log('profilesArray2 - ' + entry.startDate);
    })
  }

  // Now for the deduplication
  var profiles = [];
  profiles.push(profilesArray2[0]); // Push the last profile with a startDate before beginningOfFirstDay. That should always be included.
  var profileToCompareWith = profiles[0];
  profilesArray2.forEach(function (entry) {
    if (JSON.stringify(profileToCompareWith.basal) != JSON.stringify(entry.basal)
      || JSON.stringify(profileToCompareWith.carbratio) != JSON.stringify(entry.carbratio)
      || JSON.stringify(profileToCompareWith.sens) != JSON.stringify(entry.sens)) {
      profiles.push(entry);
      profileToCompareWith = entry;
    }
  })

  // Now we have a deduplicated array of all relevant profiles. But for the sake of not cluttering the profiles table, only display max 3 of them
  if (laDebug) {
    console.log('Total of ' + profiles.length + ' profiles are relevant to display');
  ...
  profiles.forEach(function (theProfile, index) {
    ...
    if (index < 3) {
      var startDate = theProfile.startDate;
      var basal = theProfile.basal;
      var carbratio = theProfile.carbratio;
      var sens = theProfile.sens;

      tableHtml += '<td><table>';
      tableHtml += '<caption>' + translate('Profile') + ' ' + new Date(startDate).toLocaleString() + '</caption>';
      ...
      tableHtml += '<td><table>';
      carbratio.forEach(function (entry) {
      ...
      // Add ellipsis if too many profiles to display
...
loopalyzer.generateReport = function (datastorage, daysToShow, options) {
  ...
  if (datastorage.profiles && datastorage.profiles.length > 0)
