...
loopalyzer.html = function html(client) {
  ...
  var ret =
    ...
    */
    ...
    + '  <div class="chart" id="loopalyzer-chart1" style="height:500px;"></div>'
    + '  <div class="chart" id="loopalyzer-chart2" style="height:250px;"></div>'
    + '  <div class="chart" id="loopalyzer-chart3" style="height:250px;"></div>'
    + '</div>'
    ;
    return ret;
};
...
loopalyzer.css =
    '#loopalyzer-chart1, #loopalyzer-chart2 {'
  + '  width: 100%;'
  + '  height: 100%;'
  + '}'
  ;
...
loopalyzer.getSGVs = function(datastorage,sorteddaystoshow) {
...
loopalyzer.getBasals = function(datastorage, sorteddaystoshow, profile) {
  var bins = [];
  ...
  for (var hour = 0; hour < 24; hour++) {
    for (var minute = 0; minute < 60; minute = minute + minutewindow) {
      ...
      for (var m = 0; m < minutewindow; m = m + 5) {
        sorteddaystoshow.forEach(function(d) {
          var date = new Date(d);
          date.setHours(hour);
...
loopalyzer.getTempBasalDeltas = function(datastorage, sorteddaystoshow, profile) {
  var bins = [];
...
loopalyzer.getIOBsTEST = function(datastorage,sorteddaystoshow) {
  var bins = [];

  var filterFunc = function withinWindow(record) {
    if (record.loop && record.loop.iob) {
      var recdate = new Date(record.loop.iob.timestamp);
      return recdate.getHours() === hour && recdate.getMinutes() >= minute && recdate.getMinutes() < minute + minutewindow;
    }
    return false;
  };
  
  // Loop through all hours & minutes and days and get the IOBs, 
  // and push to bins so we can compute average etc later
  for (var hour = 0; hour < 24; hour++) {
    for (var minute = 0; minute < 60; minute = minute + minutewindow) {
      var date = new Date();
      date.setHours(hour);
      date.setMinutes(minute);
      var entries = datastorage.devicestatus.filter(filterFunc);
      entries = entries.map(function(record) {
        return entries.loop.iob.iob;
      });
      bins.push([date, entries]);
      console.log(date +  " - " + entries.length);
    }
  }
  return bins;
}
...
// PREDICTIONS START
//
// predictions is an array with [day, date, prediction]
// day is the sorteddaystoshow day, e.g. 2018-05-02
// date is the date/timestamp, e.g. 2018-05-02 02:00
// prediction is the prediction for that date, if any
//
loopalyzer.predictions = [];
loopalyzer.treatmentsTimestamps = []; // Only timestamps for (carbs and bolus insulin) treatments will be captured in this array

loopalyzer.getIOBs = function(datastorage, sorteddaystoshow) {
  var bins = [];

  // Prepare the bins with one timestamp for every 5 minutes and an empty array for the values
  for (var i=0; i<288; i++) {
  ...
  console.log('BINS with dates', bins);


  // Loop through all days to show, and for each day extract the IOBs and put into an array of 24*60/5 = 288 entries, one every 5 minute
  // Then loop thru those entries and where no IOB has been found, extrapolate between the nearby so we get a continuous array
  sorteddaystoshow.forEach(function(day) {
    ...
    var iobs=[]; // The array where we keep all the IOBs and we build up
    ...
    devicestatus.forEach(function(entry) {
      ...
      date.setHours(Math.floor(minutesAfterMidnight/60));
      date.setMinutes(minutesAfterMidnight % 60);
      iobs[index]=[date, entry.loop.iob.iob];
      */
    })

    console.log('IOBs for ' + day, iobs);

    // Now loop through all IOBs (and NaNs) for this day, and add to bins
    for (var i=0;i<288; i++) {
      bins[i][1].push(iobs[i]);
    }

    /*
    // Now we have an array of 288 items, some have an IOB value and some have NaN. Now we need to loop through the array and fill in the 
    for (var i=0; i<288; i++) {
      var j=i+1;
    }
    */
  ...
  console.log('Final IOB bin', bins);
  return bins;
};

/*
loopalyzer.preparePredictedData = function(datastorage, sorteddaystoshow) {

      // First sort the days to get them in chronological order so our treatment timestamps 
      sorteddaystoshow.sort(function(a,b) { return a>b });
//      loopalyzer.treatmentsTimestamps.push(dataRange[0]); // Create a fake timestamp at midnight so we can show predictions during night
        datastorage.treatments.forEach(function(treatment) {
          if (undefined != treatment.carbs && null != treatment.carbs && treatment.carbs > 0) {
            if (treatment.timestamp)
              loopalyzer.treatmentsTimestamps.push(treatment.timestamp);
            else if (treatment.created_at)
              loopalyzer.treatmentsTimestamps.push(treatment.created_at);
          }
      ...
      console.log('TREATMENTTIMESTAMPS', loopalyzer.treatmentsTimestamps);
      datastorage.devicestatus.forEach(function(entry) {
        if (entry.loop && entry.predicted)
...
loopalyzer.report = function report_loopalyzer(datastorage,sorteddaystoshow,options) {
  ...
  var sgvBin = loopalyzer.getSGVs(datastorage,sorteddaystoshow);
  var sgvDat50 = loopalyzer.dat50(sgvBin);

  var basalsBin = loopalyzer.getBasals(datastorage,sorteddaystoshow,profile);
  var basalsDat50 = loopalyzer.dat50(basalsBin);

  var tempBasalsBin = loopalyzer.getTempBasalDeltas(datastorage,sorteddaystoshow,profile);
  var tempBasalsDat50 = loopalyzer.dat50(tempBasalsBin);

  var iobBin = loopalyzer.getIOBs(datastorage,sorteddaystoshow);
  var iobDat50 = loopalyzer.dat50(iobBin);



  console.log('BASALS ARE', basalsBin);
  console.log('TEMPBASALS ARE', tempBasalsBin);
  ...
  var chart1Options = {
    ...
    yaxes: [{
      /* First axis is glucose - black */
      min: 0,
      max: options.units === 'mmol' ? 22: 400,
      tickColor: '#CCCCCC',
      position: "left",
    }, {
      /* Second axis is basal profile - blue */
      min: 0,
      max: 10,
      tickColor: '#0099FF',
      position: "right",
  ...
  var chart2Options = {
    ...
    yaxes: [{
      /* First axis is temp basal profile */
      min: -2,
      max: 4,
      tickColor: '#CCCCCC',
      position: "left",
...
/*
  ...
  d3.select('body').append('svg')
    ...
    charts = d3.select('#loopalyzerchart-' + day).html(
      '<b>'+
...
//            .text((t.percent ? (t.percent > 0 ? '+' : '') + t.percent + '%' : '') + (t.absolute ? Number(t.absolute).toFixed(2) + 'U' : ''));
    ...
    if (options.insulindistribution) {
      ...
      $('<tr><td colspan="2"><span id="loopalyzerstatinsulinpiechart-' + day + '"></span><span id="loopalyzerstatcarbspiechart-' + day + '"></span></td></tr>').appendTo(table);
      $('#loopalyzerstatchart-' + day).append(table);
      ...
      var svg = d3.select('#loopalyzerstatinsulinpiechart-' + day)
          .append('svg')
          .attr('width', width)
          .attr('height', height)
          .append('g')
      ...
      var carbssvg = d3.select('#loopalyzerstatcarbspiechart-' + day)
          .append('svg')
          .attr('width', width)
          .attr('height', height)
          .append('g')
