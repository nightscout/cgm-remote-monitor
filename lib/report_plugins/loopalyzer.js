...
loopalyzer.getIOBs = function(datastorage, daysToShow, profile, client, treatments) {
  ...
  daysToShow.forEach(function(day) {
    ...
    if (iobStatusAvailable) {
      ...
      var startIndex = 0
        , stopIndex = 0;
      while (startIndex < iobs.length && isNaN(iobs[parseInt(startIndex)])) {
        startIndex++; // Advance start to the first real number
      }
      if (startIndex < iobs.length) {
        stopIndex = startIndex + 1;
        while (stopIndex < iobs.length) {
          while (stopIndex < iobs.length && isNaN(iobs[parseInt(stopIndex)])) {
          ...
          if (stopIndex < iobs.length) {
            ...
            // if (stopIndex-startIndex<interpolationGap && (iobs[parseInt(stopIndex)] <= iobs[parseInt(startIndex)]*interpolationRatio || (stopIndex-startIndex<interpolationGap && iobs[parseInt(startIndex)]!==0))) {
            if (loopalyzer.canInterpolate(iobs, startIndex, stopIndex)) {
              var k = (iobs[parseInt(stopIndex)] - iobs[parseInt(startIndex)]) / (stopIndex - startIndex);
              var m = iobs[parseInt(startIndex)];
...
loopalyzer.getCOBs = function(datastorage, daysToShow, profile, client, treatments) {
  ...
  daysToShow.forEach(function(day) {
    ...
    if (cobStatusAvailable) {
      ...
      while (startIndex < cobs.length && isNaN(cobs[parseInt(startIndex)])) {
        startIndex++; // Advance start to the first real number
      }
      if (startIndex < cobs.length) {
        stopIndex = startIndex + 1;
        while (stopIndex < cobs.length) {
          while (stopIndex < cobs.length && isNaN(cobs[parseInt(stopIndex)])) {
            stopIndex++; // Advance stop to the first real number after start
          }
          if (stopIndex < cobs.length) {
            ...
            // Compute the y=k*x+m = (y2-y1)/(x2-x1)*x+y1
            // Only interpolate on decreasing or steady, or on increasing if the gap is less than interpolationGap
            // if (stopIndex-startIndex<interpolationGap && (cobs[parseInt(stopIndex)] <= cobs[parseInt(startIndex)]*interpolationRatio || (stopIndex-startIndex<interpolationGap && cobs[parseInt(startIndex)]!==0))) {
            if (loopalyzer.canInterpolate(cobs, startIndex, stopIndex)) {
              k = (cobs[parseInt(stopIndex)] - cobs[parseInt(startIndex)]) / (stopIndex - startIndex);
              m = cobs[parseInt(startIndex)];
              for (var x = 0; x < (stopIndex - startIndex); x++) {
...
loopalyzer.fillNanWithTreatments = function(array, treatments) {
  treatments.forEach(function(treatment) {
    ...
    if (isNaN(array[parseInt(index)])) {
      var start = index;
      var stop = index;

      // Now move left and right until we find real numbers, so not NaN
      while (start >= 0 && isNaN(array[parseInt(start)])) {
        start--;
      }
      while (stop < array.length && isNaN(array[parseInt(stop)])) {
        stop++;
      }
      // var gap = stop - start;
      // if (isNaN(array[parseInt(start)]) || isNaN(array[parseInt(stop)]) || gap > interpolationGap || (gap < interpolationGap && array[parseInt(start)]<array[parseInt(stop)])) {
      // if ( isNaN(array[parseInt(start)]) || isNaN(array[parseInt(stop)]) || (array[parseInt(start)] < array[parseInt(stop)] && (gap >= interpolationGap || array[parseInt(start)]==0)) ) {
      var interpolate = (isNaN(array[parseInt(start)]) || isNaN(array[parseInt(stop)]) ? true : loopalyzer.canInterpolate(array, start, stop));
      if (!interpolate) {
        array[parseInt(index)] = treatment.amount;
      }
    }
  })
}
...
loopalyzer.canInterpolate = function(array, start, stop) {
  ...
  if (array[parseInt(stop)] <= array[parseInt(start)] * interpolationRatio) {
    // Falling
    if (stop - start < fallingInterpolationGap) interpolate = true;
  } else {
    // Rising
    if (stop - start < risingInterpolationGap && array[parseInt(start)] !== 0) interpolate = true;
  }
  return interpolate;
}
...
loopalyzer.getAllPredictionsForADay = function(datastorage, day) {
  ...
  for (var i = datastorage.devicestatus.length - 1; i >= 0; i--) {
    if (datastorage.devicestatus[parseInt(i)].loop && datastorage.devicestatus[parseInt(i)].loop.predicted) {
      var predicted = datastorage.devicestatus[parseInt(i)].loop.predicted;
      if (moment(predicted.startDate).isSame(dayStart, 'day'))
        predictions.push(datastorage.devicestatus[parseInt(i)].loop.predicted);
    } else if (datastorage.devicestatus[parseInt(i)].openaps && datastorage.devicestatus[parseInt(i)].openaps.suggested && datastorage.devicestatus[parseInt(i)].openaps.suggested.predBGs) {
      var entry = {};
      entry.startDate = datastorage.devicestatus[parseInt(i)].openaps.suggested.timestamp;
      // For OpenAPS/AndroidAPS we fall back from COB if present, to UAM, then IOB
      if (datastorage.devicestatus[parseInt(i)].openaps.suggested.predBGs.COB) {
        entry.values = datastorage.devicestatus[parseInt(i)].openaps.suggested.predBGs.COB;
      } else if (datastorage.devicestatus[parseInt(i)].openaps.suggested.predBGs.UAM) {
        entry.values = datastorage.devicestatus[parseInt(i)].openaps.suggested.predBGs.UAM;
      } else entry.values = datastorage.devicestatus[parseInt(i)].openaps.suggested.predBGs.IOB;
...
loopalyzer.findPredicted = function(predictions, timestamp, offset) {
  ...
  if (offset && offset < 0) { // If offset is negative, start searching from first prediction going forward
    for (let i = 0; i < predictions.length; i++) {
      if (predictions[parseInt(i)] && predictions[parseInt(i)].startDate && moment(predictions[parseInt(i)].startDate) <= ts) {
        predicted = i;
      }
    }
  ...
  daysToShow.forEach(function(day) {
    ...
    if (predictions.length > 0 && treatmentTimestamps.length > 0) {
      ...
      for (var treatmentsIndex = 0; treatmentsIndex < treatmentTimestamps.length; treatmentsIndex++) {
        var timestamp = treatmentTimestamps[parseInt(treatmentsIndex)];
        ...
        if (predictedIndex != null) {
          var entry = predictions[parseInt(predictedIndex)]; // Start entry
          ...
          if (truncatePredictions) {
            if (predictedOffset >= 0) {
              ...
              end = moment(treatmentTimestamps[parseInt(treatmentsIndex)]);
            }
          }
          for (var entryIndex in entry.values) {
            if (!d.isAfter(end)) {
              ...
              var index = Math.floor(minutesAfterMidnight / 5);
              p[parseInt(index)] = client.utils.scaleMgdl(entry.values[parseInt(entryIndex)]);
              d.add(5, 'minutes');
            }
          }
        }
    ...
    for (let i = 0; i < 288; i++) {
      bins[parseInt(i)][1].push(p[parseInt(i)]);
    }
  })
  return bins;
}
...
loopalyzer.addArrayToBins = function(bins, values) {
  if (bins && bins.length === 288 && values && values.length === 288) {
    values.forEach(function(value, index) {
      bins[parseInt(index)][1].push(value);
    });
  } else
...
loopalyzer.interpolateArray = function(values, allowNegative) {
  ...
  while (isNaN(values[parseInt(startIndex)])) {
    startIndex++; // Advance start to the first real number
  }
  stopIndex = startIndex + 1;
  while (stopIndex < values.length) {
    while (stopIndex < values.length && isNaN(values[parseInt(stopIndex)])) {
      stopIndex++; // Advance stop to the first real number after start
    }
    if (stopIndex < values.length) {
      ...
      if (values[parseInt(stopIndex)] <= values[parseInt(startIndex)]) {
        k = (values[parseInt(stopIndex)] - values[parseInt(startIndex)]) / (stopIndex - startIndex);
        m = values[parseInt(startIndex)];
      }
      for (var x = 0; x < (stopIndex - startIndex); x++) {
...
loopalyzer.min = function(xBins) {
  ...
  for (var i = 0; i < xBins.length; i++) {
    if (isNaN(min) || min === null) min = xBins[parseInt(i)][1];
    if (!isNaN(xBins[parseInt(i)][1]) && xBins[parseInt(i)][1] < min) min = xBins[parseInt(i)][1];
  }
  return min;
}
...
loopalyzer.max = function(xBins) {
  var max = xBins[0][1];
  for (var i = 0; i < xBins.length; i++) {
    if (isNaN(max) || max === null) max = xBins[parseInt(i)][1];
    if (!isNaN(xBins[parseInt(i)][1]) && xBins[parseInt(i)][1] > max) max = xBins[parseInt(i)][1];
  }
  return max;
}
      ...
      if (minutes !== 0) {
        ...
        if (minutesBy5 > 0) {
          ...
          for (let i = 0; i < count; i++) {
            tempBin[i + minutesBy5] = bins[parseInt(i)][1][parseInt(dayIndex)];
          }
        }
        if (minutesBy5 < 0) {
          let count = 288 + minutesBy5;
          ...
          for (var i = 0; i < count; i++) {
            tempBin[parseInt(i)] = bins[i - minutesBy5][1][parseInt(dayIndex)];
          }
        }
        // Put the shifted data back into original bins variable (pass by pointer)
        for (let i = 0; i < 288; i++) {
          bins[parseInt(i)][1][parseInt(dayIndex)] = tempBin[parseInt(i)];
        }
      }
    });
  }
...
loopalyzer.timeShiftSingleBin = function(bin, daysToShow, timeShift) {
  if (bin && bin.length > 0) {
    daysToShow.forEach(function(day, dayIndex) {
      var minutesToAdd = timeShift[parseInt(dayIndex)];
      var date = moment(day);
      bin.forEach(function(entry, entryIndex) {
        ...
        if (entryDate.isSame(date, 'day')) {
          ...
          bin[parseInt(entryIndex)].date = entryDate.toDate();
        }
      })
    })
  }
    ...
    if (store) {
      for (var key in store) {
        ...
        if (Object.prototype.hasOwnProperty.call(store, key)) {
          var defaultProfile = store[String(key)];
    ...
    if (t2 > t1) {
        ...
        carbTreatments.forEach(function(entry) {
          if (!found && entry.amount >= mealMinCarbs) {
            ...
            if ((date.isSame(timeShiftBegin, 'minute') || date.isAfter(timeShiftBegin, 'minute')) &&
              ...
              firstCarbs[parseInt(dayIndex)] = minutesAfterMidnight;
              found = true;
              doTimeShift = true;
            }
          }
      ...
      firstCarbs.forEach(function(minutesAfterMidnight, index) {
        if (minutesAfterMidnight) { // Avoid NaN
          ...
          timeShifts[parseInt(index)] = delta;
        }
      });
