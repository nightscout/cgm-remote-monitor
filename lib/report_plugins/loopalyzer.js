...
loopalyzer.getAllTreatmentTimestampsForADay = function(datastorage, day) {
  ...
  timestamps.push(dayStart.toDate()); // Create a fake timestamp at midnight so we can show predictions during night
  ...
  console.log('TREATEMENT TIMESTAMPS for ' + day, timestamps);
  return timestamps;
}
...
loopalyzer.getAllPredictionsForADay = function(datastorage, day) {
  ...
  console.log('PREDICTIONS for ' + day, p);
  return p;
}
...
loopalyzer.getPredictions = function(datastorage, daysToShow, client) {
  ...
  daysToShow.forEach(function(day) {
          ...
          if (predictedIndex != null) {
            ...
            if (truncatePredictions) {
              if (predictedOffset >= 0) {
                ...
                if (treatmentsIndex < treatments.length - 1) {
            ...
            for (var entryIndex in entry.values) {
              if (!d.isAfter(end)) {
                ...
                if (index>287) {
                  console.log('OUT OF BOUNDS d=' + d + '   dayStart=' + dayStart + '   index=' + index);
                }
      ...
      // Now we have an array of 288 predictions in p, update the bins
//      console.log('Day=' + day + ', predictions is ', JSON.parse(JSON.stringify(p)));
//      console.log('Day=' + day + ', bins before', JSON.parse(JSON.stringify(bins)));
      for (var i=0; i<288; i++) {
        bins[i][1].push(p[i]);
      }
      console.log('Day=' + day + ', bins after', JSON.parse(JSON.stringify(bins)));
    })
    console.log('FINAL PREDICTIONS ARE',bins);
    return bins;
  }
...
loopalyzer.report = function report_loopalyzer(datastorage,sorteddaystoshow,options) {
  ...
  daysToShow.sort(function(a,b) { return (a<b ? -1:1)});
  
  var dateInfo = moment(daysToShow[0]).format('ll').split(',')[0];
  if (daysToShow.length>1) dateInfo += ' - ' + moment(daysToShow[daysToShow.length-1]).format('ll').split(',')[0];
  ...
  if ($("#rp_loopalyzertimeshift").is(":checked") && daysToShow.length>1) {
    ...
    if (t2>t1) {
      ...
      // console.log('TIMESHIFT START AT ', timeShiftStartTime);
      // console.log('TIMESHIFT END AT ', timeShiftStopTime);
