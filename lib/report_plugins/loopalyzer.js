...
loopalyzer.html = function html(client) {
  ...
  var ret = ''
    ...
    ret += '<span id="rp_loopalyzermealaligninput" style="display:none;">' /* So we can show only if viewing multiple days */
...
loopalyzer.report = function report_loopalyzer(datastorage,sorteddaystoshow,options) {
  ...
  if (daysToShow.length==1)
    $("#rp_loopalyzermealaligninput").hide(); /* Hide timeShift if single day */
  else
    $("#rp_loopalyzermealaligninput").show(); /* and show if multi days */
  ...
  if ($("#rp_loopalyzermealalign").is(":checked") && daysToShow.length>1) {
