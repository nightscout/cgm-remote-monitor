...
loopalyzer.css =
  '#loopalyzer-profiles-inner-table.td { background-color:red; color: green; font-size:40px; }' + 
  '#loopalyzer-profiles-inner-table td { background-color:red; color: green; font-size:40px; }' + 
  '#loopalyzer-profiles-outer-table.th { background-color:red; color: green; font-size:40px; }' + 
  '#loopalyzer-profiles-outer-table th { background-color:red; color: green; font-size:40px; }' + 
...
loopalyzer.renderProfilesTable = function(basal, carbRatio, sensitivity, client) {
  var translate = client.translate;
  console.log('........BASAL is ', basal);
  console.log('........CARBRATIO is ', carbRatio);
  console.log('........SENS is ', sensitivity);
  var tableHtml = '<p>' + translate('Most recent profile data') + '</p>';
  tableHtml += '<table id="loopalyzer-profiles-outer-table"><tr><th>' + translate('Basal') + '</th><th>' + translate('Carb ratio') + '</th><th>' + translate('Sensitivity') + '</th></tr>';
  
  // Add Basal as a table in the first td
  tableHtml += '<tr><td><table id="loopalyzer-profiles-inner-table">';
  basal.forEach(function(entry) {
    tableHtml += '<tr><td>' + entry.time + '</td><td>' + entry.value + '</td></tr>'
  });
  tableHtml += '</table></td>';
...
loopalyzer.generateReport = function(datastorage,daysToShow,options) {
  ...
  // Check if we have data in the profile and render the profiles table if there is
  if (profile.data && profile.data.length>0) {
    var profileDefault = profile.data[0].store.Default;
    var defaultBasal = profileDefault.basal;
    var defaultCarbRatio = profileDefault.carbratio;
    var defaultSensitivity = profileDefault.sens;
    loopalyzer.renderProfilesTable(defaultBasal, defaultCarbRatio, defaultSensitivity, client);
  } else {
    if (loopalyzer.laDebug)
      console.log('........NO DATA AVAILABLE IN PROFILE.DATA........');
  }
