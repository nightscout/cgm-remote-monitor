...
var laDebug = true; // If we should print console.logs
var laVersion = '2019-01-16 v3';
...
loopalyzer.html = function html(client) {
  ...
  var ret = ''
    ...
    ret += '</div>'
    ret += '<div id="loopalyzer-profiles" style="padding:20px;">'
    ret += '</div>'
    ;
    return ret;
};
...
loopalyzer.css =
  ...
  + '  height: 100%;'
  + '}'
  + '#loopalyzer-profiles-table { margin: 0 10px; border-collapse: collapse; border: 0px; }'
  + '#loopalyzer-profiles-table caption { text-align: left; font-weight: bold; }'
  + '#loopalyzer-profiles-table th { background-color: #4CAF50; color: white; }'
  + '#loopalyzer-profiles-table td { text-align: right; vertical-align: top; padding: 0 20px; }'
  + '#loopalyzer-profiles-table td td { padding: 1px 8px; }'
  ;
...
loopalyzer.renderProfilesTable = function(datastoreProfiles, daysToShow, client) {
  ...
  // Sort the profiles on startDate ascending (make a clone so we don't modify the Store array)
  var profiles = [];
  ...
  profiles.sort(function(a,b) { return (a.startDate > b.startDate ? 1:-1) }); // Ascending
  if (laDebug) {
    profiles.forEach(function(entry) {
      console.log('PROFILE ' + entry.startDate);
    })
  }

  // Loop through the profiles starting with the first one, find the latest one that has 
  // a start date less than or equal to the timestamp of the first day displayed
  var theProfile = profiles[0];
  profiles.forEach(function(entry) {
    if (laDebug) 
  ...
  if (laDebug)
    console.log('CHOSEN PROFILE FOR THIS VIEW IS ' + theProfile.startDate);
  ...
  var translate = client.translate;
  var tableHtml = '<table id="loopalyzer-profiles-table">'
  tableHtml += '<caption>' + translate('Most recent profile') + ' ' + new Date(startDate).toLocaleString() + ':</caption>';
