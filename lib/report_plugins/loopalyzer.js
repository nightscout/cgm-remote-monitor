...
loopalyzer.renderProfilesTable = function(startDate, basal, carbRatio, sensitivity, client) {
  var translate = client.translate;
...
loopalyzer.generateReport = function(datastorage,daysToShow,options) {
  ...
  if (datastorage.profiles && datastorage.profiles.length>0) {
    var startDate = datastorage.profiles[0].startDate;
    var lastProfile = datastorage.profiles[0].store.Default;
    var lastBasal = lastProfile.basal;
    var lastCarbRatio = lastProfile.carbratio;
    var lastSensitivity = lastProfile.sens;
    loopalyzer.renderProfilesTable(startDate, lastBasal, lastCarbRatio, lastSensitivity, client);
  }
