...
loopalyzer.generateReport = function(datastorage, daysToShow, options) {
  ...
  var chartBasalData = [{
    ...
    , color: basalColor
    , points: { show: false }
    , bars: { show: true, fill: true, barWidth: [barWidth, true] }
    , yaxis: 1
  }];
  ...
  var chartTempBasalData = [{
    ...
    , color: insulinColor
    , points: { show: false }
    , bars: { show: true, barWidth: [barWidth, true] }
  }];
  ...
  // Preserve the zero line at the center when there is no temp-basal delta.
  // Flot 4 otherwise expands an all-zero range only above zero.
  if (!tempBasalsAvg.some(point => Number.isFinite(point[1]) && point[1] !== 0)) {
    Object.assign(chartTempBasalOptions.yaxes[0], {autoScale: 'none', min: -1, max: 1});
  }
  ...
  var chartCOBData = [{
    ...
    , color: carbColor
    , points: { show: false }
    , bars: { show: true, fil: true, barWidth: [barWidth, true] }
  }];
  var chartCOBOptions = {
    xaxis: xaxisCfg
    , yaxes: [{
