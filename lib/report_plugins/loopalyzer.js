...
loopalyzer.timeShiftSingleBin = function(bin, daysToShow, timeShift) {
  console.log('timeShiftSingleBin before: ' + JSON.stringify(bin), null, 2);
  daysToShow.forEach( (day, dayIndex) => {
    var minutesToAdd = timeShift[dayIndex];
    var date = moment(day);
    bin.forEach( (entry, entryIndex) => {
  ...
  console.log('timeShiftSingleBin after: ' + JSON.stringify(bin), null, 2);
}
...
loopalyzer.report = function report_loopalyzer(datastorage,sorteddaystoshow,options) {
  ...
  //
  // Check to see if we are asked to do timeShift
  //
  if ($("#rp_loopalyzermealalign").is(":checked")) {
        ...
        if (!found && entry.amount >= mealMinCarbs) {
          ...
          if ( (date.isSame(timeShiftBegin,'minute') || date.isAfter(timeShiftBegin,'minute')) &&
               (date.isSame(timeShiftEnd,'minute') || date.isBefore(timeShiftEnd,'minute')) ) {
                console.log(day + ',  ' + entry.amount + ' carbs on ' + date);
    ...
    /*
    // For debugging purpose timeshift 60 minutes forward
    timeShifts.forEach( (entry, index) => {
      timeShifts[index] = -60;
    });
    */
    ...
    var carbTreatmentsTable = '<b>BEFORE TIMESHIFT</b><table><tr><th>Date</th><th>Carbs</th></tr>';
    carbTreatments.forEach(entry => {
      var d = moment(entry.date).format('YYMMDD HH:mm');
      carbTreatmentsTable += '<tr><td>'+d+'</td><td>'+entry.amount+'</td></tr>';
    });
    carbTreatmentsTable += '</table>';
    loopalyzer.timeShiftSingleBin(carbTreatments, daysToShow, timeShifts);
    carbTreatmentsTable += '<b>AFTER TIMESHIFT</b><table><tr><th>Date</th><th>Carbs</th></tr>';
    carbTreatments.forEach(entry => {
      var d = moment(entry.date).format('YYMMDD HH:mm');
      carbTreatmentsTable += '<tr><td>'+d+'</td><td>'+entry.amount+'</td></tr>';
    });
    carbTreatmentsTable += '</table>';
    $('#loopalyzer-carbTreatmentsTable').append($(carbTreatmentsTable));


    var insulinTreatmentsTable = '<b>BEFORE TIMESHIFT</b><table><tr><th>Date</th><th>Insulin</th></tr>';
    insulinTreatments.forEach(entry => {
      var d = moment(entry.date).format('YYMMDD HH:mm');
      insulinTreatmentsTable += '<tr><td>'+d+'</td><td>'+entry.amount+'</td></tr>';
    });
    insulinTreatmentsTable += '</table>';
    loopalyzer.timeShiftSingleBin(insulinTreatments, daysToShow, timeShifts);
    insulinTreatmentsTable += '<b>AFTER TIMESHIFT</b><table><tr><th>Date</th><th>Insulin</th></tr>';
    insulinTreatments.forEach(entry => {
      var d = moment(entry.date).format('YYMMDD HH:mm');
      insulinTreatmentsTable += '<tr><td>'+d+'</td><td>'+entry.amount+'</td></tr>';
    });
    insulinTreatmentsTable += '</table>';
    $('#loopalyzer-insulinTreatmentsTable').append($(insulinTreatmentsTable));
