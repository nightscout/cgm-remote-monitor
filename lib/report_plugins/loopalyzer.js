...
loopalyzer.renderProfilesTable = function (datastoreProfiles, daysToShow, client) {
  ...
  profilesArray1.sort(function (a, b) { return (a.startDate > b.startDate ? -1 : 1) }); // Descending
  if (laDebug) {
  ...
  profiles.forEach(function (theProfile, index) {
    ...
    if (index < 3) {
      tableHtml += '<td><table>';
      tableHtml += '<caption>' + theProfile.profileName + ' (' + translate('Profile') + ' ' + new Date(theProfile.startDate).toLocaleString() + ')</caption>';
