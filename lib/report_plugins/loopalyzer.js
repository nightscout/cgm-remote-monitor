...
//            console.log('...predictedIndex is ' + predictedIndex);
//            console.log('...d is ' + d.toDate());
//            console.log('...end is' + end.toDate());
            if (truncatePredictions) {
              if (predictedOffset >= 0) {
//                console.log('treatmentTimestamps', treatmentTimestamps);
...
//            console.log('...final end is ' + end.toDate());
            for (var entryIndex in entry.values) {
              console.log('...comparing d ' + d.toDate() + ' with end ' + end.toDate());
              if (!d.isAfter(end)) {
...
loopalyzer.report = function report_loopalyzer(datastorage,sorteddaystoshow,options) {
  ...
  var iobBin = (client.plugins('iob').isDeviceStatusAvailable(datastorage.devicestatus) ? loopalyzer.getIOBs(datastorage,daysToShow) : []);
  var cobBin = (client.plugins('cob').isDeviceStatusAvailable(datastorage.devicestatus) ? loopalyzer.getCOBs(datastorage,daysToShow) : []);
  var predictionsBin = [];
