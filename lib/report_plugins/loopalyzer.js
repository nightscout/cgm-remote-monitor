...
var interpolationGap = 6; // How large a gap in COB/IOB graph is allowed to be to be interpolated if end value is larger than start
...
loopalyzer.html = function html(client) {
  ...
  var ret = ''
    ...
    ret += '</b></span>'
    ret += '<span id="loopalyzer-notenoughdata" style="display:none;"><b>' + translate('Please select no more than one week duration and click Show again.') + '</b><br/><br/></span>'
  ...
  daysToShow.forEach(function(day, dayIndex) {
    ...
    if (iobStatusAvailable) {
      ...
      if (startIndex < iobs.length) {
        ...
        while (stopIndex<iobs.length) {
          ...
          if (stopIndex<iobs.length) {
            ...
            if (iobs[stopIndex] <= iobs[startIndex]*interpolationRatio || (stopIndex-startIndex<interpolationGap && iobs[startIndex]!==0)) {
      ...
      for (var dt=dayStart; dt < dayEnd; dt.add(5, 'minutes')) {
        ...
        iobs.push(iob);
        if (laDebug) console.log('getIOBs ' + dt.format() + '  iob=' + iob);
      }
    }
  ...
  daysToShow.forEach(function(day, dayIndex) {
    ...
    if (cobStatusAvailable) {
      ...
      if (startIndex < cobs.length) {
        ...
        while (stopIndex<cobs.length) {
          ...
          if (stopIndex<cobs.length) {
            ...
            // Compute the y=k*x+m = (y2-y1)/(x2-x1)*x+y1
            // Only interpolate on decreasing or steady, or on increasing if the gap is less than interpolationGap
            if (cobs[stopIndex] <= cobs[startIndex]*interpolationRatio || (stopIndex-startIndex<interpolationGap && cobs[startIndex]!==0)) {
      ...
      for (var dt=dayStart; dt < dayEnd; dt.add(5, 'minutes')) {
        ...
        cobs.push(cob);
        if (laDebug) console.log('getCOBs ' + dt.format() + '  cob=' + cob);
      }
    }
...
loopalyzer.fillNanWithTreatments = function(array, treatments) {
  treatments.forEach(function(treatment, treatmentIndex) {
    ...
    if (isNaN(array[index])) {
      ...
      // console.log('fillNanWithTreatments treatment ' + treatmentIndex, treatment);
      // console.log('fillNanWithTreatments index=' + index);
      ...
      var gap = stop - start;
      // console.log('fillNanWithTreatments start found at ' + start + ' and stop at ' + stop);
      // console.log('fillNanWithTreatments array[' + start + ']=' + array[start]);
      // console.log('fillNanWithTreatments array[' + stop + ']=' + array[stop]);
...
//        console.log('fillNanWithTreatments inserting ' + treatment.amount + ' at index ' + index);
//        console.log('fillNanWithTreatments array[' + index + '] before is ' + array[index]);
        array[index] = treatment.amount;
//        console.log('fillNanWithTreatments array[' + index + '] after is ' + array[index]);
      }
    }
  })
}
...
loopalyzer.report = function(datastorage,sorteddaystoshow,options) {
  ...
  console.log('LOOPALYZER ' + firstDay.format() + ' - ' + lastDay.format() + ' is ' + days + ' days');
  if (days <= 7) {
...
loopalyzer.generateReport = function(datastorage,daysToShow,options) {
  console.log('Loopalyzer 2018-06-04 v2');
