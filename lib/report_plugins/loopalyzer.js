...
loopalyzer.renderProfilesTable = function (datastoreProfiles, daysToShow, client) {
  ...
  datastoreProfiles.forEach(function (entry) {
    ...
    var store=eval(entry.store); // convert profile store to Javascript
    if (store) {
      store.forEach(function (defaultProfile) {
      if (laDebug) console.log('profile ' + defaultProfile);
      if (defaultProfile.basal) newEntry.basal = defaultProfile.basal;
      if (defaultProfile.carbratio) newEntry.basal = defaultProfile.carbratio;
      if (defaultProfile.sens) newEntry.basal = defaultProfile.sens;
