...
loopalyzer.html = function html(client) {
  ...
  var ret = ''
    ...
    ret += '</span>' /* timeShift */
    ret += '<br/>'
    ret += '<input type="checkbox" id="rp_loopalyzerprofiles" checked>'+translate('Show profiles table')
    ret += '<br/><br/>'
...
loopalyzer.renderProfilesTable = function (datastoreProfiles, daysToShow, client) {
  ...
  datastoreProfiles.forEach(function (entry) {
    if (laDebug) {
      console.log('datastoreProfiles.forEach(' + entry + ')');
    }
    var newEntry = {};
    newEntry.startDate = entry.startDate;
    if (entry.store.Default.basal) newEntry.basal = entry.store.Default.basal;
    if (entry.store.Default.carbratio) newEntry.carbratio = entry.store.Default.carbratio;
    if (entry.store.Default.sens) newEntry.sens = entry.store.Default.sens;
  ...
  profilesArray1.forEach(function (entry) {
    ...
    var differentBasal = profileToCompareWith.basal && entry.basal && JSON.stringify(profileToCompareWith.basal) != JSON.stringify(entry.basal);
    var differentCarbratio = profileToCompareWith.carbratio && entry.carbratio && JSON.stringify(profileToCompareWith.carbratio) != JSON.stringify(entry.carbratio);
    var differentSens = profileToCompareWith.sens && entry.sens && JSON.stringify(profileToCompareWith.sens) != JSON.stringify(entry.sens);
    if (laDebug) console.log('basal=' + differentBasal + ', carbratio=' + differentCarbratio + ', sens=' + differentSens);
    if ( (profileToCompareWith.basal && entry.basal && JSON.stringify(profileToCompareWith.basal) != JSON.stringify(entry.basal))
      || (profileToCompareWith.carbratio && entry.carbratio && JSON.stringify(profileToCompareWith.carbratio) != JSON.stringify(entry.carbratio))
      || (profileToCompareWith.sens && entry.sens && JSON.stringify(profileToCompareWith.sens) != JSON.stringify(entry.sens)) )  {
  ...
  profiles.forEach(function (theProfile, index) {
    ...
    if (index < 3) {
      tableHtml += '<td><table>';
      tableHtml += '<caption>' + translate('Profile') + ' ' + new Date(theProfile.startDate).toLocaleString() + '</caption>';
