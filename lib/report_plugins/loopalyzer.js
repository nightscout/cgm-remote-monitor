...
loopalyzer.isSameProfileValues = function(a,b) {
  if (a && !b) return false; // If a has values but b doesn't they are different
  if (!a && b) return false; // if a doesn't have values but b does they are also different
  if (JSON.stringify(a.timeInSeconds) != JSON.stringify(b.timeInSeconds)) return false;
  if (JSON.stringify(a.time) != JSON.stringify(b.time)) return false;
...
loopalyzer.renderProfilesTable = function (datastoreProfiles, daysToShow, client) {
  ...
  profilesArray1.forEach(function (entry) {
    ...
    var isSame = true; // We'll set to false when we have discovered a difference
    if (!loopalyzer.isSameProfileValues(profileToCompareWith.basal, entry.basal)) isSame = false;
    if (!loopalyzer.isSameProfileValues(profileToCompareWith.carbratio, entry.carbratio)) isSame = false;
    if (!loopalyzer.isSameProfileValues(profileToCompareWith.sens, entry.sens)) isSame = false;
    if (isSame)  {
