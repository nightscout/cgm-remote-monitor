...
loopalyzer.getIOBs = function(datastorage, sorteddaystoshow) {
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    for (var i=0; i<288; i++) iobs.push(0); // Clear the IOBs by filling with NaNs
...
loopalyzer.getCOBs = function(datastorage, sorteddaystoshow) {
  var bins = [];
  ...
  // Loop through all days to show, and for each day extract the COBs and put into an array of 24*60/5 = 288 entries, one every 5 minute
  // Then loop thru those entries and where no COB has been found, extrapolate between the nearby so we get a continuous array
  sorteddaystoshow.forEach(function(day) {
    var dayStart = moment(day); // Create something so we can compute delta number of minutes later
    ...
    var cobs=[]; // The array where we keep all the COBs and we build up
    for (var i=0; i<288; i++) cobs.push(0); // Clear the COBs by filling with NaNs
    ...
    // Now loop through all COBs (and NaNs) for this day, and add to bins
    for (var i=0;i<288; i++) {
      bins[i][1].push(cobs[i]);
    }
  });
...
loopalyzer.report = function report_loopalyzer(datastorage,sorteddaystoshow,options) {
  ...
  var cobBin = loopalyzer.getCOBs(datastorage,sorteddaystoshow);
  ...
  var chart3Data = [{
    data: iobDat50,
    label: translate('Insulin on board, IOB'),
    id: 'iobs',
    color: '#FF7100',
    points: {
      show: false
    ...
    data: cobDat50,
    label: translate('Carbs on board, COB'),
    id: 'cobs',
    color: '#00AAFF',
    points: {
      show: false
  ...
  var chart3Options = {
    ...
    yaxes: [{
      /* First axis is IOB - black */
      min: -2,
      max: 12,
      tickColor: '#CCCCCC',
      position: "left",
    }, {
      /* Second axis is COB (line) - blue */
      min: 0,
      max: 100,
      tickColor: '#0099FF',
      position: "right",
