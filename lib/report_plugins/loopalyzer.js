...
loopalyzer.html = function html(client) {
  ...
  var ret = ''
    ret += '<h2>' + translate('Loopalyzer') + '&nbsp;&nbsp;<span id="dateinfo"></span></h2>'
...
loopalyzer.getTempBasalDeltas = function(datastorage, daysToShow, profile) {
  ...
  for (var i=0; i<288; i++) {
    ...
    daysToShow.forEach(function(day){
      var d = moment(day);
      d.set({'hours':date.hours(), 'minutes':date.minutes(), 'seconds':0, 'milliseconds':0});
      var basalValue = profile.getTempBasal(d.toDate());
      basalValues.push(basalValue.tempbasal - basalValue.basal);
    });
    bins.push([date.toDate(), basalValues]);
    date.add(5, 'minutes')
  }
  return bins;
}
...
/*
loopalyzer.getIOBsOLD = function(datastorage, daysToShow) {
  ...
  var hasIOB = false; // Updated later, if still remaining false we can not show IOB and must return empty array
  ...
  // If we haven't found any IOB we return empty array
  if (!hasIOB)
    return [];
...
loopalyzer.interpolateArray = function(values) {
...
loopalyzer.getIOBs = function(datastorage, daysToShow, profile, client) {
  var iobStatusAvailable = client.plugins('iob').isDeviceStatusAvailable(datastorage.devicestatus);
  console.log('iobStatusAvailable=' + iobStatusAvailable);
  ...
  daysToShow.forEach(function(day, dayIndex) {
    ...
    if (iobStatusAvailable) {
      ...
      console.log('iobArray', iobArray);
      iobArray.forEach(function(entry){
        var index = Math.floor(moment(entry.mills).diff(dayStart,'minutes') / 5);
        iobs[index] = entry.iob;
      });
      loopalyzer.interpolateArray(iobs);
    } else {
      for (var dt=dayStart; dt < dayEnd; dt.add(5, 'minutes')) {
        var iob = client.plugins('iob').iobTotal(datastorage.treatments,datastorage.devicestatus,profile,dt.toDate()).iob;
        iobs.push(iob);
        console.log(dt.format() + '  iob=' + iob);
      }
    }
    console.log('getIOBs.. ' + day, iobs);
    loopalyzer.addArrayToBins(bins, iobs);
  });
  return bins;
}

loopalyzer.getCOBs = function(datastorage, daysToShow, profile, client) {
  var cobStatusAvailable = client.plugins('cob').isDeviceStatusAvailable(datastorage.devicestatus);
  console.log('cobStatusAvailable=' + cobStatusAvailable);
...
/*
loopalyzer.getCOBsOLD = function(datastorage, daysToShow) {
  ...
  var hasCOB = false; // Updated later, if still remaining false we can not show COB and must return empty array
  ...
  // If we haven't found any COB we return empty array
  if (!hasCOB)
    return [];
...
loopalyzer.report = function report_loopalyzer(datastorage,sorteddaystoshow,options) {
  ...
  var iobBin = loopalyzer.getIOBs(datastorage,daysToShow, profile, client);
  var cobBin = loopalyzer.getCOBs(datastorage,daysToShow, profile, client);
  var predictionsBin = [];
