...
loopalyzer.html = function html(client) {
  ...
  var ret =
    ...
    + '<input type="checkbox" id="rp_loopalyzertreatments" checked>'+translate('Show bolus &amp; carb treatments')+'&nbsp;&nbsp;&nbsp;&nbsp;'
    + '<input type="checkbox" id="rp_loopalyzermealalign">'
    + translate('Timeshift meals larger than')
    + '<input type="number" size="1" value="10" id="rp_loopalyzermincarbs">'+' '+translate('g carbs')
    + '&nbsp;'+translate('consumed between')
    + ' <select id="rp_loopalyzert1">'
    + '  <option t1="06:00">06:00</option>'
    + '  <option t1="07:00">07:00</option>'
    + '  <option t1="08:00">08:00</option>'
    + '  <option t1="09:00">09:00</option>'
    + '  <option t1="10:00">10:00</option>'
    + '</select>'
    + '  '+translate('and')
    + ' <select id="rp_loopalyzert2">'
    + '  <option t2="06:00">06:00</option>'
    + '  <option t2="07:00">07:00</option>'
    + '  <option t2="08:00">08:00</option>'
    + '  <option t2="09:00">09:00</option>'
    + '  <option t2="10:00">10:00</option>'
    + '</select>'
    + '<br/><br/>'
    + '<input type="button" onclick="loopalyzerMoreBackward();" value="&lt;&lt;&lt;&nbsp;' + translate('Backward')+'">'
    ...
    + '<input type="button" onclick="loopalyzerMoreForward();" value="' + translate('Forward')+'&nbsp;&gt;&gt;&gt;">'
    + '</div>'
...
loopalyzer.getCarbTreatments = function(datastorage, daysToShow) {
  ...
  datastorage.treatments.filter(treatment => treatment.carbs && treatment.carbs >0).forEach(treatment => {
    if (moment(treatment.created_at).isBetween(startDate, endDate)) {
      treatments.push({date:treatment.created_at, carbs:treatment.carbs});
    }
  })
  return treatments;
}
...
loopalyzer.getInsulinTreatments = function(datastorage, daysToShow) {
  ...
  datastorage.treatments.filter(treatment => treatment.insulin && treatment.insulin >0).forEach(treatment => {
    if (moment(treatment.created_at).isBetween(startDate, endDate)) {
      treatments.push({date:treatment.created_at, insulin:treatment.insulin});
    }
  })
  return treatments;
}
...
loopalyzer.report = function report_loopalyzer(datastorage,sorteddaystoshow,options) {
  ...
  if ($("#rp_loopalyzermealalign").is(":checked")) {
    ...
    console.log(mealMinCarbs);    
    console.log(t1 + '  ' + timeShiftBegin, timeShiftBegin);
    console.log(t2 + '  ' + timeShiftEnd, timeShiftEnd);

    console.log('cobBin', cobBin);
  ...
  if ($("#rp_loopalyzertreatments").is(":checked")) {
    ...
    insulinTreatments.forEach(treatment => {
      ...
      markings.push( { xaxis: { from: startDate.toDate(), to: endDate.toDate()}, yaxis: { from: 0, to: treatment.insulin }, color: markingColor } );
    })
  }
  ...
  if ($("#rp_loopalyzertreatments").is(":checked")) {
    ...
    carbTreatments.forEach(treatment => {
      ...
      endDate.set(todayJSON);
      endDate.add(5, 'minutes');
      markings.push( { xaxis: { from: startDate.toDate(), to: endDate.toDate()}, yaxis: { from: 0, to: treatment.carbs }, color: markingColor } );
    })
  }
