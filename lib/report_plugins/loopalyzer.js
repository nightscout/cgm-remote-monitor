...
loopalyzer.html = function html(client) {
  ...
  var ret =
    ...
    */
    ...
    + '  <div class="chart" id="loopalyzer-chart1" style="height:600px;"></div>'
...
loopalyzer.getIOBs = function(datastorage, sorteddaystoshow) {
  ...
  console.log("getIOBs sorteddays before & after");
  sorteddaystoshow.forEach(d => {console.log(d)});
  var days = sorteddaystoshow.sort((a,b) => {return a>b});
  sorteddaystoshow.forEach(d => {console.log(d)});
...
loopalyzer.getCOBs = function(datastorage, sorteddaystoshow) {
  ...
  // We re-sort the sorteddaystoshow in ascending order to make it easier to fill in the blank NaNs later
  console.log("getCOBs sorteddays before & after");
  sorteddaystoshow.forEach(d => {console.log(d)});
  var days = sorteddaystoshow.sort((a,b) => {return a>b});
  sorteddaystoshow.forEach(d => {console.log(d)});
...
loopalyzer.report = function report_loopalyzer(datastorage,sorteddaystoshow,options) {
  ...
  var sgvMin = loopalyzer.ss.min(sgvBin);
  var sgvMax = loopalyzer.ss.max(sgvBin);
  var basalsMin = loopalyzer.ss.min(basalsBin);
  var basalsMax = loopalyzer.ss.max(basalsBin);
  var tempBasalsMin = loopalyzer.ss.min(tempBasalsBin);
  var tempBasalsMax = loopalyzer.ss.max(tempBasalsBin);
  var iobMin = loopalyzer.ss.min(iobBin);
  var iobMax = loopalyzer.ss.max(iobBin);
  var cobMin = loopalyzer.ss.min(cobBin);
  var cobMax = loopalyzer.ss.max(cobBin);
  ...
  var chart1Options = {
    ...
    yaxes: [{
      /* First axis is glucose - black, we push it a bit north to make room for basal profile below */
      min: options.units === 'mmol' ? 3 : 55,
