...
function appendLeadingTableRow(table, header, current, recommended) {
  var tr = $('<tr/>');
...
autotune.report = function report_autotune(dataStorage, sorteddaystoshow, options) {
  ...
  appendLeadingTableRow(table, 'ISF', '', dataStorage.sens);
  appendLeadingTableRow(table, 'CSF', '', dataStorage.csf);
  appendLeadingTableRow(table, 'Carb Ratio', '', dataStorage.carb_ratio);
  appendLeadingTableRow(table, 'Basal Profile', '', dataStorage.basalprofile);
};
