...
var lastResult = null;
var getIOB = require('./autotune/iob');
var autotune = {
  name: 'autotune'
  , label: 'Autotune'
  , pluginType: 'report'
  ...
  for (iTime = 0; iTime < 24*2; iTime++) {
    ...
    if (recommendedRecord !== null) {
      ...
      if (lastBasalRecord !== null){
        ...
        {
          if (recommendedRecord.rate / lastBasalRecord.value >= criticalLimit)
            prefix += "[!]";
          prefix += "[up]";
        }
        else if (lastBasalRecord.value > recommendedRecord.rate) {
          if (lastBasalRecord.value / recommendedRecord.rate >= criticalLimit)
            prefix += "[!]";
          prefix += "[dn]";
        }
      }
      $('<td>' + prefix + recommendedRecord.rate + '</td>').appendTo(tr);
    }
    else {
      $('<td/>').appendTo(tr);
    }
    ...
    if (recommendedRecord && basalRecord) {
      appendArrowIcon(lastTd, basalRecord.value, recommendedRecord.rate);
    }
...
autotune.generate = function generate (inputs) {
  ...
  var treatments = autotune.find_meals(inputs);
...
autotune.find_meals = function findMealInputs (inputs) {
...
autotune.sum = function categorizeBGDatums(opts) {
    ...
    for (var i=bucketedData.length-5; i > 0; --i) {
        ...
        //sens = ISF
        var sens = autotune.isfLookup(IOBInputs.profile.isfProfile,BGDate);
        ...
        currentBasal = autotune.basalLookup(opts.basalprofile, BGDate);
        BGDate1hAgo = new Date(BGTime-1*60*60*1000);
        BGDate2hAgo = new Date(BGTime-2*60*60*1000);
        BGDate3hAgo = new Date(BGTime-3*60*60*1000);
        basal1hAgo = autotune.basalLookup(opts.basalprofile, BGDate1hAgo);
        basal2hAgo = autotune.basalLookup(opts.basalprofile, BGDate2hAgo);
        basal3hAgo = autotune.basalLookup(opts.basalprofile, BGDate3hAgo);
