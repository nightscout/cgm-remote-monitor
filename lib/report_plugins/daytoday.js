...
var showInsulinUnitsWithCarbTreatment = false;
module.exports = init;
...
//            .text((t.percent ? (t.percent > 0 ? '+' : '') + t.percent + '%' : '') + (t.absolute ? Number(t.absolute).toFixed(2) + 'U' : ''));
      ...
      if (treatment.carbs && options.carbs) {
        ...
        context.append('text')
          ...
          .attr('transform', 'translate(' +
              (xScale2(treatment.mills) + padding.left + 7) + ',' + (padding.top + yCarbsScale(treatment.carbs) - 7) + ') ' +
               'rotate(-45)')
          .text(treatment.carbs + ' g');
      }
      
      if (treatment.insulin && options.insulin) {
        var dataLabel = (Math.round(Number(treatment.insulin)*100)/100)+ ' U';
        context.append('rect')
        ...
        context.append('text')
          ...
          .attr('transform', 'translate(' +
              (xScale2(treatment.mills) + padding.left + 7 - 2) + ',' + (padding.top+yInsulinScale(treatment.insulin) - 7) + ') ' +
              'rotate(-45)')
          .text(dataLabel);
      }
    ...
    function appendProfileSwitch(context, treatment) {
      ...
      context.append('text')
        ...
        .attr('dy', '.35em')
        .attr('transform', 'translate(' + (xScale2(treatment.mills) + padding.left) + ',' + (yScaleBasals(0) + padding.top - 10) + ') rotate(-90)' )
        .text(text);
    }
