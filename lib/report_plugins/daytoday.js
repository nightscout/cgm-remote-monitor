...
daytoday.report = function report_daytoday(datastorage,sorteddaystoshow,options) {
  ...
  d3.select('body').append('svg')
    ...
    var morningLimits = {
      start: (new Date(day)).setHours(5),
      end:(new Date(day)).setHours(11)
    };
    var afternoonLimits = {
      start: (new Date(day)).setHours(11),
      end:(new Date(day)).setHours(24)
    };

    var timeLimits = [morningLimits, afternoonLimits];
    var carbratioAM = { totCarb:0, totBolus:0};
    var carbratioFM = { totCarb:0, totBolus:0};
    var carbRatios =[carbratioAM, carbratioFM];
    ...
    for (var dt=moment(from); dt < to; dt.add(5, 'minutes')) {
      ...
      if (options.basal)  {
        ...
        for(i = 0; i < 2; i++)
        {
          if(timeLimits[i].start < dt && timeLimits[i].end > dt)
          {
              carbRatios[i].totBolus += tempPart;
          }
        }
...
//            .text((t.percent ? (t.percent > 0 ? '+' : '') + t.percent + '%' : '') + (t.absolute ? Number(t.absolute).toFixed(2) + 'U' : ''));
    ...
    data.treatments.forEach(function (treatment) {
      ...
      var dateString = new Date(treatment.created_at);
      var dateMills= new Date(treatment.mills);

      var i;
      for(i = 0; i < 2; i++)
      {
        if(timeLimits[i].start < dateMills && timeLimits[i].end > dateMills)
        {
          if(treatment.insulin)
          {
            carbRatios[i].totBolus += treatment.insulin;
          }
          if(treatment.carbs)
          {
            carbRatios[i].totCarb += treatment.carbs;
          }
        }
      }
    ...
    if (options.insulindistribution) {
      ...
      $('<tr><td><b>' + translate('Carb ratio AM:') + '</b></td><td align="right">' + (carbRatios[0].totCarb/carbRatios[0].totBolus).toFixed(1) + 'g/U</td></tr>').appendTo(table);
      $('<tr><td><b>' + translate('Carb ratio PM:') + '</b></td><td align="right">' + (carbRatios[1].totCarb/carbRatios[1].totBolus).toFixed(1) + 'g/U</td></tr>').appendTo(table);
