...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  if (options.insulindistribution) {
    $('#distrDiv').remove();
    var html = `<div id="distrDiv" style="border:1px solid black; display: inline-block; margin: 10px; padding:5px;"><b>` + translate('TDD average') + ':</b> ' + tddAverage.toFixed(1) + 'U&nbsp; ';
    html += '<br><b>' + translate('Bolus average') + ':</b> ' + bolusAveragePercent + '%&nbsp; ';
    html += '<br><b>' + translate('Basal average') + ':</b> ' + basalAveragePercent + '%&nbsp; ';
    html += '<br><b>(' + translate('Base basal average:') + '</b> ' + baseBasalAveragePercent + '%<b>)</b>&nbsp; ';
    html += '<br><b>' + translate('Carbs average') + ':</b> ' + carbsAverage.toFixed(0) + 'g';
    
    html += '</div>'
    $(html).insertBefore('#daytodaycharts');
    //skippar nedan...
    html += '<br><b>' + translate('Protein average') + ':</b> ' + proteinAverage.toFixed(0) + 'g';
    html += '<br><b>' + translate('Fat average') + ':</b> ' + fatAverage.toFixed(0) + 'g';
  }
  ...
  function drawChart (day, data, options) {
    var tickValues
      ...
      , context
      , xScale2, yScale2
      , yInsulinScale, yCarbsScale, yScaleBasals, yActivityScale
      , xAxis2, yAxis2
    ...
    yInsulinScale.range([chartHeight*2/3, 0]);
    yCarbsScale.range([chartHeight/3, 0]);
    yActivityScale.range([chartHeight/3, 0]);
    ...
    actpolyline += (xScale2(moment(from)) + padding.left) + ',' + (yActivityScale(0) + padding.top) + ' ';
    ...
    var activityStatusAvailable = client.plugins('activity').isDeviceStatusAvailable(datastorage.devicestatus);
    
    console.log("Device COB status available: ", cobStatusAvailable);
    console.log("Device IOB status available: ", iobStatusAvailable);
    console.log("Device ACT status available: ", activityStatusAvailable);
    ...
    // Draw ACT from devicestatuses if available
    if (activityStatusAvailable) {
      lastdate = 0;
      previousdate = 0;
      var actArray = client.plugins('activity').ACTIVITYDeviceStatusesInTimeRange(datastorage.devicestatus, from.valueOf(), to.valueOf());
      _.each(actArray, function drawAct (point) {
        if (previousdate !== 0 && point.mills - previousdate > times.mins(15).msecs) {
          actpolyline += ', ';
          actpolyline += (xScale2(previousdate) + padding.left) + ',' + (yActivityScale(0) + padding.top) + ' ';
          actpolyline += ', ';
          actpolyline += (xScale2(point.mills) + padding.left) + ',' + (yActivityScale(0) + padding.top) + ' ';
        }
        actpolyline += ', ';
        actpolyline += (xScale2(point.mills) + padding.left) + ',' + (yActivityScale(point.activity) + padding.top) + ' ';
        if (point.mills > lastdate) lastdate = point.mills;
        previousdate = point.mills;
      });
      actpolyline += ', ' + (xScale2(lastdate) + padding.left) + ',' + (yActivityScale(0) + padding.top);

      context.append('polyline')
      .attr('stroke', 'red')
      .attr('opacity', '1')
      .attr('fill-opacity', '0')
      .attr('points', actpolyline);
      
    } else {
      actpolyline += ', ' + (xScale2(to) + padding.left) + ',' + (yActivityScale(0) + padding.top);
    }
    ...
    data.treatments.forEach(function(treatment) {
      ...
      if (treatment.eventType === 'Sensor Change'){
        console.log("kalle: ");
      }
      ...
      if (treatment.carbs && options.carbs) {
        ...
        context.append('rect')
          ...
          .attr('height', chartHeight/3 - yCarbsScale(treatment.carbs))
          .attr('width', 2)
          .attr('stroke', 'red')
          .attr('opacity', '0.5')
          .attr('fill', 'red')
        ...
        context.append('text')
          ...
          .attr('fill', 'red')
          .attr('transform', 'rotate(-45,' + (xScale2(treatment.mills) + padding.left+5) + ',' + (padding.top + yCarbsScale(treatment.carbs) - 5) + ') ' +
            'translate(' + (xScale2(treatment.mills) + padding.left+5) + ',' + (padding.top + yCarbsScale(treatment.carbs) - 5) + ')')
          .text('' + label);
      }
      ...
      if (treatment.insulin && options.insulin) {
        ...
        context.append('rect')
          ...
          .attr('height', chartHeight/3 - yInsulinScale(treatment.insulin))
          .attr('width', 2)
          .attr('stroke', 'blue')
          .attr('opacity', '0.5')
          .attr('fill', 'blue')
        ...
        context.append('text')
          ...
          .style('font-weight', 'bold')
          .attr('fill', 'blue')
          .attr('transform', 'rotate(45,' + (xScale2(treatment.mills) + padding.left-3) + ',' + (chartHeight/3 + padding.top + 10) + ') ' +
            'translate(' + (xScale2(treatment.mills) + padding.left-3) + ',' + (chartHeight/3 + padding.top + 10) + ')')
          .text(dataLabel);
      }
      ...
      if (treatment.insulin || treatment.carbs || treatment.eventType == 'Temp Basal' || treatment.eventType == 'Combo Bolus') {
        ...
        context.append('circle')
          ...
          .attr('stroke', 'black')
          .attr('r', 4);
        if(1==1 && treatment.eventType != "Announcement"){
          context.append('text')
            .style('font-size', '8px')
            .style('font-weight', 'bold')
            .attr('fill', 'purple')
            ...
            .attr('transform', 'rotate(-45,' + (xScale2(treatment.mills) + padding.left + 10) + ',' + (yScale2(scaledTreatmentBG(treatment, data.sgv)) + padding.top - 10) + ')' )
            .text(translate(client.careportal.resolveEventName(treatment.eventType)));
        }else if(treatment.notes != undefined){
          context.append('text')
            .style('font-size', '8px')
            .style('font-weight', 'bold')
            .attr('fill', 'purple')
    ...
    function appendProfileSwitch (context, treatment) {
      ...
      // }
      if (treatment.cutting) {
        if(/\(\d{2,3}%\)/.test(treatment.cutting)){
          //ret = '100% ▲▲▲';
          text = '▲▲ ' + treatment.cutting.match(/\(\d{2,3}%\)/)[0] + ' ▲▲';
        }else{
          text = sign + ' ' + treatment.cutting + ' ' + '►►►' + ' ' + treatment.profile + ' ' + sign;
        }
      } else {
        if(/\(\d{2,3}%\)/.test(treatment.profile)){
          text = '▼▼ ' + treatment.profile.match(/\(\d{2,3}%\)/)[0] + ' ▼▼';
      ...
      context.append('text')
        ...
        .attr('dy', '.35em')
        .attr('transform', 'rotate(-90 ' + (xScale2(treatment.mills) + xoffset + padding.left) + ',' + (yScaleBasals(0) + padding.top - 10) + ') ' +
          'translate(' + (xScale2(treatment.mills) + xoffset + padding.left) + ',' + (yScaleBasals(0) + padding.top - 10) + ')')
        .text(text);
    }
