...
// 2026 visual refresh: typography, card framing and print-safe colored fills.
// Style-only; does not change chart structure, data or dimensions.
daytoday.css =
  '#daytoday-placeholder {' +
  '  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;' +
  '}' +
  '#daytodaycharts text, #daytodaycharts svg text {' +
  '  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;' +
  '}' +
  '#daytodaycharts .axis text {' +
  '  fill: #607d8b;' +
  '  stroke: none;' +
  '}' +
  '#daytodaycharts .axis path.domain, #daytodaycharts .axis line {' +
  '  stroke: #cfd8dc;' +
  '  stroke-width: 1;' +
  '  shape-rendering: crispEdges;' +
  '}' +
  '#daytodaycharts .high-line {' +
  '  stroke: #eceff1;' +
  '}' +
  // Frame each day chart block + its stats in a subtle card. The chart rows are
  // rendered as <table><tr><td>chart</td><td>stats</td></tr></table> blocks.
  '#daytodaycharts > table {' +
  '  background: #ffffff;' +
  ...
  '}' +
  // Print-safe colored fills so the refreshed palette survives PDF/print export.
  '#daytodaycharts svg rect, #daytodaycharts svg path,' +
  '#daytodaycharts svg circle, #daytodaycharts svg polyline,' +
  '#daytodaycharts svg line {' +
...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  // Canonical 2026 palette (style-only refresh).
  var palette = {
    targetBand: 'rgba(76,175,80,0.12)'
    , targetBandStroke: '#cfd8dc'
    , basal: '#1f6fc4'
    , tempBasal: 'rgba(31,111,196,0.5)'
    , iob: '#2f80ed'
    , cob: '#f2994a'
    , prediction: '#8E5FD0'
    , gridline: '#eceff1'
  ...
  function drawChart (day, data, options) {
    ...
    context.append('rect')
      ...
      .attr('stroke', palette.targetBandStroke);
          ...
          if (predictedIndex != null) {
            ...
            for (var entryIndex in entry.values) {
              if (!d.isAfter(end)) {
                ...
                value.date = d.toDate();
                value.color = palette.prediction;
                p.push(value);
                d.add(5, 'minutes');
              }
            }
    ...
    if (options.insulindistribution) {
      ...
      var labelArc = d3.arc()
        .outerRadius(radius * 0.775)
        .innerRadius(radius * 0.775);
      ...
      var carbscolor = d3.scaleOrdinal().range([palette.cob]);
      ...
      var carbsOuter = radius * data.dailyCarbs / options.maxDailyCarbsValue;
      var carbsarc = d3.arc()
        .innerRadius(carbsOuter * 0.55)
        .outerRadius(carbsOuter);
