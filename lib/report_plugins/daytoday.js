  ...
  d3.select('body').append('svg')
    ...
    for (var dt=moment(from); dt < to; dt.add(5, 'minutes')) {
      ...
      if (options.basal)  {
        ...
        var tempPart = basalvalue.tempbasal - basalvalue.basal;
        if (tempPart > 0) {
          positiveTemps += tempPart * 5 / 60;
        } if (tempPart < 0) {
          negativeTemps += tempPart * 5 / 60;
        }
...
//            .text((t.percent ? (t.percent > 0 ? '+' : '') + t.percent + '%' : '') + (t.absolute ? Number(t.absolute).toFixed(2) + 'U' : ''));
    ...
    data.treatments.forEach(function (treatment) {
      ...
      if (treatment.boluscalc && treatment.boluscalc.foods && treatment.boluscalc.foods.length > 0 || renderNotes) {
        ...
        if (renderNotes && !treatment.duration) {
          context.append('text')
            .style('font-size', '10px')
            .style('font-weight', 'normal')
            .attr('fill', 'black')
    ...
    if (options.insulin && options.basal) {
      var boluscolor = 'blue';
      ...
      var chartData = [
        { label: translate('Basal'), count: totalBasalInsulin, pct: (totalBasalInsulin / totalDailyInsulin * 100).toFixed(0) }, 
