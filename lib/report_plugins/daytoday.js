...
daytoday.html = function html (client) {
  ...
  var ret =
    ...
    '  <option x="1200" y="400" selected>1200x400px</option>' +
  ...
  function drawChart (day, data, options) {
    ...
    data.treatments.forEach(function(treatment) {
      ...
      if (treatment.insulin && options.insulin) {
        ...
        var displayval = treatment.insulin * 100;
        if (Math.round(displayval) == displayval) {
          dataLabel = client.utils.toRoundedStr(treatment.insulin, 2)+ 'U';
        }
        context.append('rect')
    ...
    if (options.insulindistribution) {
      ...
      var table = $('<table>');
      $('<tr><td>' + translate('Bolus insulin:') + '</td><td align="right">' + bolusInsulin.toFixed(3) + 'U</td></tr>').appendTo(table);
      if (positiveTemps > 0 || negativeTemps > 0) {
        $('<tr><td><b>' + translate('Base basal insulin:') + '</b></td><td align="right">' + baseBasalInsulin.toFixed(3) + 'U</td></tr>').appendTo(table);
      }
      if (positiveTemps > 0) {
        $('<tr><td>' + translate('Positive temp basal insulin:') + '</td><td align="right">' + positiveTemps.toFixed(3) + 'U</td></tr>').appendTo(table);
      }
      if (negativeTemps < 0) {
        $('<tr><td>' + translate('Negative temp basal insulin:') + '</td><td align="right">' + negativeTemps.toFixed(3) + 'U</td></tr>').appendTo(table);
      }
      var totalBasalInsulin = baseBasalInsulin + positiveTemps + negativeTemps;
      $('<tr><td>' + translate('Total basal insulin:') + '</td><td align="right">' + totalBasalInsulin.toFixed(3) + 'U</td></tr>').appendTo(table);
      ...
      var chartData = [
        {
          label: translate('Base basal')
          , count: baseBasalInsulin
          , pct: (baseBasalInsulin / totalDailyInsulin * 100).toFixed(0)
        }
        , { label: translate('non-base'), count: (totalDailyInsulin-baseBasalInsulin), pct: ((totalDailyInsulin-baseBasalInsulin) / totalDailyInsulin * 100).toFixed(0) }
      ];
