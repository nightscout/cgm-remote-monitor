...
daytoday.report = function report_daytoday(datastorage,sorteddaystoshow,options) {
  ...
  function drawChart(day,data,options) {
   ...
   // create svg and g to contain the chart contents
    ...
    // sandbox with data for iob and cob
    var sbxData = _.defaults(_.pick(data, ['treatments', 'pumpStatuses']), client.sbx.data);
    var iobCobSbx = client.sbx.clientInit(client.sbx.settings, client.sbx.time, client.sbx.pluginBase, sbxData);
    ...
    profile.updateTreatments([], datastorage.tempbasalTreatments);
    
    for (var dt=moment(from); dt < to; dt.add(5, 'minutes')) {
      if (options.iob) {
        var iob = Nightscout.plugins('iob').calc(iobCobSbx,dt.toDate()).iob || 0;
        if (!dt.isSame(from)) {
          iobpolyline += ', ';
        }
      ...
      if (options.cob) {
        var cob = Nightscout.plugins('cob').cobTotal(iobCobSbx,dt.toDate()).cob;
        if (!dt.isSame(from)) {
          cobpolyline += ', ';
        }
