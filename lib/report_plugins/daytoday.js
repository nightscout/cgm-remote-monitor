...
daytoday.report = function report_daytoday(datastorage,daystoshow,options) {
  ...
  function drawChart(day,data,options) {
    ...
    // Tick Values
    if (options.scale === report_plugins.getProperty('SCALE_LOG')) {
    ...
    charts = d3.select('#'+report_plugins.getProperty('containerprefix')+day).html(
      '<b>'+
