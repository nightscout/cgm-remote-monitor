...
agp.html = function html (client) {
  ...
  var ret =
    ...
    '      <button type="button" id="agp-copy" class="agp-btn">' + translate('Copy summary') + '</button>' +
    '      <button type="button" id="agp-download" class="agp-btn agp-btn-secondary">' + translate('Download') + '</button>' +
    '      <span id="agp-export-status" class="agp-export-status" role="status" aria-live="polite"></span>' +
    '    </div>' +
    '  </div>' +
    '  <div class="agp-top">' +
...
agp.css =
  ...
  '.agp-toolbar{margin:10px 0 2px 0;display:flex;gap:8px;align-items:center;flex-wrap:wrap;}' +
  '.agp-btn{font:inherit;font-size:13px;font-weight:600;padding:6px 14px;border:1px solid #1f6fc4;background:#1f6fc4;color:#fff;border-radius:8px;cursor:pointer;}' +
  '.agp-btn:hover{background:#1a5da6;border-color:#1a5da6;}' +
  '.agp-btn-secondary{background:#fff;color:#1f6fc4;}' +
  '.agp-btn-secondary:hover{background:#eef4fb;}' +
  '.agp-export-status{font-size:12px;color:#2e7d32;font-weight:600;}' +
  '@media print{.agp-toolbar{display:none;}}'
  ;
...
agp.report = function report_agp (datastorage, sorteddaystoshow, options) {
  ...
  // ---- Copy / export: a clinician-friendly plain-text summary ----
  function buildSummaryText () {
    var unit = isMmol ? 'mmol/L' : 'mg/dL';
    var lines = [];
    lines.push(translate('Statistics Summary'));
    lines.push(countDays + ' ' + translate('days') + ', ' + firstDay + ' - ' + lastDay);
    lines.push(translate('% Time CGM Active') + ': ' + pctActive.toFixed(1) + '%');
    lines.push(translate('Number of readings') + ': ' + data.length);
    lines.push('');
    lines.push(translate('Average Glucose') + ': ' + fmtDisplay(meanMgdl) + ' ' + unit);
    lines.push(translate('Glucose Management Indicator (GMI)') + ': ' + gmiPct.toFixed(1) + '% (' + gmiMmolMol.toFixed(1) + ' mmol/mol)');
    lines.push(translate('Glucose Variability (CV)') + ': ' + cv.toFixed(1) + '% (' + translate('Target') + ' <= 36%)');
    lines.push(translate('Standard Deviation') + ': ' + fmtDisplay(sdMgdl) + ' ' + unit);
    lines.push('');
    lines.push(translate('Time in Ranges') + ':');
    bands.forEach(function (band) {
      lines.push('  ' + band.name + ' (' + band.range + '): ' + band.pct.toFixed(1) + '%  (' +
        band.hh + translate('h') + ' ' + band.mm + translate('min') + '/' + translate('day') + ')');
    });
    lines.push('');
    lines.push('Nightscout');
    return lines.join('\n');
  }

  function flashStatus (msg) {
    $('#agp-export-status').text(msg);
    window.setTimeout(function () { $('#agp-export-status').text(''); }, 2500);
  }

  function fallbackCopy (text) {
    var ta = $('<textarea>').css({ position: 'fixed', top: '-1000px', opacity: 0 }).val(text);
    $('body').append(ta);
    ta[0].select();
    try {
      document.execCommand('copy');
      flashStatus(translate('Copied to clipboard'));
    } catch (e) {
      flashStatus(translate('Copy failed'));
    }
    ta.remove();
  }
  ...
  function downloadSummary () {
    ...
    a.href = url;
    a.download = 'nightscout-summary-' + (sorteddaystoshow[0] || 'report') + '.txt';
  ...
  $('#agp-copy').off('click').on('click', copySummary);
  $('#agp-download').off('click').on('click', downloadSummary);
