...
profiles.report = function report_profiles (datastorage) {
  ...
  for (var r = 0; r < profileRecords.length; r++) {
    databaseRecords.append('<option value="' + r + '">' + translate('Valid from:') + ' ' + new Date(profileRecords[parseInt(r)].startDate).toLocaleString() + '</option>');
  }
  ...
  function recordChange (event) {
    ...
    var currentrecord = profileRecords[parseInt(currentindex)];
    ...
    Object.keys(currentrecord.store).forEach(key => {
      tr.append(displayRecord(currentrecord.store[String(key)], key));
    });
  ...
  function displayRanges (array, array2) {
    ...
    for (var i = 0; i < array.length; i++) {
      text += array[parseInt(i)].time + '&nbsp:&nbsp' + array[parseInt(i)].value + (array2 ? ' - ' + array2[parseInt(i)].value : '') + '<br>';
    }
