...
profiles.report = function report_profiles (datastorage) {
  ...
  function recordChange (event) {
    ...
    Object.keys(currentrecord.store).forEach(key => {
      tr.append(displayRecord(currentrecord.store[key], key, currentrecord));
    });
  ...
  function displayRecord (record, name, currentrecord) {
    var td = $('<td>');
    var table = $('<table>');
    ...
    if (currentrecord.loopSettings !== undefined)
    {
      table.append($('<tr>').append($('<td>').append('<b>' + translate('Maximum bolus') + '</b>:&nbsp' + currentrecord.loopSettings.maximumBolus)));
      table.append($('<tr>').append($('<td>').append('<b>' + translate('Maximum basal rate [unit/hour]') + '</b>:&nbsp' + currentrecord.loopSettings.maximumBasalRatePerHour)));
      table.append($('<tr>').append($('<td>').append('<b>' + translate('Minimum BG Guard [mg/dL,mmol/L]') + '</b>:&nbsp' + currentrecord.loopSettings.minimumBGGuard)));
    }
