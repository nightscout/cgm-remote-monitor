...
profiles.report = function report_profiles(datastorage) {
  ...
  function displayRecord(record, name, startdate, lastRecord) {
  ...
  function displayTargetRanges(a1, a1_2, a2, a2_2) {
    ...
    if (a2 == null) { //first profile
      for (let i = 0; i < maxI; i++) {
        text += a1[i].time + '&nbsp:&nbsp' + a1[i].value + (a1_2 ? ' - ' + a1_2[i].value : '') + '<br>';
      }
    } else {
      for (let h = 0; h < 24; h++) {
        ...
        if (!T1 && !T2) {
          ...
          text += '<div style="border-radius:5px; background-color:lightgreen">' + T1.time + '&nbsp:&nbsp' + T1_1.value + (T1_2 ? ' - ' + T1_2.value : '') + '</div><br>';
        } else {
          text += T1.time + '&nbsp:&nbsp' + T1_2.value + (T1_2 ? ' - ' + T1_2.value : '') + '<br>';
        }
    ...
    for (let h = 0; h < 24; h++) {
      if (a2 == null) {
        if (i < a1.length && (a1[i].time.substring(0, 2)) == h) {
          ...
          text += String(h).padStart(2, '0') + ':00&nbsp|<br>';
        }
      } else {
        ...
        if (!b1 && !b2) {
          ...
          text += '<div style="border-radius:5px; background-color:lightgreen">' + b1.time + '&nbsp:&nbsp' + b1.value + '</div>';
        } else if (b1.value != b2.value) {
          text += '<div style="border-radius:5px; background-color:yellow">' + b1.time + '&nbsp:&nbsp' + b1.value + '</div>';
        } else if (b1.value == b2.value) {
          text += b1.time + '&nbsp:&nbsp' + b1.value + '<br>';
        } else {
          text += String(h).padStart(2, '0') + ':00&nbsp:&nbsp|<br>';
        }
      }
