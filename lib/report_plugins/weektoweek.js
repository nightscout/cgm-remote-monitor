...
weektoweek.report = function report_weektoweek(datastorage,sorteddaystoshow,options) {
  ...
  var weekIdx = 0;
  var dayIdx = 0;
  var firstDayOfWeek = moment.localData().firstDayOfWeek();

  sorteddaystoshow.forEach( function eachDay(day) {
    if (moment(day).day() === firstDayOfWeek) {
      weekIdx += 1;
      dayIdx = 0;
  ...
  daytoday.prepareHtml(weekstoshow);

  weekstoshow.forEach( function eachWeek(week) {
    day.forEach( function eachDay(week) {
      drawChart(week, day, datastorage[day], options);
    });
  });
  ...
  function drawChart(week,day,data,options) {
    var tickValues
      , charts
      , context
      , xScale2, yScale2
