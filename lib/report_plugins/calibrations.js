...
calibrations.report = function report_calibrations(datastorage,daystoshow) {
  ...
  function calibrations_drawelements() {
    ...
    for (var i=0; i<events.length; i++) {
      e = events[i];
      if (e.checked) {
        calibrations_drawmbg(e);
        e.cals.forEach(calibrations_drawcal);
      }
    }
  }
  ...
  function calibrations_drawcal(cal) {
  ...
  function calibrations_calcmbg(mbg) {
    var lastsgv = calibrations_findlatest(new Date(mbg.mills),sgvs);
  ...
  function calibrations_drawmbg(mbg) {
