...
calibrations.report = function report_calibrations (datastorage, sorteddaystoshow) {
  ...
  sorteddaystoshow.forEach(function(day) {
    treatments = treatments.concat(datastorage[String(day)].treatments.filter(function(t) {
  ...
  var cals = [];
  sorteddaystoshow.forEach(function(day) {
    cals = cals.concat(datastorage[String(day)].cal);
  });

  var sgvs = [];
  sorteddaystoshow.forEach(function(day) {
    sgvs = sgvs.concat(datastorage[String(day)].sgv);
  });

  var mbgs = [];
  sorteddaystoshow.forEach(function(day) {
    mbgs = mbgs.concat(datastorage[String(day)].mbg);
  });
  ...
  function checkLastCheckboxes (maxcals) {
    for (i = events.length - 1; i > 0; i--) {
      if (typeof events[parseInt(i)].device !== 'undefined') {
        events[parseInt(i)].checked = true;
  ...
  function checkboxevent (event) {
    ...
    events[parseInt(index)].checked = $(this).is(":checked");
    drawelements();
  ...
  function findlatest (date, storage) {
    ...
    for (var i = 0; i < storage.length; i++) {
      if (storage[parseInt(i)].mills > time) {
        return last;
      }
      last = storage[parseInt(i)];
    }
    return last;
  }
};
