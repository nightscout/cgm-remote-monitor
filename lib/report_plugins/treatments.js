...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  sorteddaystoshow.forEach(function (day) {
    ...
    for (var t=0; t<treatments.length; t++) {
      ...
      carbs += tr.foodType ? ' ' + client.utils.escapeHtml(tr.foodType) : ''
      ...
      table.append($('<tr>').addClass('border_bottom')
        ...
        .append($('<td>').append(tr.eventType ? translate(client.careportal.resolveEventName(tr.eventType)) + (tr.reason ? '<br>' + client.utils.escapeHtml(tr.reason) : '')  +
          (tr.insulinNeedsScaleFactor ? '<br>' + tr.insulinNeedsScaleFactor * 100 + '%' : '') + (tr.correctionRange ? ' ' + correctionRangeText : '') : ''))
        .append($('<td>').attr('align','center').append(tr.glucose ? tr.glucose + ' ('+client.utils.escapeHtml(translate(tr.glucoseType))+')' : ''))
        ...
        .append($('<td>').attr('align','center').text(tr.profile ? tr.profile : ''))
        .append($('<td>').text(tr.enteredBy ? tr.enteredBy : ''))
        .append($('<td>').text(tr.notes ? tr.notes : ''))
      );
    }
  });
