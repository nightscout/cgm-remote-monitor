...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  function treatmentRow (tr, day) {
    ...
    return $('<tr>').addClass('border_bottom')
      ...
      .append($('<td>').append(profile.applyTimezone(moment(tr.created_at)).format(client.settings.timeFormat === 24 ? 'HH:mm' : 'h:mm A')))
  ...
  function eventTypeLabel (key) {
    return key === NO_EVENT_TYPE ? translate('(none)') : translate(client.careportal.resolveEventName(key));
  }
