...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  sorteddaystoshow.forEach(function (day) {
    ...
    for (var t=0; t<treatments.length; t++) {
      var tr = treatments[t];
      var carbs = tr.carbs ? tr.carbs : ''//KS
      carbs += tr.foodType ? ' ' + tr.foodType : ''//KS
      carbs += tr.absorptionTime ? ' ' + (Math.round(tr.absorptionTime / 60.0 * 10) / 10) + 'h' : ''//KS
      table.append($('<tr>').addClass('border_bottom')
        ...
        .append($('<td>').attr('align','center').append(carbs))//KS
