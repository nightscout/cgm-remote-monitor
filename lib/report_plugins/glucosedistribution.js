...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '  </h2>' +
    '<table><tr>' +
    '<td rowspan="2" style="valign:top;"><div id="glucosedistribution-overviewchart"></div></td>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['Low', 'Lowish', 'Normal', 'Elevated', 'High'].forEach(function(range) {
    ...
    if (range == 'Lowish') {
      rangeExp = ' (>=' + 3.5 + ', <' + options.targetLow + ')';
    }
    if (range == 'Elevated') {
      rangeExp = ' (>=' + options.targetHigh + ', <' + 10 + ')';
    }
    if (range == 'High') {
      rangeExp = ' (>=' + 10 + ')';
    }
  ...
  for (i = 0; i <= glucose_data.length - 2; i++) {
    ...
    if (entry.bgValue > 10) { // options.targetHigh) {
      ...
      RMSTotal += Math.pow(entry.bgValue - 10, 2);
    }
  }
  ...
  var tirMultiplier = (result.Normal.readingspct + result.Elevated.readingspct) / 100.0;
