...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  for (var i = 0; i < 24; i++) {
    ...
    enabledHours[parseInt(i)] = $('#glucosedistribution-' + i).is(':checked');
  }
  ...
  for (i = 1; i <= glucose_data.length - 2; i++) {
    let entry = glucose_data[parseInt(i)];
  ...
  ['Low', 'Normal', 'High'].forEach(function(range) {
    ...
    $('<td class="tdborder" style="background-color:' + tablecolors[String(range)] + '"><strong>' + translate(rangeLabel) + rangeExp + ': </strong></td>').appendTo(tr);
  ...
  for (i = 0; i <= glucose_data.length - 2; i++) {
    const entry = glucose_data[parseInt(i)];
