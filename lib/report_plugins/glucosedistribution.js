...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    translate('GVI (Glycemic Variability Index) are measures developed by Dexcom, detailed ') +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (range == 'Up') {
      ...
      $('<td class="tdborder" style="background-color:' + tablecolors['Lowish'] + '" rowspan="3">' + translate('Gravid') + '<br/><strong>' + preg2 + '%</strong></td>').appendTo(tr);
    }
