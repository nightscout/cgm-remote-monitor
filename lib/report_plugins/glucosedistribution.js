  ...
  report.append(table);
   ...
   $('<tr><th>' + translate('Mean Absolute Change') + '</th><th>' + translate('Time in fluctuation') + '<br>('+t1exp+')</th><th>'+ translate('Time in rapid fluctuation') + '<br>('+t2exp+')</th></tr>').appendTo(stabilitytable);
	...
	var glucose_data = [data[0]];
     ...
     if (timeDelta < 9 * 60 * 1000 || timeDelta > 20 * 60 * 1000) {
     	glucose_data.push(entry);
     	continue;
     }
     
	var missingRecords = Math.floor(timeDelta / 5);
	
	var timePatch = timeDelta / missingRecords;
	var bgDelta = (nextEntry.bgValue - entry.bgValue) / (missingRecords+1);
	
	glucose_date.push(entry);
	
	console.log('patching between',entry,'and',nextEntry);
	
	for (var j = 0; j < missingRecords; j++) {
		var bg = entry.bgValue + bgDelta;
		var t = new Date(entry.displayTime.getTime() + j*timePatch);
		var newEntry = {bgValue: bg, displayTime: t }:
		console.log('patch:',newEntry);
		glucose_date.push(newEntry);
	}
	
}
     
		// data cleaning pass 1 - replace single jumpy measures with interpolated values

	/*
	for (var i = 1; i < data.length-2; i++) {
   
     var prevEntry = data[i-1];
     var entry = data[i];
     var nextEntry = data[i+1];
     ...
     if (timeDelta > 6 * 60 * 1000 || timeDelta2 > 6 * 60 * 1000) {
     	glucose_data.push(entry);
     	console.log("Cleaning pass 1 Record gap");
     	continue;
     }
