...
glucosedistribution.report = function report_glucosedistribution(datastorage,daystoshow,options) {
  ...
  ['Low', 'Normal', 'High'].forEach(function(range) {
    ...
    if (r.rangeRecords.length > 0) {
      r.mean = Math.floor(ss.mean(r.localBgs)).toFixed(1);
      r.median = r.rangeRecords[r.midpoint].sgv.toFixed(1);
      r.stddev = Math.floor(ss.standard_deviation(r.localBgs)).toFixed(1);
    }
  });
  ...
  ['Low', 'Normal', 'High'].forEach(function(range) {
    ...
    if (r.rangeRecords.length > 0) {
      $('<td class="tdborder">' + r.mean + '</td>').appendTo(tr);
      $('<td class="tdborder">' + r.median + '</td>').appendTo(tr);
      $('<td class="tdborder">' + r.stddev + '</td>').appendTo(tr);
      $('<td> </td>').appendTo(tr);
    } else {
  ...
  if (data.length > 0) {
    ...
    $('<td class="tdborder">' + Math.round(ss.mean(localBgs)).toFixed(1) + '</td>').appendTo(tr);
    $('<td class="tdborder">' + Math.round(ss.quantile(localBgs, 0.5)).toFixed(1) + '</td>').appendTo(tr);
    $('<td class="tdborder">' + Math.round(ss.standard_deviation(localBgs)).toFixed(1) + '</td>').appendTo(tr);
    $('<td class="tdborder"><center>' + Math.round((ss.mean(mgDlBgs)+46.7)/28.7).toFixed(1) + '%<sub>DCCT</sub> | ' + Math.round(((ss.mean(mgDlBgs)+46.7)/28.7 - 2.15)*10.929) + '<sub>IFCC</sub></center></td>').appendTo(tr);
  } else {
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
