...
//     var prevEntry = glucose_data[i-1];
   ...
   glucose_data = data = glucose_data2.filter(function(r) {
    ...
    if (glucose_data.length > 0) {
        ...
        $('<td class="tdborder">' + (localBgs.length > 0? (Math.round(ss.standardDeviation(localBgs) * 10) / 10).toFixed(1): 'N/A') + '</td>').appendTo(tr);
...
//	console.log('TADC',TDC,'days',days);
    ...
    var t1exp = '>5 mg/dL/5min';
    var t2exp = '>10 mg/dL/5min';

    if (client.settings.units == 'mmol') {
        t1exp = '>0.27 mmol/L/5min';
        t2exp = '>0.55 mmol/L/5min';
    }
