...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  $('<th>' + translate('CV') + '</th>').appendTo(thead);
  ...
  ['Low', 'Lowish', 'Normal', 'Elevated', 'High'].forEach(function(range) {
    ...
    if (r.rangeRecords.length > 0) {
      ...
      $('<td class="tdborder">' + Math.round((r.stddev / r.mean) * 100).toFixed(0) + '%</td>').appendTo(tr);
      $('<td> </td>').appendTo(tr);
    } else {
  ...
  if (glucose_data.length > 0) {
    ...
    $('<td class="tdborder">' + (Math.round(100*(ss.standard_deviation(localBgs)/ss.mean(localBgs)))).toFixed(0) + '</td>').appendTo(tr);
