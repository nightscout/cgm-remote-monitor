...
glucosedistribution.report = function report_glucosedistribution(datastorage, sorteddaystoshow, options) {
   ...
   //     console.log('time delta is', timeDelta/60/1000, 'minutes');
...
//        console.log('patching', missingRecords, 'records between', entry, 'and', nextEntry);
//        console.log('timePatch',timePatch/60/1000,'bgDelta',bgDelta);
...
//     	console.log("Cleaning pass 2 Record gap");
     	continue;
     }
	 ...
	 if (delta1 <= 8 && delta2 <= 8) {
	    glucose_data2.push(entry);
 //    	console.log("Cleaning pass 1 Allowing jitter");
     	continue;
	 }
	 
	 
	 if ((delta1 > 0 && delta2 <0) || (delta1 < 0 && delta2 > 0)) {
//	 	 console.log('Interpolating noise',prevEntry,entry,nextEntry);
...
//            console.log("Record skipped");
	...
	var TADC = Math.round(deltaTotal / days / 18.0 * 100) / 100;
	console.log('TADC',TADC,'days',days);
    ...
    $('<tr><th>' + translate('Total Absolute Daily Change') + '</th><th>' + translate('GVI') + '</th><th>' + translate('PGS') + '</th></tr>').appendTo(stabilitytable);
    $('<tr><td class="tdborder">' + TADC + '</td><td class="tdborder">' + GVI + '</td><td class="tdborder">' + PGS + '</td></tr>').appendTo(stabilitytable);
