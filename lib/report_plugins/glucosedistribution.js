...
glucosedistribution.report = function report_glucosedistribution(datastorage,daystoshow,options) {
  ...
  var Statician = require('simple-statistics');
  ...
  ['Low', 'Normal', 'High'].forEach(function(range) {
    ...
    if (r.rangeRecords.length > 0) {
      r.mean = Math.floor(10*Statician.mean(r.localBgs))/10;
      r.median = r.rangeRecords[r.midpoint].sgv;
      r.stddev = Math.floor(Statician.standard_deviation(r.localBgs)*10)/10;
    }
  });
