...
function enabledFeatures (env) {
  var settings = env && env.settings || {};
  var features = (settings.enable || []).slice();
  var source = env && env.extendedSettings && env.extendedSettings.connect && env.extendedSettings.connect.source;
  var connectFeature = allowlists.connectFeature(source);
  if (connectFeature) {
    features.push('connect');
    features.push(connectFeature);
  }
  return allowlists.filterFeatures(features);
}
