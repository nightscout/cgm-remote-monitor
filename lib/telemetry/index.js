...
function create (env, ctx, options) {
  ...
  var store = options.store || createStore({
    storeDir: telemetryConfig.storeDir,
    mongoStore: ctx && ctx.store,
    collection: telemetryConfig.collection
  });
  ...
  function preview (options) {
    ...
    return {
      ...
      payload: payload.build(env, ctx, {
        now: options.now,
        installationSecret: options.installationSecret || secretResult.secret,
  ...
  function start () {
    if (started || !telemetryConfig.scheduledSend || !ctx || !ctx.bus || typeof ctx.bus.on !== 'function') {
      return false;
    }
    onTick = function telemetryTick (tick) {
      if (inFlight) {
        return;
      }
      inFlight = true;
      runDue({
        now: tick && tick.now ? new Date(tick.now) : new Date(),
        random: options.random
      }, function done () {
        inFlight = false;
      });
    };
    ctx.bus.on('tick', onTick);
    started = true;
    return true;
  }

  function stop () {
    if (!started || !ctx || !ctx.bus || typeof ctx.bus.removeListener !== 'function' || !onTick) {
      return false;
    }
    ctx.bus.removeListener('tick', onTick);
    started = false;
    onTick = null;
    return true;
  }
  ...
  function runDue (options, callback) {
    if (typeof options === 'function') {
      callback = options;
      options = {};
    }
    ...
    if (!telemetryConfig.scheduledSend) {
      process.nextTick(function notScheduled () {
        callback(null, { sent: false, reason: 'scheduled-disabled' });
      });
      return;
    }
    ...
    writeScheduleState(store, scheduleState);
    if (!schedule.isDue(scheduleState, now, telemetryConfig.enabled)) {
      process.nextTick(function notDue () {
        callback(null, { sent: false, reason: 'not-due', next_due_at: scheduleState.next_due_at });
      });
      return;
    }

    sendOnce(options, callback);
  }
