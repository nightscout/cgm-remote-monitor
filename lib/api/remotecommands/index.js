...
function configure(app, wares, ctx) {
  ...
  function config_authed(app, api, wares, ctx) {
    ...
    api.put('/remotecommands/:id', ctx.authorization.isPermitted('api:remotecommands:update'), function (req, res) {
      ...
      //if data._id is set too, it must match this id
      if (data._id != undefined && data._id != req.params.id) {
        res.sendJSONStatus(res, consts.HTTP_INTERNAL_ERROR, 'id url parameter must match _id in body');
        console.log('Error: _id in body does not match url parameter');
        return
      }
