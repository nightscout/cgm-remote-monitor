...
function configure (app, wares, ctx, env) {
    ...
    ctx.alexa.configureIntentHandler('AddTreatment', function (callback, slots, sbx) {
        var values = Object.keys(slots)
                        .map(function(k) { if(slots[k].value !== undefined) { return slots[k] } })
                        .filter( function(j) { return j !== undefined });
        var treatment = {};

        if(values.length > 0) {
            values.forEach(function(v) {
                treatment[v.name] = v.value;
            });
        }

        ctx.treatments.create([treatment], function (err, records) {
            callback('Successfully added treatment', 'success');
        });
    });
