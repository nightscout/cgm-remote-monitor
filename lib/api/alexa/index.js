...
function configure (app, wares, ctx, env) {
    ...
    var translatePl = ctx.language.translatePlaceholders;
    ...
    api.post('/alexa', ctx.authorization.isPermitted('api:*:read'), function (req, res, next) {
        //console.log('Incoming request from Alexa', req, res, next);
    ...
    ctx.alexa.addToRollup('Status', function bgRollupHandler(slots, sbx, callback) {
        entries.list({count: 1}, function (err, records) {
            ...
            var map = new Map();
            map.set( 'sgv',  sbx.scaleMgdl(records[0].sgv));
            map.set('direction', direction);
            map.set('time', moment(records[0].date).from(moment(sbx.time)));
            var status = translatePl('alexaStatus', map);
    ...
    ctx.alexa.configureIntentHandler('MetricNow', function ( callback, slots, sbx, locale) {
        entries.list({count: 1}, function(err, records) {
            ...
            var status =  translatePl('alexaStatus',[ sbx.scaleMgdl(records[0].sgv), direction, moment(records[0].date).from(moment(sbx.time))]);
            //var status = sbx.scaleMgdl(records[0].sgv) + direction + ' as of ' + moment(records[0].date).from(moment(sbx.time));
