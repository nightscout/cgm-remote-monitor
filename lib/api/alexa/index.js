...
function configure (app, wares, ctx, env) {
  ...
  function handleConversationLogic(request, handler, next, createAllowed) {
    // check if create access is required
    if (handler.createAccess) {
      var confStatus = _.get(request, 'intent.confirmationStatus');
      if (!createAllowed) {
  ...
  function handleIntent(request, next, createAllowed) {
    var intentName = _.get(request, 'intent.name')
      , slots = _.get(request, 'intent.slots')
      , metric = null;
    ...
    if (handler) {
      // check if create access is required
      var needCreateAccess = handleConversationLogic(request, handler, next, createAllowed);
      if (needCreateAccess === true || needCreateAccess === false) {
        // true or false means it handled the response, null means continue to use the regular handler
        return needCreateAccess;
      }
