...
function configure (app, wares, ctx, env) {
  ...
  function handleIntent(intentName, slots, next) {
    if (slots.metric.resolutions.resolutionsPerAuthority[0].status.code != "ER_SUCCESS_MATCH"){
      next('Unknown Intent', 'I\'m sorry. I don\'t know what you\'re asking for.');
    }

    var metricValues = slots.metric.resolutions.resolutionsPerAuthority[0].values;
    if (metricValues.length == 0){
      next('Unknown Intent', 'I\'m sorry. I don\'t know what you\'re asking for.');
    }

    var handler = ctx.alexa.getIntentHandler(intentName, metricValues[0].value.name);
    if (handler){
