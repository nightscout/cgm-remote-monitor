...
function configure (app, wares, ctx, env) {
	...
	if (intentName == 'carbsvodka') {		
            next(intent.name, 'There are no carbs in vodka, so drink drink drink');
	}
	else
	{
		if (intentName == 'cookiespoison') {		
		    next(intent.name, 'No definitely not, that would be silly');
		}
		else
		{
			if (intentName == 'cookies') {		
				next(intent.name, 'Yes of course you can eat '+intentName);
			}
			else
			{
				...
				if (handler){
				    ...
				    next('Unknown Intent', 'I\'m sorry I don\'t know what you\'re wanting to know');
				}
			}
		}
	}
