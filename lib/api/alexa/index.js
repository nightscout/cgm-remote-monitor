...
function configure (app, wares, ctx, env) {
  ...
  api.post('/alexa', ctx.authorization.isPermitted('api:*:read'), function (req, res, next) {
    ...
    var locale = __get(req, 'body.request.locale');
    if(locale){
      if(locale.length > 2) {
  ...
  function handleIntent(intentName, slots, next) {
    var metric;
    if (slots) {
      var slotStatus = __get(slots, 'metric.resolutions.resolutionsPerAuthority[0].status.code');
      var slotName = __get(slots, 'metric.resolutions.resolutionsPerAuthority[0].values[0].value.name');
