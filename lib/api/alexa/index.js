...
function configure (app, wares, ctx, env) {
  ...
  function handleConversationLogic(intentDetails, res, next) {
    // check where we are in the conversation
    switch (_.get(intentDetails, 'dialogState')) {
      case 'STARTED':
      case 'IN_PROGRESS':
        ...
        res.json({
          ...
          updatedIntent: {
            name: _.get(intentDetails, 'intent.name'),
      ...
      case 'COMPLETED':
        ...
        switch (_.get(intentDetails, 'intent.confirmationStatus')) {
          case 'DENIED':
  ...
  function handleIntent(intentDetails, res, next, createAllowed) {
    ...
    if (handler) {
      ...
      if (handler.createAccess) {
        ...
        var goodToGo = handleConversationLogic(intentDetails, res, next);
        if (!goodToGo) {
          return false;
        }
      }
