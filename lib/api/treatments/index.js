...
function configure (app, wares, ctx) {
  ...
  function config_authed (app, api, wares, ctx) {
    ...
    function post_response(req, res) {
      ...
      if (_.isNil(t.timestamp) || _.isNil(t.eventType)) {
        res.sendJSONStatus(res, consts.HTTP_INTERNAL_ERROR, 'Event type or timestamp missing', err);
        };
      
      var dupeFound = false;
      
      _.foreach(ctx.treatments, function findDupe(t) {
        if (t.timestamp == treatment.timestamp && t.eventType = 
