...
function configure(app, wares, ctx) {
    ...
    function config_authed(app, api, wares, ctx) {
        ...
        function post_response(req, res) {
            ...
            var inMemoryTreatments = _.concat(ctx.ddata.treatments
                , ctx.ddata.tempTargetTreatments
                , ctx.ddata.tempbasalTreatments
                , ctx.ddata.profileTreatments
                , ctx.ddata.insulinchangeTreatments
                , ctx.ddata.sensorTreatments
                , ctx.ddata.sitechangeTreatments);
            
            _.forEach(treatments, function checkForDupes(treatment) {
            
                var dupeFound = false;

                if (_.isNil(treatment.timestamp) ||  _.isNil(treatment.eventType)) {
                    console.log(treatment);
                    res.sendJSONStatus(res, consts.HTTP_INTERNAL_ERROR, 'Event type or timestamp missing from a record');
                }

                _.forEach(inMemoryTreatments, function findDupe(t) {
                    if (t.timestamp == treatment.timestamp && t.eventType == treatment.eventType) {
                        dupeFound = true;
                        return false;
                    }
                });
            ...
            if (insertTreatments.length == 0) {
                return res.sendJSONStatus(res, 409, '409 Conflict - Uploaded Record(s) Already Exist(s)');
            }
