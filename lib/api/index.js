...
function create (env, ctx) {
 ...
 // Start setting up routes
  ...
  // warning, and are logged once per process:
  //
  ...
  function tolerateCount (req, res) {
    ...
    const leading = countParam.leadingCount(requested);

    if (leading !== null) {
      req.query.count = leading;
      deprecate(res, 'leading', 'a count followed by other text is deprecated and may be refused by a future major release; send count as a whole number only');
    }

    if (countParam.isZeroCount(req.query.count)) {
      if (countParam.hasDateWindow(req.query.find)) {
        req.query.count = countParam.NO_LIMIT_COUNT;
      } else {
        delete req.query.count;
      }
      deprecate(res, 'zero', 'count=0 is deprecated and may be refused by a future major release; send a whole number of documents, 1 or greater');
    }
  }
  ...
  app.use(function validateCount (req, res, next) {
    ...
    if (countParam.hasCount(req.query.count) && countParam.parseCount(req.query.count) === null) {
