...
function format_feed (items) {
  var feed = new Feed({
    title: "Nightscout CGM"
  , description: "CGM Trace"
  , link: "http://nightscout/index.html"
  , image: "http://nightscout/images/logo1.png"
  , copyright: "CGM Trace"
  , author: {
    name: 'Nightscout CGM'
    // , email: 'Nightscout CGM'
    , link: 'http://nightscout/about#'
  }
  });
  console.log(feed);
  items.forEach(function iter (item) {
    var date = new Date(item.date);
    var sgv = item.sgv;
    var title = [sgv, item.dateString, item.direction].join(' ');
    feed.addItem({
      title: title
    , link: 'http://nightscout/#entry'
    , description: title
    , date: date
    });
  });
...
function configure (app, wares, entries) {
  var express = require('express'),
    ...
    api.use(wares.extensions([
      'json', 'svg', 'csv', 'txt', 'png', 'html', 'tsv', 'atom', 'rss'
    ]));
    ...
    function format_entries (req, res, next) {
      ...
      return res.format({
        ...
        rss: function ( ) {
          ...
          res.send(format_feed(res.entries).render('rss-2.0'));
        },
        atom: function ( ) {
          res.header('content-type', 'application/atom+xml');
          res.send(format_feed(res.entries).render('atom-1.0'));
        },
        json: function ( ) {
