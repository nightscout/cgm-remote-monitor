...
function configure (app, wares, ctx, env) {
    ...
    function formatWithSeparator (data, separator) {
      ...
      var csv = outputdata.map(function(row) {
        return fields.map(function(fieldName) {
          return JSON.stringify(row[String(fieldName)], replacer)
        }).join(separator)
      });
      return csv.join('\r\n');
    }
  ...
  function echo_query (req, res) {
    ...
    res.json({
      query: ctx[String(storage)].query_for(query)
      , input: input
      , params: req.params
      , storage: storage
    });
  ...
  function prep_patterns (req, res, next) {
    ...
    if (prefix.length === 1) {
      ...
      // an indexed field
      query[String(field)].$regex = prefix.map(iter_regex('^')).pop();
    }

    // Merge into existing query structure.
    if (req.query.find) {
      if (req.query.find[String(field)]) {
        req.query.find[String(field)].$in = query[String(field)].$in;
      } else {
        req.query.find[String(field)] = query[String(field)];
      }
    } else {
