...
// A 24-hex id, in either case, as every other v1 path accepts it.
const isId = require('../../server/object-id-forms').isHexId;
...
function configure (app, wares, ctx, env) {
  ...
  api.get('/entries/:spec', function(req, res, next) {
    if (isId(req.params.spec)) {
      entries.getEntry(req.params.spec, function(err, entry) {
        ...
        // An id that names no entry is a read that matched nothing: answer
        // 200 with [], as every other v1 read does. A storage fault took the
        // next(err) path above and still answers 500.
        res.entries = entry ? [entry] : [];
  ...
  function query_models (req, res, next) {
    ...
    // Queries that require storage must skip cache access entirely.
    if (!inMemoryPossible) {
      inMemoryCollection = [];
    } else if (typeQuery) {
      ...
      inMemoryCollection = ctx.cache.getDataRef('entries');
    }
