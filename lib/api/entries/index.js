...
const sendQueryValidationError = require('../shared/query-error');
...
function configure (app, wares, ctx, env) {
  ...
  // Response records are already materialized. Preserve type/default mutation
  // and legacy date de-normalization without wrapping them in streams.
  function force_typed_data (opts) {
    return function (data) {
      if (!data.type && opts.type) data.type = opts.type;
      if (env.settings.deNormalizeDates && data.dateString && Object.prototype.hasOwnProperty.call(data, 'utcOffset')) {
        data.dateString = moment(data.dateString).utcOffset(data.utcOffset).toISOString(true);
  ...
  function format_entries (req, res) {
    ...
    if (res.entries_err) {
      // A filter the v1 query guards refused is the caller's error, not a
      // storage fault, and must not be reported as one.
      if (sendQueryValidationError(res, res.entries_err)) return;
    ...
    // Transform only when producing a response, after the conditional 304 path.
    function output () { return res.entries.map(force_typed_data(type_params)); }
    return res.format({
      'text/plain': function () { res.send(formatWithSeparator(output(), '\t')); },
      'text/tab-separated-values': function () { res.send(formatWithSeparator(output(), '\t')); },
      'text/csv': function () { res.send(formatWithSeparator(output(), ',')); },
      'application/json': function () { res.json(output()); },
      'default': function () { res.json(output()); }
    });
  }
   ...
   * This inspects the http requests's incoming payload.  This normalizes the
   * materialized payload and passes one ordered batch to the storage layer.
   */
  // middleware to process "uploads" of sgv data
  function insert_entries (req, res, next) {
    ...
    // readArray previously deferred delivery to the next tick. Keep that
    // callback boundary while avoiding two streams and a duplicate array.
    process.nextTick(function () {
      if (req.persist_entries) entries.create(incoming, done);
      else done(null, incoming);
    });
  }
  ...
  function query_models (req, res, next) {
    ...
    let inMemoryPossible = storage === ctx.entries;
    let typeQuery;
  ...
  function count_records (req, res, next) {
    ...
    if (Object.prototype.hasOwnProperty.call(query, 'pipeline')) {
      return res.status(400).json({
        status: 400,
        message: 'The pipeline parameter is not supported by the Nightscout API v1. Use find[...] to filter the records a count is taken over.'
      });
    }
    var storage = req.storage || ctx.entries;
    storage.aggregate(query, function payload (err, entries) {
      ...
      res.entries = entries;
      res.entries_err = err;
      // format_results never runs on the error path here, so the refused-query
      // 400 has to be answered before next(err) hands off to express.
      if (sendQueryValidationError(res, err)) return;
      return next(err);
    });
  }
   ...
   */  function prep_storage (req, res, next) {
    if (req.params.storage && ['entries', 'treatments', 'devicestatus'].includes(req.params.storage)) {
      ...
      if (req.params.storage !== 'entries') {
        return ctx.authorization.isPermitted('api:' + req.params.storage + ':read')(req, res, next);
      }
    } else {
