...
function configure(app, wares, ctx) {
    ...
    api.get('/activity', function(req, res) {
        ...
        ctx.activity.list(req.query, function(err, results) {
            // This callback never checked err at all: on a failed read `results`
            // is null and the forEach below throws, so a refused filter surfaced
            // as an unhandled 500 from the express error handler.
