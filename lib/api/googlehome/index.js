      ...
      entries.list({count: 1}, function(err, records) {
        ...
        if (records && records.length > 0) {
          ...
          if (records[0].direction === 'FortyFiveDown') {
            direction = ' estando bajando lentamente.';
          } else if (records[0].direction === 'FortyFiveUp') {
            direction = ' estando subiendo lentamente.';
          } else if (records[0].direction === 'Flat') {
            direction = ' estando estable.';
          } else if (records[0].direction === 'SingleUp') {
            direction = ' estando subiendo.';
          } else if (records[0].direction === 'SingleDown') {
            direction = ' estando bajando.';
          } else if (records[0].direction === 'DoubleDown') {
            direction = ' estando bajando rápidamente.';
          } else if (records[0].direction === 'DoubleUp') {
            direction = ' estando subiendo rápidamente.';
          }
            moment.locale('es');
          response = buildPreamble(result.parameters);
          response += sbx.scaleMgdl(records[0].sgv) + direction + ' El valor ha sido leído ' + moment(records[0].date).from(moment(sbx.time)) + ', quieres que te lo repita?';
        } else {
