...
const countParam = require('../../server/count');
...
function configure (app, wares, ctx, env) {
  ...
  api.get('/devicestatus/', function(req, res) {
    ...
    /** @type {number} */
    let numCount;
    if (countParam.isZeroCount(queryParams.count)) {
        numCount = 0; // A request for no documents gets none, not the default
    } else if (typeof queryParams.count === 'string') {
