...
function configure (app, wares, ctx) {
  ...
  function config_authed (app, api, wares, ctx) {
    ...
    // delete old records
    api.delete('/devicestatus_delete/:date', ctx.authorization.isPermitted('api:devicestatus:delete'), function(req, res) {
      ctx.devicestatus.removeOld(req.params.date, function (err, removed) {
        if (err) {
          res.sendJSONStatus(res, consts.HTTP_INTERNAL_ERROR, 'Mongo Error', err);
        } else {
          res.json(removed);
