...
function configure (app, wares, ctx, env) {
  ...
  api.get('/devicestatus/', function(req, res) {
    ...
    const canServeFromMemory = inMemoryData.length >= q.count && Object.keys(q).length == 1 ? true : false;    if (canServeFromMemory) {
      const sorted = inMemoryData.sort((a, b) => b.mills - a.mills);

      return res.json(processDates(sorted.slice(0, q.count)));
    }
