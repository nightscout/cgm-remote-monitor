...
function configure (app, wares, ctx, env) {
  ...
  api.get('/devicestatus/', function(req, res) {
    ...
    console.log("devicestatus query", q);

    const inMemoryData = ctx.ddata.devicestatus;
    const canServeFromMemory = inMemoryData.length >= q.count && Object.keys(q).length == 1 ? true : false;

    if (canServeFromMemory) {
      console.log('Serving device statuses from RAM');

      const sorted = _sortBy(inMemoryData, function(item) {
        return -item.mills;
      });
