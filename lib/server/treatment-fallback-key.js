...
//   cannot replace a Loop, Trio or API v3 record. An empty string is no
//   identity, on the write and on the stored record, as null or absent is.
...
// A stored field that is no identity: absent, null, or "" (BF-141). A write
// with "" is a write without identity (see present), so a record stored with
// "" must be matched like one stored with null, or its re-send is a second
// copy. `$in` compares each value literally, as `$eq` does.
function noIdentity () {
  return { $in: [null, ''] };
}
