...
function setTelemetry () {
  var mode = (readENV('NIGHTSCOUT_TELEMETRY', 'off') || 'off').toLowerCase();
  if (mode !== 'off' && mode !== 'aggregate') {
    env.err.push({
      desc: 'Invalid telemetry setting',
      err: 'NIGHTSCOUT_TELEMETRY must be off or aggregate'
    });
    mode = 'off';
  }

  var idRotation = (readENV('NIGHTSCOUT_TELEMETRY_ID_ROTATION', 'monthly') || 'monthly').toLowerCase();
  if (idRotation !== 'monthly') {
    env.err.push({
      desc: 'Invalid telemetry setting',
      err: 'NIGHTSCOUT_TELEMETRY_ID_ROTATION must be monthly'
    });
    idRotation = 'monthly';
  }
  ...
  env.telemetry = {
    ...
    manualSend: readENVTruthy('NIGHTSCOUT_TELEMETRY_MANUAL_SEND', false),
    scheduledSend: readENVTruthy('NIGHTSCOUT_TELEMETRY_SCHEDULED_SEND', false),
    idRotation: idRotation,
    secret: readENV('NIGHTSCOUT_TELEMETRY_SECRET', null),
    storeDir: readENV('NIGHTSCOUT_TELEMETRY_STORE', null),
    collection: readENV('NIGHTSCOUT_TELEMETRY_COLLECTION', 'telemetry')
  };
}
