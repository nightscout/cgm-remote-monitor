...
// Everything that works out a visitor's address, or needs the TRUST_PROXY
// policy itself, goes through this module and nothing else:
//   clientIPFor(env)  the function that turns a request into its client address
//   trustFor(env)     the compiled policy (app.js hands it to Express)
// Both are compiled once per env from env.trustProxy, so no two parts of
// Nightscout can resolve a request's address differently. Nothing else is
// exported, so there is no second way to build a policy.
...
// One entry per env, recompiled only if env.trustProxy changes.
...
function policyFor (env) {
  ...
  const policy = { value, trust, resolve: req => getClientIP(req, trust) };
  if (env) policies.set(env, policy);
  return policy;
}

function trustFor (env) {
  return policyFor(env).trust;
}

// Call it where the consumer is created, so an invalid TRUST_PROXY fails at
// boot rather than on the first request.
function clientIPFor (env) {
  return policyFor(env).resolve;
}

module.exports = { trustFor, clientIPFor };
