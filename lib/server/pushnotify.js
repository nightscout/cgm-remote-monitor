...
const NotificationCache = require('../utils/notification-cache');
...
function init (env, ctx) {
  ...
  // Expire owned notification state; do not evict live receipts or dedup markers.
  var receipts = new NotificationCache({ stdTTL: times.hour().secs, checkperiod: times.mins(5).secs });
  var recentlySent = new NotificationCache({ stdTTL: times.mins(15).secs, checkperiod: 20 });
  ...
  function close () {
    if (closed) return;
    closed = true;
    // Clear retained state and cancel each cache's owned housekeeping timer.
    receipts.flushAll();
    receipts.close();
    recentlySent.flushAll();
    recentlySent.close();
    if (ctx.bus) {
      ctx.bus.removeListener('notification', pushnotify.emitNotification);
      ctx.bus.removeListener('teardown', close);
    }
  }
  if (ctx.bus) ctx.bus.once('teardown', close);
