...
function storage (collection, ctx) {
  ...
  function removeOld (date, fn) {
    ...
    numDeletes += endMonth - 1;
    numDeletes += endDate.date() - 1;

    function removeOldFn (err, obj) {
      firstErr = firstErr || err;
      ++totalDeletes;

      console.log(totalDeletes + ' of ' + numDeletes + ' of devicestatus completed - ' + obj.result.n + ' records deleted');

      if (totalDeletes === numDeletes) {
        console.info('Finished deleting old devicestatus data: ', firstErr);
        fn(firstErr, date);
      }
    }
    ...
    for (year=2015; year < endDate.year(); ++year) {
      filter = { created_at: new RegExp('^' + year) };
      console.log('Deleting devicestatus data matching: ' + filter.created_at);
      retVal = api( ).deleteMany(filter, removeOldFn);
    }

    // Delete all data prior to month in endDate
    for (month=1; month < endMonth; ++month) {
      monthStr = (month >= 10) ? month : '0' + month;
      filter = { created_at: new RegExp('^' + year + '-' + monthStr) };
      console.log('Deleting devicestatus data matching: ' + filter.created_at);
      retVal = api( ).deleteMany(filter, removeOldFn);
    }
    ...
    for (day=1; day < endDate.date(); ++day) {
      ...
      filter = { created_at: new RegExp('^' + year + '-' + monthStr + '-' + dayStr) };
      console.log('Deleting devicestatus data matching: ' + filter.created_at);
      retVal = api( ).deleteMany(filter, removeOldFn);
    }
