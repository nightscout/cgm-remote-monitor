...
function init (env, ctx) {
  ...
  loop.sendNotification = function sendNotification (data, remoteAddress, completion) {
    ...
    let profile = ctx.ddata.profiles[0];
    let loopSettings = profile.loopSettings;
    ...
    var options = {
      ...
      // Profile's isAPNSProduction is the source of truth; env var is the fallback.
      production: profile.isAPNSProduction !== undefined
        ? profile.isAPNSProduction
        : env.extendedSettings.loop.pushServerEnvironment === "production"
    };
    ...
    // A provider holds its own HTTP/2 connection to APNs, kept open with a
    // heartbeat until shutdown() is called, so close it once this one send settles.
