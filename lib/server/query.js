...
const idForms = require('./object-id-forms');
const OBJECT_ID_HEX_RE = idForms.OBJECT_ID_HEX_RE;
...
function default_options (opts) {
  ...
  if (opts) {
    ...
    // A storage module that sets no `walker` has always had `date` and `sgv`
    // read as numbers. Without a named collection that is still the whole
    // default. With one, the schema's type wins for each of the two fields it
    // declares, and the numeric reading is kept for the ones it does not: the
    // activity schema declares neither and devicestatus declares only `date`,
    // and 15.0.8 matched numeric `date` and `sgv` filters on both (BF-106).
    // No schema declares either field as a string. A module that wants
    // neither conversion says so with `walker: {}`, as profile does.
    if (keys.indexOf('walker') < 1) {
      opts.walker = opts.collection
        ? legacyNumericDefaults(opts.collection)
        : { date: parseInt, sgv: parseInt };
    }
...
/**
  * The default walker's `date` and `sgv` for a collection with a schema: each
  * field the schema gives no type is read as a number, without truncating a
  * fractional bound (the same reading the schema gives a number field).
  *
  * @param {string} collection Collection name, eg 'activity'.
  * @returns {Object} A walker spec, possibly empty.
  */
function legacyNumericDefaults (collection) {
  var spec = { };
  ['date', 'sgv'].forEach(function each (field) {
    if (!coercion.coercerFor(collection, field)) {
      spec[field] = coercion.toNumber;
    }
  });
  return spec;
}
...
function updateIdQuery (query, opts) {
  ...
  if (query._id && typeof query._id === 'object') {
    // A list names each 24-hex id in both forms, so a record stored with the
    // string form is in (or out of) the list as the ObjectId form is.
    ['$in', '$nin'].forEach(function (op) {
      if (Array.isArray(query._id[op])) {
        query._id[op] = [].concat.apply([], query._id[op].map(function (x) {
          return idForms.isHexId(x) ? idForms.idForms(x) : [normalizeIdValue(x, opts).value];
        }));
      }
    });
    traverse(query._id).forEach(function (x) {
      if (this.path.length > 0 && Array.isArray(query._id[this.path[0]]) && (this.path[0] === '$in' || this.path[0] === '$nin')) {
        this.block();
        return;
      }
      if (this.isLeaf) {
