...
function enforceDateFilter (query, opts) {
  ...
  if (dateValue) {
    Object.keys(dateValue).forEach(function(key) {
      let dateString = dateValue[String(key)];
      if (isNaN(dateString)) {
        ...
        dateValue[String(key)] = d.toISOString();
      }
    });
  }
...
function walk_prop (prop, typer) {
  function iter (opts) {
    ...
    if (opts && opts.find && opts.find[String(prop)]) {
      if (typeof opts.find[String(prop)] === 'string') {
        //simple string property, no need to traverse
        opts.find[String(prop)] = typer(opts.find[String(prop)]);
      } else {
        // Traverse any query elements associated with this property.
        traverse(opts.find[String(prop)]).forEach(function (x) {
