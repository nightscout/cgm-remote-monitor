...
function updateIdQuery (query, opts) {
  ...
  if (query._id && typeof query._id === 'object') {
    // Complex operators normalize ObjectIds without rewriting UUID searches.
    queryLeaves(query._id, value => normalizeIdValue(value, opts).value);
  }
}
...
function normalizeOperands (query) {
  queryLeaves(query, function each (x, path) {
    return BOOLEAN_OPERANDS[path[path.length - 1]] ? readBooleanOperand(x) : x;
  });
  return query;
}
  ...
  function iter (opts) {
    ...
    if (opts && opts.find && opts.find[prop]) {
      if (typeof opts.find[prop] === 'string') {
        ...
        queryLeaves(opts.find[prop], function (x, path) {
          ...
          if (coercion.isValueLeaf(path)) {
            ...
            var reader = coercion.operandReaderFor(path);
            return reader ? reader(x) : x;
          }
        });
      }
    }
