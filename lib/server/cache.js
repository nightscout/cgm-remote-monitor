...
function cache (env, ctx) {
  ...
  const retentionPeriods = {
    ...
    , devicestatus: constants.TWO_DAYS
    , entries: constants.WO_DAYS
  };
  ...
  function dataChanged (operation) {
    ...
    if (operation.op == 'update') {
      ...
      data[operation.type] = mergeCacheArrays(data[operation.type], operation.changes, retentionPeriods[datatype]);
    }
  }
