...
const { isDeleted } = require('./soft-deleted');
...
function cache (env, ctx) {
  ...
  /* The retained documents, by reference, in a fresh array the caller owns.
   *
   * The ARRAY is the caller's: sorting or reversing it does not disturb the
   * cache. The DOCUMENTS are the cache's: writing to one writes to the cache.
   * A caller that modifies what it reads, or that hands the documents to
   * something that will, must use getData and pay for the copies.
   */
  data.getDataRef = (datatype) => {
    return data[datatype].slice();
  }

  /* mergeCacheArrays passes the incoming documents through
   * ddata.idMergePreferNew, which deep-clones them, so what the cache retains is
   * always a JSON round-trip of what was inserted: never a document still shared
   * with the writer that emitted it, and never an ObjectId or a Date. That is
   * what makes getDataRef interchangeable with getData for a caller that only
   * reads, and it is why the clone belongs on the way in, where it is bounded by
   * the batch, rather than on the way out, where it is bounded by the retention
   * period.
   */
  function merge (datatype, newData) {
    data[datatype] = mergeCacheArrays(data[datatype], newData, retentionPeriods[datatype]);
  }

  data.insertData = (datatype, newData) => {
    merge(datatype, newData);
    return data.getData(datatype);
  }

  /* As insertData, but the merged documents come back by reference — see
   * getDataRef for what the caller may and may not do with them.
   */
  data.insertDataRef = (datatype, newData) => {
    merge(datatype, newData);
    return data.getDataRef(datatype);
  }
  ...
  function dataChanged (operation) {
    ...
    if (operation.op == 'update') {
      // A record updated to isValid: false is deleted (see
      // lib/server/soft-deleted.js): take it out, as a removal does, instead
      // of keeping it in the data the site computes COB and IOB from.
      var changes = Array.isArray(operation.changes) ? operation.changes : [operation.changes];
      var deleted = changes.filter(isDeleted);
      if (deleted.length > 0) {
        deleted.forEach(function (doc) { removeFromArray(data[operation.type], doc._id); });
        removalGenerations[operation.type] += 1;
      }
      merge(operation.type, changes.filter(function (doc) { return doc && !isDeleted(doc); }));
    }
  }
