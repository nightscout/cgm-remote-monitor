...
function storage (env, ctx) {
  ...
  // srvModified on every item, srvCreated kept from the food each item
  // replaces (API v3 history reads them; see srv-dates).
  function stampReplacements (bulkOps) {
    return srvDates.carryForReplace(api(), bulkOps.filter(function (op) {
  ...
  function create (docs, fn) {
    ...
    return runWithCallback(async function () {
      ...
      try {
        await stampReplacements(bulkOps);
  ...
  function list (fn) {
    return runWithCallback(function () {
      ...
      return api().find(softDeleted.visible({ })).toArray();
    }, fn);
  }
  ...
  function listquickpicks (fn) {
    ...
    return runWithCallback(function () {
      return api()
        .find({ $and: [ { 'type': 'quickpick' }, { 'hidden': { $nin: [ true, 'true' ] } }, softDeleted.notDeleted() ] })
        .toArray()
  ...
  function listregular (fn) {
    return runWithCallback(function () {
      return api().find(softDeleted.visible({ 'type': 'food' })).toArray();
    }, fn);
  }
