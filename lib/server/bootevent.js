...
function boot (env, language) {
  ...
  //////////////////////////////////////////////////
  // Check Node version.
  // Nightscout only supports Active LTS releases from NodeJS, see https://nodejs.org/en/about/releases/ 
  // This means: Latest Node 10 LTS and Latest Node 12 LTS are recommended and supported.
  // Node 8 LTS is a Maintenance LTS and thus not recommend and not supported.
  // Latest Node version on Azure is tolerated, but not recommended
  // Latest Node (non LTS) version works, but are not recommended
  ...
  // ^10.16.3 works (Active LTS). Supported and recommended
  // 11.x     does not work, not supported
  // ^13||^14 does work, not recommended, will not be supported. We only support Active Node LTS releases
  ///////////////////////////////////////////////////
  function checkNodeVersion (ctx, next) {
    var semver = require('semver');
    var nodeVersion = process.version;

    if ( semver.satisfies(nodeVersion, '^10.16.3') ) {
      //Latest Node 10 LTS is ActiveLTS 
      console.debug('Node LTS version ' + nodeVersion + ' (Dubnium) is a Active LTS version and supported');
      next();
    }
    else if (semver.satisfies(nodeVersion, '^12.13.0')) {
      //Latest Node 12 LTS is ActiveLTS (almost)
      console.debug('Node LTS version ' + nodeVersion + ' (Erbium) is a Active LTS version and supported');
      next();
    }
    ...
    else if ( semver.satisfies(nodeVersion, '^13.0.0||^14.0.0')) {
        //Latest Node version
        console.debug('WARNING: Node version ' + nodeVersion + ' is not a LTS version. Not recommended. Not supported');
        next();
    } else {
      // Other versions will not start
      console.log( 'ERROR: Node version ' + nodeVersion + ' is not supported. Please upgrade to a secure Active LTS version Node');
      process.exit(1);
    }
  }
