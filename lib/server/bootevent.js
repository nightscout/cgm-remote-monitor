...
function boot (env, language) {
  ...
  function ensureIndexes (ctx, next) {
    ...
    let uniqueTreatmentFields = {
      'enteredBy': 1,
      'eventType': 1,
      'created_at': 1
    };
    console.info('ensuring unique treatment index for ' + JSON.stringify(uniqueTreatmentFields));
    ctx.treatments().ensureIndex(
      uniqueTreatmentFields,
      { 'unique': true },
      function (err) { if (err) { console.error('unable to ensure unique index for treatments'); } }
    );
