...
function boot (env, language) {
  ...
  function setupInternals (ctx, next) {
    ...
    ctx.remotecommands = require('./remotecommands')(env.remotecommands_collection, ctx);
  ...
  function ensureIndexes (ctx, next) {
    ...
    ctx.store.ensureIndexes(ctx.remotecommands( ), ctx.remotecommands.indexedFields);
