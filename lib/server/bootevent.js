...
function boot (env, language) {
  ...
  // ^12.12.0  does work, not recommended, will not be supported. We only support Active Node LTS releases
  ///////////////////////////////////////////////////
  function checkNodeVersion (ctx, next) {
    ...
    if ( semver.satisfies(nodeVersion, '^10.16.3') ) {
      ...
      console.debug('Node LTS version ' + nodeVersion + ' is a Active LTS version and supported');
      next();
    }
    else if (semver.satisfies(nodeVersion, '^12.12.0')) {
    ...
    else if ( semver.satisfies(nodeVersion, '^14.0.0')) {
        //Latest Node version
