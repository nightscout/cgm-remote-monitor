...
function boot (env, language) {
  ...
  function setupInternals (ctx, next) {
    ...
    if (env.settings.isEnabled('alexa') || env.settings.isEnabled('googlehome') || env.settings.isEnabled('alice')) {
    ...
    if (env.settings.isEnabled('alice')) {
      ctx.alice = require('../plugins/alice')(env, ctx);
    }
