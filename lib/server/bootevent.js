...
function boot (env, language) {
  ...
  function restoreHistoryClock (ctx, next) {
    if (hasBootErrors(ctx)) return next();
    require('./srv-dates').restore(ctx.store, env).then(function () {
      next();
    }, function (err) {
      // Do not admit writers with a clock behind an already issued cursor.
      ctx.bootErrors.push({ desc: 'Unable to restore history clock', err: err.message });
      next();
    });
  }
  ...
  function setupMMConnect (ctx, next) {
    ...
    const mmconnect = require('../plugins/mmconnect');
    ctx.mmconnect = mmconnect.init(env, ctx.entries, ctx.devicestatus, ctx.bus);
    if (ctx.mmconnect) {
      ctx.mmconnect.run();
      console.log("DEPRECATION WARNING", mmconnect.DEPRECATION_WARNING);
    }
    next( );
  }
