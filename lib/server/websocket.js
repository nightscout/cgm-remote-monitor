  ...
  function listeners () {
    io.sockets.on('connection', function onConnection (socket) {
      ...
      async function processSingleDbAdd(data, collection, maxtimediff) {
        ...
        // Sanitize free-text fields before they are persisted. REST writes
        // (lib/api/treatments, entries, devicestatus, profile) already do
        // this; without it here, a client with only write_treatment (or
        // generic write) scope could store raw HTML/script via dbAdd that
        // executes later when rendered (see lib/client/renderer.js,
        // lib/report_plugins/*.js).
        ctx.purifier.purifyObject(data.data);
