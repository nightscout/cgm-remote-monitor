      ...
      socket.on('dbUpdate', function dbUpdate (data, callback) {
        ...
        (async function () {
          ...
          try {
            ...
            if (match.many) {
              await mongoCollection.updateMany(match.filter, { $set: data.data });
            } else {
              await mongoCollection.updateOne(match.filter, { $set: data.data });
            }
          } catch (err) {
            console.error(err);
            if (reply) reply({ result: 'Unable to process update' });
      ...
      socket.on('dbUpdateUnset', function dbUpdateUnset (data, callback) {
        ...
        (async function () {
          ...
          try {
            mongoCollection = ctx.store.collection(collection);
            if (unsetMatch.many) {
              await mongoCollection.updateMany(unsetMatch.filter, { $unset: data.data });
            } else {
              await mongoCollection.updateOne(unsetMatch.filter, { $unset: data.data });
            }
          } catch (err) {
            console.error(err);
            if (reply) reply({ result: 'Unable to process update' });
        ...
        if (data.collection === 'treatments') {
          ...
          try {
            ...
            if (similar) {
              ...
              await mongoCollection.updateOne({ '_id': similar._id }, { $set: { created_at: data.data.created_at } });
              safelyNotify(function () { ctx.bus.emit('data-received'); });
