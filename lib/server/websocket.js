...
function init (env, ctx, server) {
  ...
  function emitData (delta) {
    if (lastData.cals) {
      ...
      Object.keys(events).forEach(id => {
        if (now - events[id].ts < 121000) {
            events[id].queue.push({type: dataUpdate, data: delta});
        } else {
            delete events[id];
        }
      });
  ...
  server.on('request', function (req, res) {
    if (req.method === 'GET' && req.url.startsWith('/api/v1/events')) {
      ...
      let queue;
      if (events[req.query.id]) {
          queue = events[req.query.id].queue;
          events[req.query.id].queue = [];
      } else {
          queue = [];
          var history = req.query.history || 48;
          ...
          if (lastData && lastData.splitRecent) {
              ...
              //send out first chunk
              queue.push({type: 'dataUpdate', data: split.first});

              //then send out the rest
              setTimeout(function sendTheRest() {
                  split.rest.delta = true;
                  events[req.query.id] && events[req.query.id].queue.push({type: 'dataUpdate', data: split.rest});
              }, 500);
          }
          events[req.query.id] = {queue: []};
      }
      events[req.query.id].ts = Date.now();
  ...
  io.emit = function (type, data) {
    io.__emit(type, data);
    if (type === 'dataUpdate' || type === 'retroUpdate' || type === 'notification' || type === 'announcement' || type === 'alarm' || type === 'urgent_alarm' || type === 'clear_alarm') {
        const now = Date.now();
        Object.keys(events).forEach(id => {
            if (now - events[id].ts < 121000) {
                events[id].queue.push({type, data});
            } else {
                delete events[id];
            }
        });
