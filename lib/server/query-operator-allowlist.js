...
//
// TWO OPERATORS ARE REFUSED HERE THAT WORK ON origin/dev TODAY, and both are
// deliberate:
//
...
//
//   $type  -- pure element matching and harmless in itself, but outside the
//             seam's AST and unused by any surveyed client. NOTE FOR PR #8737
//             (bf/coercion), which added readTypeOperand() so that
//             find[sgv][$type]=2 keeps reaching MongoDB as the number 2: this
//             allowlist runs first, so that reader becomes unreachable through
//             the HTTP path once both land. Adding '$type' to FIELD_OPERATORS
//             below is the one-line change that keeps it, at the cost of
//             differing from the seam. That is a maintainer's call, recorded
