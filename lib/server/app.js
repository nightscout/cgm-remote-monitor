...
function create (env, ctx) {
  ...
  Object.keys(appPages).forEach(function(page) {
    app.get(page, (req, res) => {
      if (page === '/admin' || page === '/data-sources') {
        // The private CareLink login display must not be clickjacked even when
        // public Nightscout charts are configured to allow framing.
  ...
  app.use('/api/v1/connect', require('../api/connect')(env, ctx));
  app.use('/api', apiRoot);
  app.use('/api/v1', api);
  app.use('/api/v2', api2);
  app.use('/api/v3', api3);
