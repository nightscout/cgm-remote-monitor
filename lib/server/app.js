...
function create (env, ctx) {
  ...
  app.set('trust proxy', require('./client-ip').compileTrust(env.trustProxy));
  ...
  if (!insecureUseHttp) {
    ...
    if (secureHstsHeader) { // Add HSTS (HTTP Strict Transport Security) header
      ...
      // Compose the enabled headers explicitly. CSP and framing are applied
      // below using Nightscout's embedding/report-only settings, independently
      // of HSTS. Do not inherit new cross-origin defaults from the umbrella API.
  ...
  let cacheBuster = process.env.NODE_ENV == 'development' ? 'developmentMode': randomBytes(8).toString('hex');
