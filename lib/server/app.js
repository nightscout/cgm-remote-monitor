...
function create (env, ctx) {
  ...
  Object.keys(appPages).forEach(function(page) {
    var page_config = appPages[page];

    var pageBuilder = assets(page, page_config);
    app.get(page, pageBuilder, (req, res) => {
  ...
  var clockAssets = assets('clock', { asset: 'clock' });
  app.use("/clock", clockAssets, clockviews);
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    app.get('/api/bundle/info', function (req, res, next) {
      var _webpack = res.locals.webpack;
      console.log("WEBPACK", _webpack);
      var stats = _webpack.devMiddleware.stats.toJson( );
      res.json({stats});
      next( );
    });
  }
