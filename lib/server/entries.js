...
function storage (env, ctx) {
  ...
  function create (docs, fn) {
    ...
    docs.forEach(function(doc) {
      ...
      let _sysTime;
      let originalOffset = 0; // Default to UTC

      if (doc.dateString) {
        // If dateString is provided, parse it with its timezone information
        _sysTime = dayjs.tz(doc.dateString);
        if (_sysTime.isValid()) {
          originalOffset = _sysTime.utcOffset(); // Get the offset from the original dateString
        } else {
          // Fallback if dateString is invalid
          _sysTime = dayjs.utc(); // Use current UTC time
        }
      } else if (doc.date) {
        // If only date (timestamp) is provided, assume it's UTC
        _sysTime = dayjs.utc(doc.date);
      } else {
        // Otherwise, use current UTC time
        _sysTime = dayjs.utc();
      }

      doc.utcOffset = originalOffset; // Store the original offset
      doc.sysTime = _sysTime.toISOString(); // Store as UTC ISO string
      doc.date = _sysTime.valueOf(); // Ensure doc.date is also UTC milliseconds

      // Ensure dateString is always set to the UTC ISO string for query consistency.
      // _sysTime is expected to be valid due to fallbacks in its creation.
      doc.dateString = _sysTime.toISOString();
