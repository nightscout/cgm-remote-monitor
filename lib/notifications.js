...
function init (env, ctx) {
  ...
  function findInfos ( ) {
    return _.filter(requests.notifies, {level: levels.INFO});
  }
  ...
  notifications.process = function process ( ) {
    var highestAlarm = findHighestAlarm();
    var longestSnooze = findLongestSnooze();

    if (longestSnooze) {
      if (highestAlarm && highestAlarm.level > longestSnooze.level) {
        console.log('notifications.process, ignoring snooze: ', longestSnooze, ' because notify: ', highestAlarm);
      } else {
    ...
    findInfos().forEach(function eachInfo (info) {
