...
function init (env, ctx) {
  ...
  notifications.snoozedBy = function snoozedBy (notify) {
    ...
    if (longest && Date.now() + longest.mills > alarm.lastAckTime + alarm.silenceTime) {
      return longest;
    } else {
      return null;
    }
  ...
  notifications.requestNotify = function requestNotify (notify) {
    if (!notify.level || !notify.title || !notify.message) {
  ...
  notifications.requestSnooze = function requestSnooze (snooze) {
    if (!snooze.level || !snooze.mills) {
      console.error(new Error('Unable to request snooze, since the snooze isn\'t complete: ' + JSON.stringify(notify)));
      return;
    }
  ...
  notifications.process = function process ( ) {
    ...
    if (snoozedBy) {
      ...
      notifications.ack(snoozedBy.level, snoozedBy.mills)
    }
