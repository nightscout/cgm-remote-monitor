...
function init (env, ctx) {
  ...
  notifications.secureAck = function secureAck (level, lenghtMills, t, sig) {
    var expected = notifications.sign(level, lenghtMills, t);

    if (expected && expected == sig) {
      notifications.ack(level, lenghtMills, true);
      return true
    } else {
      console.info(expected, ' != ', sig);
      return false;
    }
  };

  notifications.sign = function sign (level, lenghtMills, t) {

    console.info('trying to sign with', level, lenghtMills, t);
    if (env.api_secret) {
      var shasum = crypto.createHash('sha1');
      shasum.update(level.toString());
      shasum.update(lenghtMills.toString());
      shasum.update(t.toString());
