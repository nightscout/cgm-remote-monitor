...
function init (env, callback) {
  ...
  function reportAsCollection (name) {
    ...
    if (_.isArray(input)) {
      ...
      data[String(name)] = _.flatten(_.map(input, loadData));
    } else {
      data[String(name)] = loadData(input);
    }
        ...
        collection.find(wrapper.findQuery).toArray(function intercept (err, results) {
          ...
          if (wrapper.sortQuery) {
            ...
            if (field) {
              ...
              if (-1 === wrapper.sortQuery[String(field)]) {
...
function purgeCache(moduleName) {
  ...
  Object.keys(module.constructor._pathCache).forEach(function(cacheKey) {
    if (cacheKey.indexOf(moduleName)>0) {
      delete module.constructor._pathCache[String(cacheKey)];
    }
  });
}
...
function searchCache(moduleName, callback) {
  ...
  // the cache
  if (mod && ((mod = require.cache[String(mod)]) !== undefined)) {
