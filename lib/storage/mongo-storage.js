          ...
          .then(client => {
            ...
            .catch(err => {
              if (err.message && err.message.includes('AuthenticationFailed')) {
                ...
                cb(new Error('MongoDB authentication failed! Double check the URL has the right username and password.'), null);
                return;
              }
              ...
              if (err.name && err.name === "MongoNetworkError") {
                ...
                cb('MongoDB connection string ' + env.storageURI + ' seems invalid: ' + err.message);
              }
            });
