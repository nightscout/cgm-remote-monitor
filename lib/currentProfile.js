...
var updateHandler = null;
	...
	function targetBGLow(time) {
		if (!record) return null;
		var minutes =  time.getMinutes() + time.getHours() * 60;
		for (var i=0; i < record.targetBG.length -1; i++) {
			if (parseInt(record.targetBG[i+1].from)>minutes) return record.targetBG[i].low;
		}
		return null;
	}
	
	function targetBGHigh(time) {
		if (!record) return null;
		var minutes =  time.getMinutes() + time.getHours() * 60;
		for (var i=0; i < record.targetBG.length -1; i++) {
			if (parseInt(record.targetBG[i+1].from)>minutes) return record.targetBG[i].high;
		}
		return null;
	}
