...
function init (client, serverSettings, $) {
  ...
  try {
    ...
    //Pick up current profile TargetBG and override
    var currTarget = profile.getHighBGTarget();
    if (currTarget) {
        settings.thresholds.bgTargetTop = currTarget;
    }
    currTarget = profile.getLowBGTarget();
    if (currTarget) {
        settings.thresholds.bgTargetBottom = currTarget;
    }
