...
function init(client, $) {
  ...
  var iob = Nightscout.plugins('iob');
  var cob = Nightscout.plugins('cob');
  ...
  boluscalc.updateVisualisations = function updateVisualisations (sbx) {
    boluscalc.lastsbx = sbx;
    boluscalc.oldbg = false;
    var eventTime = mergeDateAndTime().toDate();
    var sensorbg = 0;
    var lastSGV = _.last(sbx.data.sgvs);
    if (lastSGV) {
      sensorbg = lastSGV.mgdl;
      if (sensorbg < 39) sensorbg = 0;
      else sensorbg = client.utils.scaleMgdl(sensorbg);
      if (eventTime.getTime() - lastSGV.mills > 10 * 60 * 1000) {
...
//      console.log('New BG received', sensorbg, 'Sec diff', (new Date().getTime() - lastSGV.mills)/1000);
//      console.log('Current', new Date().toLocaleString(), 'LastSGV', new Date(lastSGV.mills).toLocaleString());
    }
...
//        Nightscout.utils.updateBrushToTime(Nightscout.utils.mergeInputTime($('#bc_eventTimeValue').val(), $('#bc_eventDateValue').val()).toDate());
  ...
  boluscalc.toggleDrawer = function toggleDrawer (event) {
    client.browserUtils.toggleDrawer('#boluscalcDrawer', boluscalc.prepare);
    maybePrevent(event);
  };
  ...
  boluscalc.prepare = function prepare( )  {
    ...
    setDateAndTime();
    if (boluscalc.lastsbx) {
      boluscalc.updateVisualisations(boluscalc.lastsbx);
    }
    boluscalc.calculateInsulin();
  }
  ...
  boluscalc.updateGui = function updateGui (record) {
    ...
    // Show basal rate
    $('#bc_basal').text(client.sbx.data.profile.getBasal(record.eventTime).toFixed(3));
  }
  ...
  boluscalc.gatherBoluscalcData = function gatherBoluscalcData() {
    ...
    if (!client.sbx) {
      console.log('No sandbox data yet. Exiting gatherData()');
      return;
    }
    ...
    // Load profile
    var targetBGLow = client.sbx.data.profile.getLowBGTarget(record.eventTime);
    var targetBGHigh = client.sbx.data.profile.getHighBGTarget(record.eventTime);
    var isf = client.sbx.data.profile.getSensitivity(record.eventTime);
    var ic = client.sbx.data.profile.getCarbRatio(record.eventTime);
    ...
    record.iob = 0;
    if ($('#bc_useiob').is(':checked')) {
      record.iob = roundTo(iob.calcTotal(client.sbx.data.treatments,client.sbx.data.profile,record.eventTime).iob, 0.01);
    }
    ...
    record.insulincob = 0;
    if ($('#bc_usecob').is(':checked')) {
      record.cob = roundTo(cob.cobTotal(client.sbx.data.treatments,client.sbx.data.profile,record.eventTime).cob, 0.01);
    ...
    if ($('#bc_usebg').is(':checked')) {
      ...
      if (isNaN(record.bg)) record.bg = 0;
  ...
  boluscalc.loadFoodDatabase = function loadFoodDatabase(callback) {
    categories = [];
    foodlist = [];
  ...
  function addFoodFromDatabase(event) {
    if (!databaseloaded) {
      boluscalc.loadFoodDatabase(addFoodFromDatabase);
      if (event) event.preventDefault();
      return;
    }
  ...
  function findClosestSGVToPastTime(time) {
    var closeBGs = client.sbx.data.filter(function(d) {
    ...
    return totalBG > 0 ? client.utils.scaleMgdl(totalBG / closeBGs.length) : 0;
  }
