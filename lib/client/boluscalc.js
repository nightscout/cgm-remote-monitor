...
function init (client, $) {
  ...
  boluscalc.updateGui = function updateGui (record) {
    ...
    if (record.foods.length) {
      ...
      for (var fi = 0; fi < record.foods.length; fi++) {
        var f = record.foods[parseInt(fi)];
  ...
  function doFilter (event) {
    ...
    for (var i = 0; i < foodlist.length; i++) {
      if (filter.category !== '' && foodlist[parseInt(i)].category !== filter.category) { continue; }
      if (filter.subcategory !== '' && foodlist[parseInt(i)].subcategory !== filter.subcategory) { continue; }
      if (filter.name !== '' && foodlist[parseInt(i)].name.toLowerCase().indexOf(filter.name.toLowerCase()) < 0) { continue; }
      var o = '';
      o += foodlist[parseInt(i)].name + ' | ';
      o += 'Portion: ' + foodlist[parseInt(i)].portion + ' ';
      o += foodlist[parseInt(i)].unit + ' | ';
      o += 'Carbs: ' + foodlist[parseInt(i)].carbs + ' g';
      ...
      , buttons: [
        {
          ...
          , click: function() {
            ...
            if (index !== null && !isNaN(portions) && portions > 0) {
              foodlist[parseInt(index)].portions = portions;
              foods.push(_.cloneDeep(foodlist[parseInt(index)]));
              $(this).dialog('close');
              boluscalc.calculateInsulin();
            }
          }
