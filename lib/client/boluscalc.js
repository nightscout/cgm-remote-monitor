...
//        Nightscout.utils.updateBrushToTime(Nightscout.utils.mergeInputTime($('#bc_eventTimeValue').val(), $('#bc_eventDateValue').val()).toDate());
  ...
  var plugins = {
    ...
    , foods: {
      show: function showFoods (record) {
      ...
      , load: function loadFoods (record) {
        record.carbs = 0;
    ...
    , carbs: {
      show: function showCarbs (record) {
      ...
      , load: function loadCarbs (record) {
    ...
    , basalRate: {
      show: function showBasalRate (record) {
    ...
    , corrections: {
      load: function loadCorrections (record) {
        record.othercorrection = parseFloat($('#bc_othercorrection').val());
      }
    }
    , profile: {
      load: function loadProfile (record) {
  ...
  boluscalc.updateGui = function updateGui (record) {
    ...
    plugins.IOB.show(record);
    plugins.COB.show(record);
    plugins.BG.show(record);
    plugins.foods.show(record);
    plugins.carbs.show(record);
    plugins.total.show(record);
  ...
  boluscalc.gatherBoluscalcData = function gatherBoluscalcData() {
    ...
    plugins.IOB.load(record);
    plugins.COB.load(record);
    plugins.BG.load(record);
    plugins.foods.load(record);
    plugins.carbs.load(record);
    plugins.corrections.load(record);
    plugins.total.load(record);
  };
