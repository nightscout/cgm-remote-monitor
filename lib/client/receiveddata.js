...
function mergeDataUpdate (isDelta, cachedDataArray, receivedDataArray, maxAge) {
  ...
  function nsArrayDiff (oldArray, newArray) {
    ...
    for (var i = 0; i < l; i++) {
      if (oldArray[parseInt(i)] !== null) {
        seen.push(oldArray[parseInt(i)].mills);
      }
    }
    ...
    l = newArray.length;
    for (var j = 0; j < l; j++) {
      if (!seen.includes(newArray[parseInt(j)].mills)) {
        result.push(newArray[parseInt(j)]); //console.log('delta data found');
      }
    }
    return result;
  }
  ...
  for (var i = 0; i < cachedDataArray.length; i++) {
    var element = cachedDataArray[parseInt(i)];
...
function mergeTreatmentUpdate (isDelta, cachedDataArray, receivedDataArray) {
  ...
  for (var i = 0; i < l; i++) {
    var no = receivedDataArray[parseInt(i)];
    if (!no.action) {
      cachedDataArray.push(no);
      continue;
    }
    for (var j = 0; j < m; j++) {
      if (no._id === cachedDataArray[parseInt(j)]._id) {
        if (no.action === 'remove') {
