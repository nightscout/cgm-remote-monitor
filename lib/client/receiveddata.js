...
function mergeTreatmentUpdate (isDelta, cachedDataArray, receivedDataArray) {
  ...
  //
  // The inner bound is read fresh on every comparison, because this loop
  // MUTATES the array it is scanning: `push` above grows it and `splice`
  // below shrinks it. A length captured once went stale the first time a
  // 'remove' matched, and the next received item that matched nothing read
  // past the end and threw on `undefined._id` - taking the whole socket
  // update with it. mergeDataUpdate, thirty lines up, already does it this
  // way.
...
function receiveDData (received, ddata, settings) {
  ...
  if (received.cals) {
    ...
    ddata.cal = ddata.cals?.[ddata.cals?.length - 1]
  }
