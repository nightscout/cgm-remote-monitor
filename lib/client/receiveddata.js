...
/*  _.forEach(cachedDataArray,function(value, key) {
  	var element = cachedDataArray[key];
  	if (element !== null && element.mills <= twoDaysAgo) {
  		delete cachedDataArray[key];
  		purgeCount += 1;
  	}
  });
  */
...
function receiveDData (received, ddata, settings) {
  ...
  console.log('Total devicestatus data size', ddata.devicestatus.length);
  ...
  if (received.devicestatus) {
    if (settings.extendedSettings.devicestatus && settings.extendedSettings.devicestatus.advanced) {
      ...
      ddata.devicestatus = mergeDataUpdate(received.delta, ddata.devicestatus, received.devicestatus, TWELVE_HOURS);
    } else {
