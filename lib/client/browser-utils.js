...
var SCREEN_WIDTH_TO_ANIMATE = 1000;
...
function init ($) {
  ...
  function animationTimer() {
    if (screen.width >= SCREEN_WIDTH_TO_ANIMATE) {
     return 300;
    }
    return 0;
  }
  ...
  function closeDrawer(id, callback) {
    ...
    $(id).animate({right: '-300px'}, animationTimer(), function () {
      $(id).css('display', 'none');
  ...
  function openDrawer(id, prepare) {
    ...
    closeOpenDraw(function () {
      lastOpenedDrawer = id;
      if (prepare) { prepare(); }
      $(id).css('display', 'block').animate({right: '0'}, animationTimer());
    });
