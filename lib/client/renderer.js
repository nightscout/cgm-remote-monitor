...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    prepareFocusCircles(focusCircles.transition().duration(10));
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles () {
    ...
    // if already existing then transition each circle to its new position
    prepareTreatCircles(treatCircles.transition().duration(10));
    ...
    treatRects.transition().duration(10)
      .attr('transform', rectTranslate);

    chart().focus.selectAll('.g-duration-rect').transition().duration(10)
      .attr('width', function(d) {
        return chart().xScale(new Date(d.mills + times.mins(d.duration).msecs)) - chart().xScale(new Date(d.mills));
      });

    chart().focus.selectAll('.g-duration-text').transition().duration(10)
  ...
  renderer.addContextCircles = function addContextCircles () {
    ...
    // if already existing then transition each circle to its new position
    prepareContextCircles(contextCircles.transition().duration(10));
