    ...
    renderer.drag = d3.behavior.drag()  
      .on('dragstart', function() { 
        ...
        deleteRect = {
          ...
          y: 50,
          width: 50,
          height: chart().yScale(chart().yScale.domain()[0]) - 100
        };
      ...
      .on('drag', function() { 
        ...
        operation = 'Move';
        if (isInRect(x, y, deleteRect)) {
          operation = 'Remove';
        }
        if (isInRect(x, y, insulinRect)) {
          operation = 'Move insulin';
        }
        if (isInRect(x, y, carbsRect)) {
        ...
        switch (operation) {
          ...
          case 'Move insulin':
            if (window.confirm(translate('Change insulin time to %1 ?', { params: [newTime.toLocaleTimeString()] } ))) {
              if (treatment.carbs) {
                dboperation = 'dbUpdateUnset';
              } else {
                dboperation = 'dbRemove';
              }
              client.socket.emit(
                dboperation, 
                { 
