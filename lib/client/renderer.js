...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    prepareFocusCircles(focusCircles.transition().duration(DEFAULT_TRANS_MS));
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles () {
    ...
    // if already existing then transition each circle to its new position
    prepareTreatCircles(treatCircles.transition().duration(DEFAULT_TRANS_MS));
    ...
    treatRects.transition().duration(DEFAULT_TRANS_MS)
      .attr('transform', rectTranslate);

    chart().focus.selectAll('.g-duration-rect').transition().duration(DEFAULT_TRANS_MS)
      .attr('width', function(d) {
        return chart().xScale(new Date(d.mills + times.mins(d.duration).msecs)) - chart().xScale(new Date(d.mills));
      });

    chart().focus.selectAll('.g-duration-text').transition().duration(DEFAULT_TRANS_MS)
