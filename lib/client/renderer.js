...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    var shownForecastPoints = [ ];

    if (client.sbx.pluginBase.forecastPoints) {
      shownForecastPoints = _.filter(client.sbx.pluginBase.forecastPoints, function isShown (point) {
    ...
    var focusCircles = chart().focus.selectAll('circle.entry-dot').data(focusData, client.entryToDate);
