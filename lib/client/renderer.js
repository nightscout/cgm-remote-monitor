...
function init (client, d3) {
  ...
  function prepareArc (treatment, radius, bolusSettings) {
    ...
    if (treatment.insulin > 0) {
      ...
      if (_.includes(['concise', 'minimal'], bolusSettings.renderFormat)) {
        dosage_units = (dosage_units + "").replace(/^0/, "");
      }

      var unit_of_measurement = (bolusSettings.renderFormat === 'minimal' ? '' : ' U'); // One international unit of insulin (1 IU) is shown as '1 U'
  ...
  renderer.drawTreatments = function drawTreatments (client) {
    ...
    _.forEach(client.ddata.treatments, function eachTreatment (d) {
      var showLabels =  d.carbs || d.insulin > bolusSettings.renderOver;
