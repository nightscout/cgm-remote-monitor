...
var treatmentTime = require('./treatmenttime');
...
var profileRedirectRequested = false;
...
function init (client, d3) {
        ...
        switch (operation) {
          case 'Move':
            if (window.confirm(translate('Change treatment time to %1 ?', { params: [newTime.toLocaleTimeString()] }))) {
              moved = treatmentTime.timeFields(treatment, newTime);
              client.socket.emit(
                'dbUpdateUnset', {
          ...
          case 'Move insulin':
            if (window.confirm(translate('Change insulin time to %1 ?', { params: [newTime.toLocaleTimeString()] }))) {
              moved = treatmentTime.timeFields(treatment);
              client.socket.emit(
                'dbUpdateUnset', {
                  collection: 'treatments'
                  , _id: treatment._id
                  , data: Object.assign({ insulin: 1 }, moved.unset)
                }
              );
              if (moved.set.date !== undefined) {
                client.socket.emit(
                  'dbUpdate', {
                    collection: 'treatments'
                    , _id: treatment._id
              ...
              newTreatment = treatmentTime.splitRecord(treatment, 'carbs', newTime);
              client.socket.emit(
                'dbAdd', {
                  collection: 'treatments'
                  , data: newTreatment
          ...
          case 'Move carbs':            if (window.confirm(translate('Change carbs time to %1 ?', { params: [newTime.toLocaleTimeString()] }))) {
              ...
              client.socket.emit(
                'dbUpdateUnset', {
                  collection: 'treatments'
                  , _id: treatment._id
                  , data: Object.assign({ carbs: 1 }, moved.unset)
                }
              );
              if (moved.set.date !== undefined) {
                client.socket.emit(
              ...
              newTreatment = treatmentTime.splitRecord(treatment, 'insulin', newTime);
              client.socket.emit(
                'dbAdd', {
                  collection: 'treatments'
                  , data: newTreatment
  ...
  renderer.addBasals = function addBasals (client) {
    ...
    if (profile?.activeProfileToTime(from) == null) {
      if (profileRedirectRequested) { return; }
      profileRedirectRequested = true;
