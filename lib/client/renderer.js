...
var times = require('../times');
  //DEFAULT WIDTH_SMALL_DOTS=420 WIDTH_BIG_DOTS=800  TOOLTIP_TRANS_MS=100  TOOLTIP_WIDTH=150
var DEFAULT_FOCUS = times.hours(3).msecs
  , WIDTH_SMALL_DOTS = 250
  , WIDTH_BIG_DOTS = 500
...
function init (client, d3) {
  ...
  function appendLabels(treatmentDots, arc, opts) {
    ...
    if (opts.showLabels) {
      ...
      label.append('text')
        // reduce the treatment label font size to make it readable with SMB  default 40 and .10em for dy 2.1v1.6 added addl xform
        .style('font-size', 30 / opts.scale)
        .style('text-shadow', '0px 0px 10px rgba(0, 0, 0, 1)')
        .attr('text-anchor', 'middle')
        .attr('dy', '2em')
        .attr("transform", "rotate(-45)")
        .attr('transform', "rotate(-45)" function (d) {
          d.outerRadius = d.outerRadius * 1.6;
          d.innerRadius = d.outerRadius * 1.6;
