...
function init (client, d3) {
  ...
  renderer.addTreatmentProfiles = function addTreatmentProfiles (client) {
    ...
    // treatment profile switch
    var treatProfiles = chart().focus.selectAll('.g-profile').data(client.treatments.filter(function(treatment) {
      return treatment.eventType === 'Profile Switch';
    }));

    treatProfiles.transition()
      .attr('transform', function (t) {
        return 'rotate(-90,' + chart().xScale(t.mills) + ',' + chart().yScaleBasals(-0.05) + ') ' +
      'translate(' + chart().xScale(t.mills) + ',' + chart().yScaleBasals(-0.05) + ')' 
      });

    //var g = treatProfiles.enter().append('g');
    treatProfiles.enter().append('text')
      ...
      .text(function (t) { return '---  ' + t.profile + '  ---'; })
      .on('mouseover', function (d) {
        client.tooltip.transition().duration(TOOLTIP_TRANS_MS).style('opacity', .9);
