...
function init (client) {
  ...
  renderer.highlightBrushPoints = function highlightBrushPoints(data) {
    if (data.mills >= client.chart.brush.extent()[0].getTime() && data.mills <= client.chart.brush.extent()[1].getTime()) {
      return client.chart.futureOpacity(data.mills - client.latestSGV.mills);
    } else {
      return 0.5;
    }
  };
  ...
  renderer.addFocusCircles = function addFocusCircles ( ) {
    ...
    var focusCircles = client.chart.focus.selectAll('circle').data(focusData, client.dateFn);
    ...
    function prepareFocusCircles(sel) {
      ...
      sel.attr('cx', function (d) {
        ...
        .attr('opacity', function (d) {
          return client.chart.futureOpacity(d.mills - client.latestSGV.mills);
        })
    ...
    function focusCircleTooltip (d) {
      ...
      client.tooltip.html('<strong>' + bgType() + ' BG:</strong> ' + client.sbx.scaleEntry(d) +
        ...
        (rawbgInfo.value ? '<br/><strong>Raw BG:</strong> ' + rawbgInfo.value : '') +
        (rawbgInfo.noise ? '<br/><strong>Noise:</strong> ' + rawbgInfo.noise : '') +
        '<br/><strong>Time:</strong> ' + client.formatTime(new Date(d.mills)))
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles ( ) {
    ...
    // bind up the focus chart data to an array of circles
    var treatCircles = client.chart.focus.selectAll('rect').data(client.treatments.filter(function(treatment) {
  ...
  renderer.addContextCircles = function addContextCircles ( ) {
    ...
    var contextCircles = client.chart.context.selectAll('circle').data(client.data);
    ...
    function prepareContextCircles(sel) {
      ...
      sel.attr('cx', function (d) { return client.chart.xScale2(new Date(d.mills)); })
        .attr('cy', function (d) {
          ...
          if (isNaN(scaled)) {
            badData.push(d);
            return client.chart.yScale2(utils.scaleMgdl(450));
          } else {
  ...
  renderer.drawTreatment = function drawTreatment(treatment, scale, showValues) {
    ...
    var treatmentDots = client.chart.focus.selectAll('treatment-dot')
      .data(arc_data)
      .enter()
      .append('g')
