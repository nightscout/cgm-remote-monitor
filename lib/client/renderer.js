  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    function prepareTreatCircles (sel) {
      ...
      updateTreatCircles(sel)
        ...
        .attr('fill', fillColor)
// added fill-opacity for Circles
...
//        client.tooltip.style('opacity', .9);
        client.tooltip.style('opacity', .5);
		client.tooltip.style('fill-opacity', .5);
  ...
  function prepareArc (treatment, radius, bolusSettings) {
    var arc_data = [
      ...
      { 'element': '', 'color': '#300099ff', 'start': 1.5708, 'end': 4.7124, 'inner': 0, 'outer': radius.R1 },
      ...
      .on('start', function() {
        ...
        if (treatment.insulin && treatment.carbs) {
          ...
          chart().drag.append('rect')
            ...
            .attr('fill', '#300099ff')
// added fill-opacity to reduce insulin bolus visual effect
...
//        ret = sign + '    ' + t.cutting + '    ' + '►►►' + '    ' + t.profile + '    ' + sign;
        ret = sign + '►' + t.cutting.substr(0,13) + sign;
      } else {
//        ret = sign + ' ' + t.profile + '    ' + sign;
        ret = sign + '▬' + t.profile.substr(0,13) + sign;
      }
      return ret;
    };
