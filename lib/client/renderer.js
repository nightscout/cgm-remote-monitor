...
function init (client) {
  ...
  renderer.highlightBrushPoints = function highlightBrushPoints(data) {
    if (data.mills >= client.brush.extent()[0].getTime() && data.mills <= client.brush.extent()[1].getTime()) {
    ...
    function prepareFocusCircles(sel) {
      ...
      sel.attr('cx', function (d) {
        if (!d) {
          ...
          return client.xScale(new Date(0));
        } else if (!d.mills) {
          console.error('Bad data, no mills', d);
          return client.xScale(new Date(0));
        } else {
          return client.xScale(new Date(d.mills));
        }
      })
        .attr('cy', function (d) {
          ...
          if (isNaN(scaled)) {
            badData.push(d);
            return client.yScale(scaleBg(450));
          } else {
            return client.yScale(scaled);
          }
        })
        ...
        if (d.type === 'sgv' || d.type === 'mbg') {
          ...
          if (d.type === 'sgv') {
            if (client.rawbg.showRawBGs(d.mgdl, d.noise, cal, client.sbx)) {
              rawbgValue = scaleBg(client.rawbg.calc(d, cal, client.sbx));
            }
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles ( ) {
    ...
    // bind up the focus chart data to an array of circles
    var treatCircles = client.focus.selectAll('rect').data(treatments.filter(function(treatment) {
    ...
    function prepareTreatCircles(sel) {
      ...
      sel.attr('cx', function (d) {
        ...
        .attr('cy', function (d) {
          return client.yScale(client.sbx.scaleEntry(d));
        })
        .attr('r', function () {
  ...
  renderer.addContextCircles = function addContextCircles ( ) {
    ...
    function prepareContextCircles(sel) {
      var badData = [];
      sel.attr('cx', function (d) { return client.xScale2(new Date(d.mills)); })
        .attr('cy', function (d) {
          var scaled = client.sbx.scaleEntry(d);
          if (isNaN(scaled)) {
            badData.push(d);
            return client.yScale2(scaleBg(450));
          } else {
            return client.yScale2(scaled);
          }
        })
  ...
  renderer.drawTreatment = function drawTreatment(treatment, scale, showValues) {
    ...
    if (Math.abs(client.xScale(new Date(treatment.mills))) > window.innerWidth) { return; }
    ...
    var treatmentDots = client.focus.selectAll('treatment-dot')
      ...
      .enter()
      .append('g')
      .attr('transform', 'translate(' + client.xScale(new Date(treatment.mills)) + ', ' + client.yScale(client.sbx.scaleEntry(treatment)) + ')')
