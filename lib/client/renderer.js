...
function init (client, d3) {
  ...
  // Short alias: escapes untrusted treatment/profile fields (notes,
  // enteredBy, reason, ...) before they are concatenated into HTML strings
  // that get inserted with .html(). Server-side purification
  // (lib/server/purifier.js) is the primary control; this is
  // defense-in-depth so a single missed write path can't lead to XSS.
  // Falls back to identity when utils.escapeHtml isn't available (e.g.
  // lightweight test doubles for unrelated renderer functions).
  var e = (utils && typeof utils.escapeHtml === 'function') ? utils.escapeHtml : function identity (s) { return s; };
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    function treatmentTooltip (d) {
      ...
      return '<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + '<br/>' +
        ...
        (d.glucose ? '<strong>' + translate('BG') + ':</strong> ' + d.glucose + (d.glucoseType ? ' (' + e(translate(d.glucoseType)) + ')' : '') + '<br/>' : '') +
  ...
  function appendTreatments (treatment, arc) {
    ...
    function treatmentTooltip () {
      ...
      client.tooltip.html('<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(treatment)) + '<br/>' + '<strong>' + translate('Treatment type') + ':</strong> ' + e(translate(client.careportal.resolveEventName(treatment.eventType))) + '<br/>' +
          ...
          (treatment.glucose ? '<strong>' + translate('BG') + ':</strong> ' + glucose + (treatment.glucoseType ? ' (' + e(translate(treatment.glucoseType)) + ')' : '') + '<br/>' : '') +
