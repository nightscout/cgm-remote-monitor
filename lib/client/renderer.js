...
function init (client, d3) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    var treatCircles = chart().focus.selectAll('.treatment-dot').data(client.ddata.treatments.filter(function(treatment) {
      ...
      if(treatment.eventType=='Note' && treatment.notes.includes(treatment.originalProfileName)){
        var noteIsTempProfile=true;
      }
      ...
      return !noteIsTempProfile && notCarbsOrInsulin && !treatment.duration && treatment.durationType !== 'indefinite' && notTempOrProfile && notOpenAPSSpam;
  ...
  function prepareArc (treatment, radius, bolusSettings) {
    ...
    if (treatment.carbs > 0) {
      ...
      arc_data_1_elements.push(Math.round(treatment.carbs) + ' g' + (treatment.duration != undefined ? '(->)' : ''));
    }
  ...
  function appendTreatments (treatment, arc) {
    ...
    function treatmentTooltip () {
      ...
      client.tooltip.html('<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(treatment)) + '<br/>' + '<strong>' + translate('Treatment type') + ':</strong> ' + translate(client.careportal.resolveEventName(treatment.eventType)) + '<br/>' +
          ...
          (treatment.duration ? '<strong>Duration:</strong> ' + treatment.duration/1000/60 + 'min' : '') +
  ...
  renderer.addTreatmentProfiles = function addTreatmentProfiles (client) {
    ...
    var generateText = function(t) {
      ...
      var ret;
      if (t.cutting) {
        if(/\(\d{2,3}%\)/.test(t.cutting)){
          //ret = '100% ▲▲▲';
          ret = '--▲--';//'▲▲ ' + t.cutting.match(/\(\d{2,3}%\)/)[0] + ' ▲▲';
        }else{
          ret = sign + ' ' + t.cutting + ' ' + '►►►' + ' ' + t.profile + ' ' + sign;
        }
      } else {
        if(/\(\d{2,3}%\)/.test(t.profile)){
          ret = '-▼ ▼-';//'▼▼ ' + t.profile.match(/\(\d{2,3}%\)/)[0] + ' ▼▼';
    ...
    function transformera(t){
      var toppen = topOfText;
      var xoffset = 0;
      if(/\(\d{2,3}%\)/.test(t.profile) || t.cutting){
        toppen = -6;
        if(t.cutting)
          toppen = -6.1;
          xoffset = 0;//-8;
      ...
      return 'rotate(-90 ' + (chart().xScale(t.mills) + xoffset) + ',' + chart().yScaleBasals(toppen) + ') ' +
        'translate(' + (chart().xScale(t.mills) + xoffset) + ',' + chart().yScaleBasals(toppen) + ')';
    }
    
    //scroll-function
    treatProfiles.attr('transform', function(t){return transformera(t);})
    .text(generateText);
    ...
    treatProfiles.enter().append('text')
      ...
      .attr('fill', function(t) {
        if (t.cutting)
          return '#5d6efc';
        else if(/\(\d{2,3}%\)/.test(t.profile))
          return '#03f8fc';
        else
          return '#0099ff';
        //return t.profile.includes('%') ? '#0099ff' : '#4ceb34';
      })
      .attr('text-anchor', 'end')
      .attr('dy', '.35em')
      .attr('transform', function(t){return transformera(t);})
      .text(generateText)
      .on('mouseover', function(d) {
        client.tooltip.style('opacity', .9);
