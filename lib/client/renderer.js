...
function init (client, d3) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    var durationTreatments = client.ddata.treatments.filter(function(treatment) {
      return !treatment.carbs && !treatment.insulin && (treatment.duration !== undefined || treatment.durationType !== undefined) &&
    ...
    gs.append('text')
      ...
      .style('font-size', 15)
      .attr('fill', 'white')
      .attr('text-anchor', function(d) { return d.durationType === "indefinite" ? 'left' : 'middle'} )
      .attr('dy', '.35em')
      .attr('transform', treatmentTextTransform)
      .text(function(d) {
        if (d.eventType === 'Temporary Target') {
