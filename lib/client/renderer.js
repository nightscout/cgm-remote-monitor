...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles ( ) {
    ...
    if (client.sbx.pluginBase.forecastPoints) {
      var maxForecastMills = _.max(_.map(client.sbx.pluginBase.forecastPoints, function (point) {return point.mills}));
      client.forecastTime = maxForecastMills - client.now;
