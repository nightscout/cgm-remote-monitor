...
client.load = function load (serverSettings, callback) {
  ...
  function updateBrushToNow (skipBrushing) {
    ...
    console.log('Resetting brush in updateBrushToNow');

    // chart.theBrush.node().transition().duration(UPDATE_TRANS_MS).call(chart.brush.move, brushExtent.map(chart.xScale2));
    chart.theBrush.transition().call(chart.brush.move, brushExtent.map(chart.xScale2));
