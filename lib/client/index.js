...
// clears the current user brush and resets to the current real time data
  ...
  function generateAlarm (file, notify) {
    ...
    function onSnoozeUntilSelectChange() {
        hrValue = snoozeUntilHrSelect.value;
        minValue = snoozeUntilMinSelect.value;
        var now = new Date();
        var future = new Date(
            now.getFullYear(),
            now.getMonth(),
        ...
        var msDiff = future.getTime() - now.getTime();
        snoozeUntilHrefEl.setAttribute("data-snooze-time", msDiff);
    }

    var snoozeUntilHrSelect = document.createElement("select");
    for (var i = 0; i < 24; ++i) {
        var tempHrOption = new Option(i);
        snoozeUntilHrSelect.append(tempHrOption);
    }
    snoozeUntilHrSelect.onchange = onSnoozeUntilSelectChange;

    var snoozeUntilMinSelect = document.createElement("select");
    for (var i = 0; i <= 60; ++i) {
        var tempMinOption = new Option(i);
        snoozeUntilMinSelect.append(tempMinOption);
    }
    snoozeUntilMinSelect.onchange = onSnoozeUntilSelectChange;

    var snoozeUntilHrefEl = document.createElement("a");
    snoozeUntilHrefEl.append(document.createTextNode("Silence until "));
    snoozeUntilHrefEl.append(snoozeUntilHrSelect);
    snoozeUntilHrefEl.append(document.createTextNode(" : "));
    snoozeUntilHrefEl.append(snoozeUntilMinSelect);

    var DEFAULT_HR = 20;
    var DEFAULT_MIN = 0;
    snoozeUntilHrSelect.value = DEFAULT_HR;
    snoozeUntilMinSelect.value = DEFAULT_MIN;
    onSnoozeUntilSelectChange();

    var snoozeUntilLiEl = document.createElement("li");
    snoozeUntilLiEl.append(snoozeUntilHrefEl);
    jQuerySnoozeUntilLiEl = $(snoozeUntilLiEl);

    jQuerySnoozeUntilLiEl.click(snoozeAlarm);
    silenceBtn.append(jQuerySnoozeUntilLiEl);
