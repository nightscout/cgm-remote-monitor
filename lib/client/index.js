...
client.init = function init(serverSettings, plugins) {
  ...
  client.hashauth.init(client, $).initAuthentication(function afterAuth(isAuthenticated) {
    $('#editbutton').toggle(client.hashauth.isAuthenticated());
  });
...
// clears the current user brush and resets to the current real time data
  ...
  function brushed ( ) {
    ...
    function updatePlugins (sgvs, time) {
      ...
      client.sbx = sandbox.clientInit(
        ...
        , tempbasalTreatments: client.tempbasalTreatments
        , profileTreatments: client.profileTreatments
  ...
  function dataUpdate (d) {
    ...
    client.treatments = mergeTreatmentUpdate(d.delta, client.treatments, d.treatments);

    // filter & prepare 'Site Change' events
    client.sitechangeTreatments = client.treatments.filter( function filterSensor(t) {
      return t.eventType.indexOf('Site Change') > -1;
    }).sort(function (a,b) { return a.mills > b.mills; });

    // filter & prepare 'Insulin Change' events
    client.insulinchangeTreatments = client.treatments.filter( function filterInsulin(t) {
      return t.eventType.indexOf('Insulin Change') > -1;
    }).sort(function (a,b) { return a.mills > b.mills; });

    // filter & prepare 'Sensor' events
    client.sensorTreatments = client.treatments.filter( function filterSensor(t) {
      return t.eventType.indexOf('Sensor') > -1;
    }).sort(function (a,b) { return a.mills > b.mills; });

    // filter & prepare 'Profile Switch' events
    client.profileTreatments = client.treatments.filter( function filterProfiles(t) {
    ...
    if (d.devicestatus) {
      if (client.settings.extendedSettings.devicestatus && client.settings.extendedSettings.devicestatus.advanced) {
        ...
        devicestatusData = mergeDataUpdate(d.delta, devicestatusData, d.devicestatus);
      } else {
