...
client.init = function init(plugins, callback) {
  ...
  $.ajax({
    ...
    client.load(serverSettings, plugins, callback);
  }).fail(function fail(err) {
    console.error('server settings were not loaded, will not call init', err);
  });

};

client.load = function load(serverSettings, plugins, callback) {
  ...
  client.hashauth.init(client, $).initAuthentication(client.afterAuth);
