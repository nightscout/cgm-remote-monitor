...
client.init = function init(plugins, serverSettings) {

  console.log('>>>>doing client init');
  var translate = language.translate;

  var browserSettings = getBrowserSettings();
  client.browserSettings = browserSettings;
  ...
  language.set(serverSettings.defaults.language).DOMtranslate();
...
// clears the current user brush and resets to the current real time data
  ...
  function brushed(skipTimer) {
    ...
    function updatePlugins (sgvs, time) {
      ...
      sbx = sandbox.clientInit(serverSettings
        , browserSettings
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  plugins.init(serverSettings);
  sbx = sandbox.clientInit(serverSettings, browserSettings, Date.now());
  $('.container').toggleClass('has-minor-pills', plugins.hasShownType('pill-minor', browserSettings));
  ...
  function getBrowserSettings() {
    ...
    try {
      ...
      $('#alarm-urgenthigh-browser').prop('checked', json.alarmUrgentHigh).next().text(translate('Urgent High Alarm') + appendThresholdValue(serverSettings.thresholds.bg_high));
      $('#alarm-high-browser').prop('checked', json.alarmHigh).next().text(translate('High Alarm') + appendThresholdValue(serverSettings.thresholds.bg_target_top));
      $('#alarm-low-browser').prop('checked', json.alarmLow).next().text(translate('Low Alarm') + appendThresholdValue(serverSettings.thresholds.bg_target_bottom));
      $('#alarm-urgentlow-browser').prop('checked', json.alarmUrgentLow).next().text(translate('Urgent Low Alarm') + appendThresholdValue(serverSettings.thresholds.bg_low));
      ...
      json.showPlugins = setDefault(json.showPlugins, serverSettings.defaults.showPlugins || plugins.enabledPluginNames());
