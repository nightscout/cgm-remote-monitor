...
client.load = function load (serverSettings, callback) {
  ...
  var FORMAT_TIME_12 = '%-I:%M %p'
    ...
    , FORMAT_TIME_12_COMPACT_SECONDS = '%-I:%M:%S'
    , FORMAT_TIME_24_SECONDS = '%H:%M:%S'
    , FORMAT_TIME_12_SCALE_SECONDS = '%-I:%S %p'
    , FORMAT_TIME_24_SCALE_SECONDS = '%H:%S';
  ...
  function getTimeFormat (isForScale, compact) {
    const { timeFormat, showSeconds } = client.settings;

    if (timeFormat === 24 && showSeconds) {
      return isForScale ? FORMAT_TIME_24_SCALE_SECONDS : FORMAT_TIME_24_SECONDS;
    }
    if (timeFormat === 24 && !showSeconds) {
      return isForScale ? FORMAT_TIME_24_SCALE : FORMAT_TIME_24;
    }
    if (timeFormat === 12 && showSeconds) {
      return isForScale ?
        FORMAT_TIME_12_SCALE_SECONDS :
        (compact ? FORMAT_TIME_12_COMPACT_SECONDS : FORMAT_TIME_12_SECONDS);
    }
    return isForScale ?
  ...
  function updateClock () {
    ...
    if (client.settings.showSeconds) {
      ...
      // Update at least every 15 seconds when not showing seconds
