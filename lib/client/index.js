...
client.init = function init(serverSettings, plugins) {
  ...
  client.bottomOfPills = function bottomOfPills ( ) {
    return minorPills.offset().top + minorPills.height();
  };
...
// clears the current user brush and resets to the current real time data
  ...
  function brushed ( ) {
    ...
    function updatePlugins (sgvs, time) {
      ...
      var preBottomOfPills = client.bottomOfPills();
      ...
      if (client.bottomOfPills() != preBottomOfPills) {
        chart.update(false);
      }

      var chartContainer = $('#chartContainer');

      console.info('>>>>updatePlugins client.bottomOfPills()', client.bottomOfPills());
      chartContainer.css('top', (client.bottomOfPills() + 10) + 'px');
      chartContainer.find('svg').css('height', 'calc(100vh - ' + (client.bottomOfPills() + 10)+ 'px)');
