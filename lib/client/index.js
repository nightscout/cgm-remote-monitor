...
client.loadLanguage = function loadLanguage (serverSettings, callback) {
  ...
  if (client.settings.language == 'en') {
    $('#centerMessagePanel').hide();
    client.load(serverSettings, callback);
  } else {
    $.ajax({
      method: 'GET'
      , url: '/translations/' + client.settings.language + '.json'
      ...
      $('#loadingMessageText').html('Loading localizations failed, attempting again in 5 seconds');
      window.setTimeout(window.Nightscout.client.init(), 5000);
      return;
    });
  }
