...
client.load = function load (serverSettings, callback) {
  ...
  */
  ...
  client.exportData = function exportData(type) {
    var translate = client.translate;

    // Check if user is authenticated
    if (!client.hashauth.isAuthenticated()) {
      alert(translate('You must be authenticated to export data'));
      client.hashauth.requestAuthentication();
      return;
    }

    // Build the API URL based on the type
    var apiUrl = '/api/v1/' + type;
    
    // Add query parameters to get more records
    var queryParams = '?count=10000';
    ...
    $.ajax({
      ...
      success: function(csvData) {
        ...
        console.log('Export successful for ' + type);
      },
      error: function(jqXHR, textStatus, errorThrown) {
        console.error('Export failed:', textStatus, errorThrown);
        if (jqXHR.status === 401 || jqXHR.status === 403) {
          alert(translate('Authentication required. Please authenticate and try again.'));
          client.hashauth.requestAuthentication();
        } else {
          alert(translate('Export failed. Please try again.'));
        }
      }
    });
  };
