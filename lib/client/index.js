...
client.load = function load (serverSettings, callback) {
  ...
  function playAlarm (audio) {
    ...
    var isMuted = Storages.localStorage.get('isMuted') === 'true' || client.browserUtils.queryParms().mute === 'true';
    if (!isMuted) {
      audio.play();
    } else {
      client.browserUtils.showNotification('Alarm was muted');
    }
  }
  ...
  function updateMuteButtonIcon() {
    var isMuted = Storages.localStorage.get('isMuted');
    var muteButtonSrc = isMuted ? "/images/volume-mute.svg" : "/images/volume-on.svg";

    $('#mute-button').attr("src", muteButtonSrc);
  }

  $('#mute-button').click(function(event) {
    var isMuted = Storages.localStorage.get('isMuted');

    Storages.localStorage.set('isMuted', !isMuted);

    var notificationText = isMuted ? 'Sound is un-muted' : 'Sound is muted';
    client.browserUtils.showNotification(notificationText, 'success');
  ...
  $('#test-alarm-sound-button').click(function(event) {
