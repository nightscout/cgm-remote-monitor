...
client.init = function init (callback) {
  ...
  $.ajax({
    ...
    if (serverSettings.runtimeState !== 'loaded') {
      ...
      window.setTimeout(window.Nightscout.client.init.bind(window.Nightscout.client, callback), 5000);
      return;
    }
    ...
    if (client.settingsFailed) {
      ...
      client.hashauth.requestAuthentication(function afterRequest () {
        window.setTimeout(client.init.bind(client, callback), 5000);
      });
    }
  });
...
client.load = function load (serverSettings, callback) {
  ...
  client.updateAdminMenu = function updateAdminMenu(isAdmin) {
    ...
    $('.needsadminaccess').toggle(isAdmin);
}
