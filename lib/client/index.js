...
client.init = function init(serverSettings, plugins) {
  ...
  var dateFn = client.dateFn = function (d) { return new Date(d.mills) };
...
// clears the current user brush and resets to the current real time data
  ...
  function inRetroMode() {
    ...
    return !alarmingNow() && time + times.mins(15).msecs < client.now;
  }
  ...
  function brushed ( ) {
    ...
    var nowDate = new Date(brushExtent[1]);
    ...
    if (inRetroMode()) {
      $('#currentTime')
        .text(formatTime(brushExtent[1], true))
    ...
    if (focusPoint && brushExtent[1].getTime() - focusPoint.mills < times.mins(15).msecs) {
