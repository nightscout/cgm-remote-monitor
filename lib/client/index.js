...
client.load = function load (serverSettings, callback) {
  ...
  client.socket = socket = (typeof io !== 'undefined') ? io.connect() : {on: function () {}, emit: function () {}, fake: true};

  function startPolling() {
    console.log('Start polling every minute');
    client.pollingId = new Date().getTime() + Math.round(Math.random() * 1000);
    client.pollingTimer = setInterval(doPolling, 60000);
    doPolling();
  }
  ...
  function doPolling() {
    var token = client.browserUtils.queryParms().token;
    var secret = client.hashauth.apisecrethash || Storages.localStorage.get('apisecrethash');
    var src = '/api/v1/events.json?t=' + new Date().getTime() + '&id=' + client.pollingId + '&history=' + history;
    ...
    $.ajax({
      ...
      , url: src
      , headers: client.headers()
    }).done(function success(events) {
      if (callback) {
        callback();
        callback = null;
      }
      // add indication about slow update
      var text = $('.customTitle').html();
      if (text && text.indexOf(client.translate('slow update')) === -1) {
        setTimeout(function () {
          var $customTitle = $('.customTitle');
          $customTitle.html($customTitle.html() + ' <span style="color: red">(' + client.translate('slow update') + '!)</span>');
        }, 500);
      }
      ...
      events && events.length && events.forEach(function (event) {
        ...
        switch (event.type) {
          case 'dataUpdate':
            dataUpdate(event.data);
            break;
          case 'retroUpdate':
            (function retroUpdate(retroData) {
      ...
      if (!jqXHR.readyState) {
        ...
        client.pollingTimer = null;
        window.setTimeout(window.Nightscout.client.init(), 2000);
      } else {
        console.warn('Cannot poll nightscout server');
      }
    });
  }

  client.socket.fake && startPolling();
  // try fallback on polling
