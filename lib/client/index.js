...
client.load = function load (serverSettings, callback) {
  ...
  var FORMAT_TIME_12 = '%-I:%M %p'
    ...
    , FORMAT_TIME_24 = '%H:%M%'
    , FORMAT_TIME_12_SECONDS = '%-I:%M:%S %p'
    , FORMAT_TIME_24_SECONDS = '%H:%M:%S'
  ...
  client.ctx = {
    ...
    , settings: client.settings
    , pluginBase: client.plugins.base(majorPills, minorPills, statusPills, bgStatus, client.tooltip, Storages.localStorage, client.translate)
    , moment: moment
    , timezones: timezones
  };
  ...
  function getTimeFormat (isForScale, compact) {
    var showSeconds = !isForScale && client.settings.showSeconds;
    var timeFormat = FORMAT_TIME_12;
    if (client.settings.timeFormat === 24) {
      timeFormat = isForScale ? FORMAT_TIME_24_SCALE : (showSeconds ? FORMAT_TIME_24_SECONDS : FORMAT_TIME_24);
    } else {
      timeFormat = isForScale ? FORMAT_TIME_12_SCALE : (showSeconds ? FORMAT_TIME_12_SECONDS : (compact ? FORMAT_TIME_12_COMPACT : FORMAT_TIME_12));
    }
  ...
  function updateClock () {
    updateClockDisplay();
    var interval;
    if (client.settings.showSeconds) {
      // Tick every second so the seconds display stays current
      interval = 1000;
    } else {
      // Update at least every 15 seconds
      interval = Math.min(15 * 1000, (60 - (new Date()).getSeconds()) * 1000 + 5);
    }
  ...
  // An alarm can reach a page that holds no reading: an opt-in device alert
  // (pump, loop, OpenAPS, site or sensor age) fires while no CGM value is in
  // the loaded window, or the page may not read data at all. Neither of these
  // may throw in the handlers below. They do not change whether the alarm
  // sounds - that is still isAlarmForHigh / isAlarmForLow - only what the
  // "disabled locally" log line reads and whether there is a chart to redraw,
  // since the chart is created by the first data update and a page that has
  // had none has no chart.
  function latestMgdlForLog () {
    return client.latestSGV ? client.latestSGV.mgdl : 'no reading loaded';
  }

  function updateChartAfterAlarm () {
    if (chart) {
      chart.update(false);
    }
  }
  ...
  alarmSocket.on('alarm', function(notify) {
    ...
    if (enabled) {
      ...
      console.info('alarm was disabled locally', latestMgdlForLog(), client.settings);
    }
    updateChartAfterAlarm();
  });
  ...
  alarmSocket.on('urgent_alarm', function(notify) {
    ...
    if (enabled) {
      ...
      console.info('urgent alarm was disabled locally', latestMgdlForLog(), client.settings);
    }
    updateChartAfterAlarm();
  });
