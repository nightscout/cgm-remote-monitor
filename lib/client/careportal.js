...
function init (client, plugins, $) {
  ...
  _.forEach(careportal.allEventTypes, function each (event) {
    inputMatrix[event.val] = _.pick(event, ['bg', 'insulin', 'carbs', 'prebolus', 'duration', 'percent', 'absolute', 'profile', 'split']);
  });
   ...
   * @param data {object}  the data to be altered
   */
  function handleSupportForTempTo0 (data) {
