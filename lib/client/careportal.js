...
function init (client, $) {
  ...
  function refreshEventTypes() {
    ...
    careportal.allEventTypes.forEach(function each (event) {
      inputMatrix[event.val] = pick(event, ['otp','remoteCarbs', 'remoteAbsorption', 'remoteBolus', 'bg', 'insulin', 'carbs', 'protein', 'fat', 'prebolus', 'duration', 'percent', 'absolute', 'profile', 'split', 'sensor', 'reasons', 'targets']);
  ...
  careportal.reasonable = function reasonable () {
    ...
    var selected = $('#reason').val();    var reason = reasons.find(function matches (r) {
  ...
  function gatherData () {
    ...
    if (Object.prototype.hasOwnProperty.call(inputMatrix, eventType)) {
      /* eslint-disable-next-line security/detect-object-injection */ // verified false positive
      reasons = inputMatrix[eventType]['reasons'];
    }    var reason = reasons.find(function matches (r) {
