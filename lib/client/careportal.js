...
function init (client, $) {
  ...
  function refreshEventTypes() {
    ...
    _.forEach(careportal.allEventTypes, function each (event) {
      inputMatrix[event.val] = _.pick(event, ['remoteCarbs', 'remoteAbsorption', 'remoteBolus', 'bg', 'insulin', 'carbs', 'protein', 'fat', 'prebolus', 'duration', 'percent', 'absolute', 'profile', 'split', 'reasons', 'targets']);
  ...
  function buildConfirmText (data) {
    ...
    pushIf(data.remoteAbsorption, translate('Remote Absorption') + ': ' + data.remoteAbsortion);
