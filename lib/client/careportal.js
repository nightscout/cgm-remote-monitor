...
function init (client, $) {
  ...
  function refreshEventTypes() {
    ...
    _.forEach(careportal.allEventTypes, function each (event) {
      inputMatrix[event.val] = _.pick(event, ['otp','remoteCarbs', 'remoteAbsorption', 'remoteBolus', 'bg', 'insulin', 'carbs', 'protein', 'fat', 'prebolus', 'duration', 'percent', 'absolute', 'profile', 'split', 'reasons', 'targets', 'nutrition']);
  ...
  careportal.filterInputs = function filterInputs (event) {
    ...
    var nutrition = inputMatrix[eventType]['nutrition'];
    $('#nutrition').css('display',displayType(nutrition));
