...
function graphInfo () {
  ...
  //console.log('graphMills = ' + graphMills);
  //console.log('now-graphMills = ' + (Date.now() - graphMills));
  for (let i in accumSGV) {
    let mills = accumSGV[i][0];
    let bg = accumSGV[i][1];
    let xPct = 100 * (mills - (Date.now() - graphMills)) / graphMills;
    let yPct = 100 * bg / maxMGDL;
    //console.log('mills = ' + mills + ', diff = ' + (mills - (Date.now() - graphMills)) + ' xPct = ' + xPct);
    if (yPct > 100) {
      yPct = 100;
    }
    if (yPct < 0) {
