...
client.render = function render () {
  ...
  $('.dt').text(toTextContent(deltaDisplayValue));
...
// Redraw the last reading received before fetching again, so its age, the
// stale state and the face keep moving while fetches fail or go unanswered.
function refresh () {
  if (latestProperties.bgnow && latestProperties.bgnow.sgvs) {
    client.render();
  }
...
client.init = function init () {
  ...
  client.query();
  setInterval(refresh, 20 * 1000); // update every 20 seconds
