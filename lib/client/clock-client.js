...
client.render = function render () {
	...
	for (let param in faceParams) {
		if (param === '0') {
			...
			/* eslint-disable-next-line security/detect-object-injection */ // verified false positive
			let faceParam = faceParams[param];
			let div = ('<div class="' + faceParam.substr(0, 2) + '"' + ((faceParam.substr(2, 2) - 0 > 0) ? (' style="' + ((faceParam.substr(0, 2) === 'ar') ? 'height' : 'font-size') + ':' + faceParam.substr(2, 2) + 'vmin' + ((faceParam.substr(0, 2) === 'ar') ? ';display:flex;width:' + faceParam.substr(2, 2) + 'vmin";' : '"')) : '') + '></div>');
			$inner.append(div);
		}
	}
