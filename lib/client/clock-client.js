...
client.render = function render () {
  ...
  let displayValue = rec.mgdl
  if (currentUnits == "mmol") {
    displayValue = parseFloat(displayValue / consts.MMOL_TO_MGDL).toFixed(1);
  }
...
function handleUnitToggle () {
  $('.unit-option').removeClass('active');
  if (currentUnits === 'mmol') {
    $('#mmol-clock').addClass('active');
  } else {
    $('#mgdl-clock').addClass('active');
  }
}

function changeUnits (newUnit) {
  if (currentUnits !== newUnit) {
    currentUnits = newUnit;
    storage.set('units', newUnit);
    handleUnitToggle();
    client.render();
  }
}
...
client.init = function init () {
  ...
  currentUnits = client.settings.units;

  $('#mmol-clock').on('click', function() {
    changeUnits('mmol');
  });

  $('#mgdl-clock').on('click', function() {
    changeUnits('mgdl');
  });
