...
client.render = function render () {
  ...
  if (bgColor || sgColor) {
    ...
    if (bgNum < bgLow) {
      $(coloredElement).css('background-color', red);
    }
    if ((bgLow <= bgNum) && (bgNum < bgTargetBottom)) {
      $(coloredElement).css('background-color', blue);
    }
    if ((bgTargetBottom <= bgNum) && (bgNum < bgTargetTop)) {
      $(coloredElement).css('background-color', green);
    }
    if ((bgTargetTop <= bgNum) && (bgNum < bgHigh)) {
      $(coloredElement).css('background-color', yellow);
    }
    if (bgNum >= bgHigh) {
      $(coloredElement).css('background-color', red);
    }
