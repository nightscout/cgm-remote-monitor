...
client.render = function render () {
  ...
  let pluginDisplayValue = clientUtils.pluginInfo(latestProperties.iob, latestProperties.basal, latestProperties.dbsize,
                                                  latestProperties.cage, latestProperties.sage, latestProperties.iage,
                                                  latestProperties.bage, latestProperties.upbat, latestProperties.pump,
                                                  latestProperties.openaps, latestProperties.sensorState);
  let weatherDisplayValue = clientUtils.weatherInfo();

  // process buckets for graph:
  let graphHTML = clientUtils.graphInfo(latestProperties.buckets);
  let graphHTMLPre = graphHTML[0];
  let graphHTMLPost = graphHTML[1];
