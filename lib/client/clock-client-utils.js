  ...
  if (buckets && graphHours > 0) {
    ...
    for (let i in accumSGV) {
      ...
      if ((xPct >= 0) && (xPct <= 100)) {
        ...
        if ((bg * (1 - chartFactor)) < minMGDL) {
          minMGDL = bg * (1 - chartFactor);
        }
      }
    }
    for (let i in accumSGV) {
    ...
    graphHTML += '<span style="position: absolute; bottom: 100%; right: 0%; color: dimgrey; font-size: 3vmin; line-height: normal;">' + maxBGText + '</span>';
    graphHTML += '<span style="position: absolute; top: 100%; right: 0%; color: dimgrey; font-size: 3vmin; line-height: normal;">' + minBGText + '</span>';
    let d = new Date();
    d.setHours(d.getHours() - graphHours);
    graphHTML += '<span style="position: absolute; top: 100%; left: 0%; color: dimgrey; font-size: 3vmin; line-height: normal;">' + formatTimeHHMMA(d) + '</span>';
