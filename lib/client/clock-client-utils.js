...
clientUtils.pluginInfo = function pluginInfo(iob, cob, basal, dbsize, cage, sage, iage, bage, upbat, sensorState,
  ...
  if (sensorState && sensorState.lastStateStringShort) {
    ...
    if (sensorState.lastTxActivation !== undefined) {
      pluginDisplayValue += " ta:" + Math.floor((Date.now() - sensorState.lastTxActivation) / 1000.0 / 60.0 / 60.0 / 24.0);
    }
  }
...
clientUtils.processTreatments = function processTreatments(data) {
  ...
  for (let i = data.length - 1; i >= 0; i--) {
    ...
    if (event && (event == 'Announcement') || (event == 'Profile Switch') || (event == 'Exercise') || (event == 'Temporary Target')) {
      treatmentData[event] = data[i];
    } else if (event.includes('Bolus')) {
      treatmentData['Bolus'] = data[i];
    }
    if (render) {
      window.Nightscout.client.render();
    }
  ...
  if ($('.gt').css('font-size')) {
    ...
    if (treatmentData['Announcement']) {
      ...
      if (elapsedms < graphTime) {
        value += '<tr><td>Annc:</td><td>' + treatmentData['Announcement'].notes.substring(0, 25) + '</td><td>' + Math.floor(elapsedms / 1000 / 60) + ' min ago</td></tr>';
      }
    }
    ...
    if (treatmentData['Exercise']) {
      ...
      if (elapsedms < graphTime) {
        value += '<tr><td>Exer:</td><td>' + treatmentData['Exercise'].notes.substring(0, 25);
    ...
    if (treatmentData['Temporary Target']) {
      let elapsedms = now - Date.parse(treatmentData['Temporary Target'].created_at);
      let durationms = treatmentData['Temporary Target'].duration * 60 * 1000;
      if ((elapsedms < graphTime) && ((durationms - elapsedms) > 0)) {
        if (treatmentData['Temporary Target'].duration > 0) {
          value += '<tr><td>TTrg:</td><td>' + treatmentData['Temporary Target'].targetTop + '/' + treatmentData['Temporary Target'].targetBottom;
          value += ', ' + treatmentData['Temporary Target'].duration + ' min';
          value += '</td><td>' + Math.floor(elapsedms / 1000 / 60) + ' min ago</td></tr>';
        }
      }
    }
    if (treatmentData['Bolus']) {
      let elapsedms = now - Date.parse(treatmentData['Bolus'].created_at);
      if (elapsedms < graphTime) {
        if (treatmentData['Bolus'].insulin > 0) {
          value += '<tr><td>Bls:</td><td>' + treatmentData['Bolus'].insulin;
          value += '</td><td>' + Math.floor(elapsedms / 1000 / 60) + ' min ago</td></tr>';
        }
      }
    }
...
clientUtils.graphInfo = function graphInfo (buckets) {
  ...
  if (buckets && graphHours > 0) {
    ...
    let units = 'px';
    let graphHTML = '<div id="graph" style="width: ' + graphWidth + units + '; height: ' + graphHeight + units + '; border: 1px solid grey; position: relative; background-color: black; color: grey">';
