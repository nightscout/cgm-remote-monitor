...
function init (client, d3, $) {
  ...
  chart.setBasalMode = function setBasalMode (mode) {
    renderer.addBasals(client, mode);
  };
...
//          .call(chart.yAxisBasals);
  ...
  chart.scroll = function scroll (nowDate) {
    ...
    //TODO: find a better way to get the set mode
    var mode = $.localStorage.get('basal-chart-mode');
    mode = mode === undefined ? 'none' : mode;
    renderer.addBasals(client, mode);
