...
function init (client, d3, $) {
  ...
  var focusYDomain = [utils.scaleMgdl(focusMin), utils.scaleMgdl(focusMax)];
  var contextYDomain = [utils.scaleMgdl(contextMin), utils.scaleMgdl(contextMax)];
  ...
  chart.futureOpacity = d3.scaleLinear( )
  ...
  chart.inRetroMode = function inRetroMode() {
    ...
    var brushedRange = chart.theBrush && d3.brushSelection(chart.theBrush.node()) || null;
    var maxTime;
    var brushTime = maxTime = chart.xScale2.domain()[1].getTime();

    if (brushedRange) {
      brushTime = chart.xScale2.invert(brushedRange[1]).getTime();
    }
  ...
  chart.update = function update(init) {
    ...
    var currentBrushExtent = chart.createAdjustedRange();
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      if (init) {
        ...
        chart.context.select('.x.brush').select('.background').style('cursor', 'move');
        chart.context.select('.x.brush').select('.resize.e').style('cursor', 'move');
        chart.context.select('.x.brush').select('.resize.w').style('cursor', 'move');

        // add a line that marks the current time
        chart.focus.append('line')
          .attr('class', 'now-line')
          .attr('x1', chart.xScale(new Date(client.now)))
          .attr('y1', chart.yScale(utils.scaleMgdl(focusMin)))
          .attr('x2', chart.xScale(new Date(client.now)))
          .attr('y2', chart.yScale(utils.scaleMgdl(focusMax)))
        ...
        chart.context.append('line')
          .attr('class', 'now-line')
          .attr('x1', chart.xScale(new Date(client.now)))
          .attr('y1', chart.yScale2(utils.scaleMgdl(contextMin)))
          .attr('x2', chart.xScale(new Date(client.now)))
          .attr('y2', chart.yScale2(utils.scaleMgdl(contextMax)))
          .style('stroke-dasharray', ('3, 3'))
          .attr('stroke', 'grey');
        ...
        chart.theBrush.transition().call(chart.brush.move, currentBrushExtent.map(chart.xScale2));
        ...
        chart.context.select('.open-top')
          ...
          .attr('y1', chart.yScale2(utils.scaleMgdl(contextMax)))
  ...
  chart.scroll = function scroll (nowDate) {
    ...
    chart.xScale.domain(currentBrushExtent);
    chart.yScale.domain(dynamicDomainOrElse(focusYDomain));
    chart.xScaleBasals.domain(currentBrushExtent);
    ...
    chart.context.select('.open-top')
      .attr('x1', chart.xScale2(currentBrushExtent[0]))
      .attr('y1', chart.yScale2(utils.scaleMgdl(contextMax)))
      .attr('x2', chart.xScale2(new Date(currentBrushExtent[1].getTime() + client.forecastTime)))
      .attr('y2', chart.yScale2(utils.scaleMgdl(contextMax)));
    ...
    // transition open-right line to correct location
    chart.context.select('.open-right')
      .attr('x1', chart.xScale2(new Date(currentBrushExtent[1].getTime() + client.forecastTime)))
      .attr('y1', chart.focusHeight)
      .attr('x2', chart.xScale2(new Date(currentBrushExtent[1].getTime() + client.forecastTime)))
      .attr('y2', chart.prevChartHeight);
