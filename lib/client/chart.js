...
function init (client, d3, $) {
  ...
  var xScaleBasals = chart.xScaleBasals = d3.time.scale().domain(extent);

  var yScaleBasals = chart.yScaleBasals = d3.scale.linear()
    .domain([0, 5]);
...
//    .attr('class', 'y axis');
  ...
  chart.update = _.debounce(function debouncedUpdateChart(init) {
    ...
    var basalsHeight = chart.basalsHeight = focusHeight / 4;
...
//          .call(chart.yAxisBasals);
        ...
        var basalsTransition = chart.basals.transition();
