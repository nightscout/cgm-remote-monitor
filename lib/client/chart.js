...
function init (client, d3, $) {
  ...
  chart.update = _.debounce(function debouncedUpdateChart(init) {
    ...
    var currentBrushExtent = chart.brush.extent().slice();
    currentBrushExtent[1] = new Date(currentBrushExtent[1].getTime() + client.forecastTime);
  ...
  chart.scroll = function scroll (nowDate) {
    ...
    range[1] = new Date(range[1].getTime() + client.forecastTime);
    chart.xScale.domain(range);
