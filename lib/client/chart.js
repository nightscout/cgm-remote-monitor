...
function init (client, d3, $) {
  ...
  chart.createAdjustedRange = function () {
    ...
    var range = client.dataExtent();

    if (brushedRange) {
      var rangeWidth=range[1]-range[0]

      range[0] = brushedRange[0] * rangeWidth + range[0]
      range[1] = brushedRange[1] * rangeWidth + range[0]
    }

    var test = brushedRange && chart.xScale2.invert(brushedRange[1]);
  ...
  chart.update = function update(init) {
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      if (init) {
        ...
        chart.theBrush.attr('class', 'x brush')
          .call(chart.brush);
...
//          .call(chart.yAxisBasals);
        ...
        // console.log('Clearing current brushes in chart update');
        // chart.context.select('.x.brush').call(chart.brush.clear());
