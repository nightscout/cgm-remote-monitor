  ...
  chart.update = function update(init) {
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      if (init) {
        ...
        var focusTransition = chart.focus.transition().duration(10);
        ...
        var contextTransition = chart.context.transition().duration(10);
        ...
        chart.basals.transition().duration(10);
    ...
    // Refresh currentBrushExtent to catch when we are not in Retro mode
    // and an update comes in during execution of the code above that could
    // drive us into retro mode.
    currentBrushExtent = chart.createBrushedRange();
