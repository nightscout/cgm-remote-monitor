...
function init (client, d3, $) {
  ...
  function beforeBrushStarted (event) {
    ...
    var cx = d3.pointer(event.touches ? event.touches[0] : event, this)[0];
    var x0 = cx - dx / 2;
    var x1 = cx + dx / 2;
  ...
  function focusRangeConfigured () {
    var low = client.settings.focusRangeLow;
    var high = client.settings.focusRangeHigh;
    return low > 0 && high > 0 && low < high;
  }

  function focusRangeDomain () {
    var mult = 1.15;
    var low = client.settings.focusRangeLow;
    var high = client.settings.focusRangeHigh;
    // Soft range is measured against the data in the visible focus window only,
    // so an excursion elsewhere in the loaded history does not widen the axis.
    var win = chart.xScale.domain();
    var from = win[0].getTime();
    var to = win[1].getTime();
    function windowSgv (d) {
      if (d.type !== 'sgv' || d.mills < from || d.mills > to) { return undefined; }
      return d.mgdl;
    }
    var dataMin = d3.min(client.entries, windowSgv);
    var dataMax = d3.max(client.entries, windowSgv);
    var domainLow = dataMin === undefined ? low : Math.min(low, dataMin);
    var domainHigh = dataMax === undefined ? high : Math.max(high, dataMax * mult);
    return [utils.scaleMgdl(domainLow), utils.scaleMgdl(domainHigh)];
  }

  // define the parts of the axis that aren't dependent on width or height
  var xScale = chart.xScale = d3.scaleTime().domain(extent);

  if (focusRangeConfigured()) {
    focusYDomain = focusRangeDomain();
  } else {
  ...
  chart.getForecastData = function getForecastData () {
    ...
    return points.filter(function isShown (point) {
  ...
  chart.setForecastTime = function setForecastTime () {
    ...
    if (client.sbx.pluginBase.forecastPoints) {
      ...
      if (shownForecastPoints.length > 0) {
        availForecastMills = Math.max(...shownForecastPoints.map(function(point) { return point.mills }));
      }
