...
function init (client, d3, $) {
  ...
  function yDomain() {
    var mult = 1.2;
    if (client.settings.scaleY === 'linear') {
      return [utils.scaleMgdl(30), Math.max(utils.scaleMgdl(d3.max(client.data, function(d) { return d.mgdl; })*mult),utils.scaleMgdl(200*mult))];
    } else {
      return [utils.scaleMgdl(30), utils.scaleMgdl(510)];
    }
  }
