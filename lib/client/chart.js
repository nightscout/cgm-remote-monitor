...
function init (client, d3, $) {
  ...
  chart.update = function update(init) {
    ...
    if ( !chartContainer || chartContainer.length < 1) {
      //console.warn('Unable to find element for #chartContainer'); // there is no #chartContainer on all pages 
      //TODO: webpack splitting so that chart.js is not included in these pages
      return;
    }
