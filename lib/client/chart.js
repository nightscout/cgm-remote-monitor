...
function init (client, d3, $) {
  ...
  chart.brush = d3.brushX()
    // .extent(extent)  // No longer set the extent, let it default to the full extent of the owner
  ...
  chart.createAdjustedRange = function () {
    var brushedRange = d3.brushSelection(chart.context.select('.x.brush'));
  ...
  chart.inRetroMode = function inRetroMode() {
    ...
    var brushedRange = d3.brushSelection(chart.context.select('.x.brush'));
    var brushTime = range[1].getTime();

    if (brushedRange) {
      var rangeWidth=range[1]-range[0]

      brushTime = brushedRange[1] * rangeWidth + range[0]
    }

    var maxTime = range[1].getTime();
  ...
  chart.update = function update(init) {
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      if (init) {
        ...
        chart.context.append('g')
          .attr('class', 'x brush')
          .call(d3.brushX().on('brush', client.brushed))
          .selectAll('rect')
          .attr('y', focusHeight)
...
//          .call(chart.yAxisBasals);
        ...
        // d3.select('.brush').transition().call(chart.brush.extent(currentBrushExtent)); TODO: FixMe
