...
function init () {
  ...
  var valueMappers = {
    ...
    , focusRangeLow: mapNumber
    , focusRangeHigh: mapNumber
  ...
  function enableAndDisableFeatures (accessor, nameType) {
    ...
    // On an mmol site, decide each threshold's unit on its own: a value below 30
    // is mmol/L and is converted, a value of 30 or more is mg/dl and is kept.
    // No alarm threshold is meaningful at or above 30 mmol/L (540 mg/dl) or below
    // 30 mg/dl, so this keeps old installs that have these set in mg/dl, and the
    // mg/dl defaults of any threshold left unset.
    if (settings.units.toLowerCase().includes('mmol')) {
      ['bgHigh', 'bgTargetTop', 'bgTargetBottom', 'bgLow'].forEach(function eachThreshold (key) {
        var given = thresholds[key];
        if (given < 30) {
          thresholds[key] = Math.round(given * constants.MMOL_TO_MGDL);
          console.info('Threshold ' + key + ' ' + given + ' taken as mmol/L, converted to ' + thresholds[key] + ' mg/dl');
        }
      });
    }

    if (settings.units.toLowerCase().includes('mmol') && settings.focusRangeHigh && settings.focusRangeHigh < 50) {
      settings.focusRangeLow = Math.round(settings.focusRangeLow * constants.MMOL_TO_MGDL);
      settings.focusRangeHigh = Math.round(settings.focusRangeHigh * constants.MMOL_TO_MGDL);
    }
  ...
  function verifyFocusRange () {
    settings.focusRangeLow = Number(settings.focusRangeLow) || 0;
    settings.focusRangeHigh = Number(settings.focusRangeHigh) || 0;
    var low = settings.focusRangeLow;
    var high = settings.focusRangeHigh;
    if (!low && !high) { return; }
    if (!low || !high || low >= high) {
      console.warn('FOCUS_RANGE_LOW(' + low + ') / FOCUS_RANGE_HIGH(' + high + ') invalid; focus range disabled');
      settings.focusRangeLow = 0;
      settings.focusRangeHigh = 0;
    }
  }
