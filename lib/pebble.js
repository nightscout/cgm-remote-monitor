...
var cob = require('./plugins/cob')();
var bwp = require('./plugins/boluswizardpreview')();
var bgnow = require('./plugins/bgnow')();
...
function addExtraData (first, req, sbx) {
  ...
  function addDelta() {
    var delta = sbx.properties.bgnow && sbx.properties.bgnow.delta;
  ...
  function addCOB() {
    if (req.cob) {
      var cobResult = cob.cobTotal(data.treatments, data.devicestatus, data.profile, Date.now());
      if (cobResult) {
