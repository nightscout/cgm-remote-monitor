...
function pebble (req, res) {
  ...
  var predict_hr = 3, // 3 hours of predictive data (should be smaller than predict_hr in predictDIYPS and websocket.js)
    SIXTY_MINS_IN_MS = 3600000;

  function get_latest (err, results) {
    var now = Date.now();// - (predict_hr * SIXTY_MINS_IN_MS);
  ...
  req.devicestatus.last(function(err, value) {
      ...
      req.treatments.list(function (err, trs) {
...
function iobTotal(treatments, time) {
  ...
  treatments.forEach(function(treatment) {
    console.info(">>>treatment.created_at", new Date(treatment.created_at).getTime());
    if(new Date(treatment.created_at).getTime() < time) {
      console.info(">>>treatment", treatment);
