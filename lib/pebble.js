...
function addExtraData (first, req, sbx) {
  ...
  function addPumpIOB () {
    var result = iob.getPumpIOB(data.pumpStatuses, sbx.withExtendedSettings(iob).extendedSettings, Date.now());
    if (result) {
      first.iob = result.pebbleDisplay;
    }
  }
...
function configure (env, ctx) {
  function middle (req, res, next) {
    ...
    req.pumpiob = req.iob && _.get(env, 'extendedSettings.iob.source') === 'pump';
