...
function init () {
  ...
  ddata.idMergePreferNew = (oldData, newData) => {
    ...
    // Start with all new data (this ensures we get the newest version of any duplicates)
  ...
  ddata.clone = function clone () {
    return _.clone(ddata, function(value) {
      // MongoDB ObjectIDs have special methods that need to be handled
      // Rather than importing the MongoDB types, we check for the characteristic
      // methods that indicate an ObjectID
