...
function init( ) {
  ...
  ddata.split = function split (time, cutoff) {
    var result = {
      first: { }
      , rest: { }
    };
    ...
    function partition (field) {
      var parts = _.partition(ddata[field], recent);
      result.first[field] = parts[0];
      result.rest[field] = parts[1];
    }

    partition('treatments');
    partition('devicestatus');

    result.first.sgvs = ddata.sgvs;
    result.first.cals = ddata.cals;
    result.first.profiles = ddata.profiles;

    result.rest.mbgs = ddata.mbgs;
