...
ddata.processDurations = function processDurations (treatments) {
  ...
  treatments = _.sortedUniqBy(treatments, 'mills');
  ...
  function cutIfInInterval (base, end) {
    if (base.mills < end.mills && base.mills + times.mins_msecs(base.duration) > end.mills) {
      base.duration = times.msecs_mins(end.mills-base.mills);
    }
  }

  // cut by end events
  _.forEach( treatments, function allTreatments (t) {
    _.forEach( endevents, function allEndevents (e) {
        cutIfInInterval(t, e);
    });
  });
  ...
  for (var i=0; i<treatments.length; i++)  {
    for (var j=i; j<treatments.length; j++) {
      var base = treatments[i];
      var end = treatments[j];
      if (base.mills + times.mins_msecs(base.duration) > end.mills) {
        base.duration = times.msecs_mins(end.mills-base.mills);
      }
    }
  }
