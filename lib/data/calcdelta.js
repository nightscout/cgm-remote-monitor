...
module.exports = function calcDelta (oldData, newData) {
  ...
  function nsArrayTreatments(oldArray, newArray) {
    ...
    for (i = 0; i < l; i++) {
      no = newArray[parseInt(i)];
      no._id = no._id.toString();
      found = false;
      founddiff = false;
      for (j = 0; j < m; j++) {
        oo = oldArray[parseInt(j)];
  ...
  function nsArrayDiff(oldArray, newArray) {
    ...
    for (var i = 0; i < l; i++) {
      seen[oldArray[parseInt(i)].mills] = true;
    }
    var result = [];
    l = newArray.length;
    for (var j = 0; j < l; j++) {
    ...
    for (var array in compressibleArrays) {
      if (compressibleArrays.hasOwnProperty(array)) {
        ...
        if (newData.hasOwnProperty(a)) {
          ...
          var deltaData = (a === 'treatments' ? nsArrayTreatments(oldData[String(a)], newData[String(a)]) : nsArrayDiff(oldData[String(a)], newData[String(a)]));
          if (deltaData.length > 0) {
            //console.log('delta changes found on', a);
            changesFound = true;
            sort(deltaData);
            delta[String(a)] = deltaData;
          }
        }
      }
    }
      ...
      if (skippableObjects.hasOwnProperty(object)) {
        ...
        if (newData.hasOwnProperty(o)) {
          if (JSON.stringify(newData[o]) !== JSON.stringify(oldData[o])) {
            ...
            changesFound = true;
            delta[String(o)] = newData[String(o)];
          }
        }
      }
    }
