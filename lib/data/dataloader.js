...
function loadEntries(ddata, ctx, callback) {
    ...
    const latestEntry = ddata.sgvs && ddata.sgvs.length > 0 ? _.last(ddata.sgvs).mills : ddata.lastUpdated;
...
function loadTreatments(ddata, ctx, callback) {
    ...
    ctx.treatments.list(tq, function(err, results) {
        if (!err && results) {
            ...
            ddata.shadow.treatments = mergeProcessSort(ddata.shadow.treatments, results, ageFilter).reverse();
