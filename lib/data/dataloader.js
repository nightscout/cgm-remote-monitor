...
const findSmallestMills = (data) => {
    if (!data) return;
    let min = data[0].mills;
    for (let i = 0, len = data.length; i < len; i++) {
        let o = data[i];
        min = o.mills < min ? o.mills : min;
    }
    return min;
}
...
function loadEntries(ddata, ctx, callback) {
    ...
    const latestEntry = ddata.sgvs && ddata.sgvs.length > 0 ? findSmallestMills(ddata.sgvs) : ddata.lastUpdated;
...
function loadTreatments(ddata, ctx, callback) {
    ...
    const latestEntry = ddata.treatments && ddata.treatments.length > 0 ? findSmallestMills(ddata.treatments) : ddata.lastUpdated;
    const loadTime = Math.min(ddata.lastUpdated - loadPeriod, latestEntry);

    if (ddata.treatments && ddata.treatments.length > 0) {
        console.log('Treatments LAST', _.last(ddata.treatments).created_at);
        console.log('Treatments HEAD', _.head(ddata.treatments).created_at);
    }
    
    console.log('Treatments loadPeriod', loadPeriod, 'latestEntry', latestEntry, 'loadTime', loadTime);
    ...
    ctx.treatments.list(tq, function(err, results) {
        if (!err && results) {
            ...
            console.log('treatment HEAD status after load', _.head(ddata.treatments).created_at);
...
function loadDeviceStatus(ddata, env, ctx, callback) {
    ...
    const latestEntry = ddata.devicestatus && ddata.devicestatus.length > 0 ? findSmallestMills(ddata.devicestatus) : ddata.lastUpdated;
    const loadTime = Math.min(ddata.lastUpdated - loadPeriod, latestEntry);

    if (ddata.devicestatus && ddata.devicestatus.length > 0) {
      console.log("DeviceStatus LAST", _.last(ddata.devicestatus).created_at);
      console.log("DeviceStatus HEAD", _.head(ddata.devicestatus).created_at);
    }
    ...
    ctx.devicestatus.list(opts, function(err, results) {
        if (!err && results) {
            ...
            console.log('status HEAD status after load', _.head(ddata.devicestatus).created_at);
        } else {
            ddata.devicestatus = [];
        }
        callback();
