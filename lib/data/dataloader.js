...
function mergeToTreatments (ddata, results) {
  ...
  var treatments = _.map(filtered, function update (treatment) {
    ...
    treatment.carbs = Number(treatment.carbs);
    treatment.insulin = Number(treatment.insulin);
    return treatment;
  });
...
function loadSensorAndInsulinTreatments (ddata, ctx, callback) {
  var tq = {
    find: {
      ...
      , created_at: {
        $gte: new Date(ddata.lastUpdated - (ONE_DAY * 21)).toISOString()
      }
    }
    , sort: {created_at: -1}
  };
...
function loadDeviceStatus (ddata, env, ctx, callback) {
  var opts = {
    find: {
      created_at: {
        $gte: new Date(ddata.lastUpdated - ONE_HOUR).toISOString()
      }
    }
    , sort: {created_at: -1}
  };
