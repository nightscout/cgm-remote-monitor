...
function loadDeviceStatus(ddata, env, ctx, callback) {
    var deviceStatusDays = (env.extendedSettings.devicestatus && env.extendedSettings.devicestatus.days)
        ? env.extendedSettings.devicestatus.days
        : 1;
    var dateRange = {
        $gte: new Date(ddata.lastUpdated - (ONE_DAY * deviceStatusDays)).toISOString()
    };
    if (ddata.page && ddata.page.frame) {
        dateRange['$lte'] = new Date(ddata.lastUpdated).toISOString();
    }
