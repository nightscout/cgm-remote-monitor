...
function loadEntries(ddata, ctx, callback) {
    ...
    var dateRange = {
        $gte: ddata.lastUpdated - loadPeriod
    };
    if (withFrame) {
    ...
    ctx.entries.list(q, function(err, results) {
        ...
        if (!err && results) {
            ...
            ddata.shadow.sgvs = mergeProcessSort(ddata.shadow.sgvs, shadowSgvs, ageLimit);
            ddata.shadow.mbgs = mergeProcessSort(ddata.shadow.mbgs, uniqBasedOnMills(shadowMbgs), ageLimit);
            ddata.shadow.cals = mergeProcessSort(ddata.shadow.cals, uniqBasedOnMills(shadowCals), ageLimit);
        }
        callback();
    });
}
...
function mergeToTreatments(ddata, results) {
    ...
    //filter out temps older than 25 hours since we don't display them
...
function loadTreatments(ddata, ctx, callback) {
    ...
    const withFrame = ddata.page && ddata.page.frame;
    const longLoad = constants.TWO_DAYS; //ONE_DAY * 2.5;
...
function loadDeviceStatus(ddata, env, ctx, callback) {
    ...
    ctx.devicestatus.list(opts, function(err, results) {
        if (!err && results) {
            ...
            const ageLimit = ddata.lastUpdated - constants.TWO_DAYS;
            ddata.devicestatus = mergeProcessSort(ddata.devicestatus, r, ageLimit);
        } else {
            ddata.devicestatus = [];
        }
        callback();
