...
function boot (env) {
  ...
  var proc = bootevent( )
    ...
    .acquire(function data (ctx, next) {
      ...
      ctx.food = require('./food')(env.food_collection, ctx.store);
      ...
      store.ensureIndexes(ctx.food( ), ctx.food.indexedFields);

      // load current profile record to memory
	  currentProfile.updateFromStorage(ctx.profiles);
