...
function storage(name, storage, pushover) {
  ...
  function sendPushover(doc) {
    if (doc.type && doc.date && pushover) {
      if (doc.type == 'mbg') {
        ...
        console.info("Not an MBG or SGV: " + JSON.stringify(doc));
      }
    } else {
      console.info("What are you: " + JSON.stringify(doc));
    }
  }
  ...
  function sendSGVPushover(doc) {
    ...
    if (!doc.sgv || doc.type != 'sgv') {
      console.info("Not an SGV: " + JSON.stringify(doc));
      return;
    }
    ...
    if (doc.sgv < 39) {
      if (sinceLastAlert > TIME_10_MINS) {
        priority = 2;
        sound = "siren";
      }
    } else if (doc.sgv < 55) {
      priority = 2;
      sound = "spacealarm";
    } else if (doc.sgv < 70 && sinceLastAlert > TIME_15_MINS) {
      priority = 1;
      sound = "falling";
      ...
      priority = 1;
      sound = "intermission";
    } else if (doc.sgv > 250 && sinceLastAlert > TIME_30_MINS) {
      priority = 1;
      sound = "tugboat";
    } else if (doc.sgv > 300 && sinceLastAlert > TIME_15_MINS) {
      priority = 2;
      sound = "climb";
    }

    if (priority === 0 && readago > TIME_15_MINS && sinceLastAlert > TIME_30_MINS) {
      priority = 1;
      sound = "cosmic";
    }
