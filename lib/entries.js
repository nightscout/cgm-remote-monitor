...
function find_sgv_query (opts) {
  ...
  opts = walk_prop('date', parseInt)(opts);
  opts = walk_prop('sgv', parseInt)(opts);
  return opts;
}
...
function walk_prop (prop, typer) {
  function iter (opts) {
    if (opts && opts.find && opts.find[prop]) {
      Object.keys(opts.find[prop]).forEach(function (key) {
        var is_keyword = /^\$/g;
        if (is_keyword.test(key)) {
          opts.find[prop][key] = typer(opts.find[prop][key]);
        }
      });
    }
    return opts;
