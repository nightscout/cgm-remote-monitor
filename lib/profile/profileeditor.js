...
//  defaultprofile.startDate.setMilliseconds(0);
  ...
  function buildUnsetProfile () {
    var profile = cloneDeep(defaultprofile);
    profile.dia = '';
    ['carbratio', 'sens', 'basal', 'target_low', 'target_high'].forEach(function eachField (field) {
      profile[field].forEach(function eachEntry (entry) {
        entry.value = '';
      });
    });
    return profile;
