...
//  defaultprofile.startDate.setMilliseconds(0);
  ...
  $.ajax('/api/v1/profile.json?count=20', {
    ...
    , success: function (records) {
      ...
      mongorecords = JSON.parse(JSON.stringify(client.profilefunctions.data)); // deep clone
  ...
  function convertToRanges(profile) {
    ...
    if (profile.target_high.length !== profile.target_low.length) {      window.alert(translate('Time ranges of target_low and target_high don\'t  match. Values are restored to defaults.'));
      profile.target_low = JSON.parse(JSON.stringify(defaultprofile.target_low));
      profile.target_high = JSON.parse(JSON.stringify(defaultprofile.target_high));
    }
  }
  ...
  function profileAdd (event) {
    ...
    record.store[newname] = JSON.parse(JSON.stringify(defaultprofile));
  ...
  function profileClone (event) {
    ...
    record.store[newname] = JSON.parse(JSON.stringify(record.store[currentprofile]));
  ...
  // Grab values from html GUI to object  function GUIToObject() {
    var oldProfile = JSON.parse(JSON.stringify(c_profile));
    ...
    // Compare profiles using JSON.stringify for deep equality comparison
