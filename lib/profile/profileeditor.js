...
//  defaultprofile.startDate.setMilliseconds(0);
  ...
  function convertToRanges(profile) {
    ...
    if (profile.target_high.length !== profile.target_low.length) {
      ...
      profile.target_low = cloneDeep(defaultprofile.target_low);
      profile.target_high = cloneDeep(defaultprofile.target_high);
    }
  }
  ...
  function profileAdd (event) {
    ...
    record.store[newname] = cloneDeep(defaultprofile);
  ...
  function profileClone (event) {
    ...
    record.store[newname] = cloneDeep(record.store[currentprofile]);
