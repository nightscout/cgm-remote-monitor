...
module.exports = function createClient(http = request) {
  function call(connection, path, body) {
    const base = 'https://carelink.minimed.' + (connection.region === 'us' ? 'com' : 'eu');
    const url = providerUrl(new URL(path, base).href);
    return http(url.href, { method: body ? 'POST' : 'GET', body: body && JSON.stringify(body),
  ...
  async function account(connection) {
    const [user, profile, requirements] = await Promise.all([
      call(connection, '/patient/users/me'), call(connection, '/patient/users/me/profile'),
      call(connection, '/patient/countries/settings?countryCode=' + connection.country + '&language=en')
    ]);
    ...
    if (isPatient) patients = [{ username: profile.username, label: profile.username }];
      ...
      const links = await call(connection, '/patient/m2m/links/patients');
