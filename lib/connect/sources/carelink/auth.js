...
function createAuth(http = request) {
  ...
  async function discover() {
    if (!discovery || discovery.expires < Date.now()) {
      const data = await http('https://clcloud.minimed.eu/connect/carepartner/v13/discover/android/3.6');
  ...
  async function begin(country) {
    ...
    const config = await http(providerUrl(cp[cp.UseSSOConfiguration || 'Auth0SSOConfiguration']).href);
  ...
  async function tokens(config, params, previous) {
    ...
    const data = await http(config.tokenUrl, { method: 'POST', body,
