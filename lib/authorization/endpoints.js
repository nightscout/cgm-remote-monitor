...
function init (env, authorization) {
  ...
  // A subject or role without a usable name is refused by storage; that is the
  // request's fault, not the database's.
  function sendWriteError (res, err) {
    if (err === authorization.storage.NAME_REQUIRED) {
      res.sendJSONStatus(res, consts.HTTP_BAD_REQUEST, 'Bad Request', err);
    } else {
  ...
  endpoints.get('/subjects', authorization.isPermitted('admin:api:subjects:read'), function getSubjects (req, res) {
    res.json(authorization.storage.subjects.map(function eachSubject (subject) {
      // `notes` is stored and the admin UI has a column and an input for it, but
      // it was not served here, so the edit dialog populated that input from
      // undefined and the next save wrote an empty string back over whatever the
      // operator had written.
      return pick(subject, ['_id', 'name', 'accessToken', 'roles', 'notes']);
    }));
  });
