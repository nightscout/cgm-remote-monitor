...
function init (env, ctx) {
  ...
  authorization.resolve = async function resolve (data, callback) {
    ...
    //
    // No credential means this request is not a guess at one, so it is not
    // throttled. The peer key is shared by everyone behind a proxy, and a
    // request that is not attempting to authenticate must never wait on it.
    if (!authAttempted) {
    ...
    // The wait is held here, on the way out of a failed attempt, rather than on
    // the way in to every request. A request that authenticates is never made
    // to wait for somebody else's failures, which is what makes it safe to
    // throttle on the peer address even when that address is a shared proxy.
