...
function init (env, ctx) {
  ...
  var defaultRoles = require('./defaultroles').parse(env.settings.authDefaultRoles);
  ...
  authorization.resolve = async function resolve (data, callback) {
    ...
    const delayKeys = ipdelaylist.keysFor(data);

    // The wait comes BEFORE the credential is checked, as it always has: a
    // request from an address (or with a credential) that recently failed learns
    // nothing, not even a success, until the delay has passed. Checking first and
    // holding only the failure would answer a correct guess at once, which turns
    // the delay into a signal instead of a cost. The price is that every request
    // from a throttled address waits, including ones that would succeed; with
    // TRUST_PROXY set, that address is the real client, not a shared proxy.
    const requestDelay = shouldDelayRequest(delayKeys);
  ...
  /**
   * The permissions of a caller that presents no credential: the deployment's
   * AUTH_DEFAULT_ROLES, exactly what `resolve` answers when neither a secret
   * nor a token is supplied.
   *
   * Unlike `resolve`, this does not wait on the failed-authentication delay.
   * That delay slows guessing, and a caller with no credential is not
   * guessing: the answer is the same fixed set of default permissions whatever
   * the delay state of its address, so waiting would protect nothing. It
   * neither records a failure nor clears one. Only for callers that have been
   * established to carry no credential at all; anything carrying a value, good
   * or bad, must go through `resolve`.
   */
  authorization.resolveAnonymous = function resolveAnonymous (callback) {
    const result = { shiros: storage.rolesToShiros(defaultRoles), defaults: true };
    if (callback) { callback(null, result); }
    return result;
  };
