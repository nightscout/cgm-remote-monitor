...
function init (env, ctx) {
  ...
  authorization.resolve = async function resolve (data, callback) {
    ...
    async function check (data, callback) {
      ...
      console.log('Resolve call with data', data);

      data.api_secret = data.api_secret || null;

      if (data.api_secret == 'null') {
        data.api_secret = null; // TODO find what's sending this anomaly
      }
      ...
      // secret wasn't correct, try if this was a token
      ...
      // If we got this far, the auth failed
  ...
  authorization.isPermitted = function isPermitted (permission) {
    ...
    function check (req, res, next) {
      ...
      authorization.resolve(data, function resolved (err, permissions) {
        console.log('Resolved inside isPermitted!');
        console.log('permissions', JSON.stringify(permissions));
        console.log('permissionNeeded', permission);
        let success = authorization.checkMultiple(permission, permissions.shiros);
    ...
    function delayedIsPermitted (req, res, next) {
      var remoteIP = getRemoteIP(req);
      const requestDelay = shouldDelayRequest(remoteIP);

      if (requestDelay) {
        console.log('Concurrent or failed requests, delaying request by', requestDelay);
        setTimeout(() => { check(req, res, next); }, requestDelay);
      } else {
        console.log('Checking permissions without delay');
        check(req, res, next);
      }
...
/*    const boundCheck = checkWithDelay.bind({ callback: check });
  ...
  authorization.authorize = function authorize (accessToken) {
    ...
    console.log('Authorize call with token', accessToken);

    var subject = storage.findSubject(accessToken);

    console.log('Authorize found subject', subject);
