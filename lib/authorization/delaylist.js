...
const { trustFor } = require('../server/client-ip');
...
function trustsForwardedHeaders (env) {
  return Boolean(trustFor(env).legacyForwardedHeaders);
}
...
function bootMessage (env) {
  if (trustFor(env).trustsEveryHop) {
    return 'SECURITY: failed-authentication throttling is keyed on the client address, '
      + 'and TRUST_PROXY=true trusts every proxy hop, so that address is the left-most '
      + 'X-Forwarded-For entry. It is only as trustworthy as the outermost proxy: that '
      + 'proxy must replace any X-Forwarded-For header the client sent, not add to it. '
      + 'If it adds to it, the address is whatever the caller sent, and this delay does '
      + 'NOT protect against guessing passwords or tokens; set TRUST_PROXY to the number '
      + 'of proxies in front of Nightscout, or to their IP address(es) or CIDR range(s), '
      + 'instead.';
  }
...
function configuredMessage (env) {
  const hops = trustFor(env).trustedHops;
  if (hops) {
    return 'Failed-authentication throttling is keyed on the client address as '
      + 'resolved through TRUST_PROXY=' + hops + ': the X-Forwarded-For entry added '
      + 'by the proxy ' + hops + (hops === 1 ? ' hop' : ' hops') + ' from Nightscout.';
  }
  return 'Failed-authentication throttling is keyed on the client address as '
