...
function init (env, ctx) {
  ...
  storage.reload = function reload (callback) {
    ...
    storage.listRoles({sort: {name: 1}}, function listResults (err, results) {
      ...
      if (_.isEmpty(_.find(storage.roles, {name: 'admin'}))) {
        storage.roles.push({
          name: 'admin'
          , permissions: ['*']
          , autoGenerated: true
      ...
      if (_.isEmpty(_.find(storage.roles, {name: 'readable'}))) {
        storage.roles.push({
          name: 'readable'
      ...
      if (_.isEmpty(_.find(storage.roles, {name: 'careportal'}))) {
        storage.roles.push({
          name: 'careportal'
          , permissions: ['api:treatments:create', 'alarms:default:ack']
          , autoGenerated: true
        });
      }
