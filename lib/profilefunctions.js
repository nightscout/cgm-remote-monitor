...
var moment2 = require('moment-timezone');
  ...
  profile.getValueByTime = function getValueByTime (time, valueType, spec_profile) {
    ...
    console.log('TIME AFTER OFFSET', time, new Date(time).toISOString());
  ...
  var d = new Date(minuteTime);

  console.log('UTC UTC UTC', moment.utc(minuteTime));
  console.log('IS UTC?', moment.utc(minuteTime).isUTC());

  var _t = moment.utc(minuteTime);
  _t.$x.$timezone = "UTC";
  console.log('UTC UTC UTC _t', moment.utc(minuteTime));

//    var t = profile.getTimezone(spec_profile) ? moment.utc(minuteTime).tz(profile.getTimezone(spec_profile)) : moment.utc(minuteTime);
    var t = profile.getTimezone(spec_profile) ? _t.tz(profile.getTimezone(spec_profile)) : moment.utc(minuteTime);
    var t2 = profile.getTimezone(spec_profile) ? moment2(minuteTime).tz(profile.getTimezone(spec_profile)) : moment2(minuteTime);

    //console.log('guess', moment().tz.guess());

    console.log('New time from minutetime', moment.utc(minuteTime), moment2(minuteTime));

    console.log('t', t, profile.getTimezone(spec_profile) );
    console.log('t2', t2, profile.getTimezone(spec_profile) );
    ...
    console.log('timeAsSecondsFromMidnight', mmtMidnight, timeAsSecondsFromMidnight);
    ...
    console.log('RETURNING', time, valueType, returnValue);
