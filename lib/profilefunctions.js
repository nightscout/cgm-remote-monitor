...
var moment = ctx.moment;
  ...
  profile.loadData = function loadData (profileData) {
    if (profileData && profileData.length) {      profile.data = profile.convertToProfileStore(profileData);
      profile.data?.forEach(function eachProfileRecord (record) {
        record.store?.forEach(profile.preprocessProfileOnLoad);
  ...
  profile.preprocessProfileOnLoad = function preprocessProfileOnLoad (container) {
    Object.values(container || {}).forEach(function eachValue (value) {
