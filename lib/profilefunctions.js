...
function init(profileData) {
  ...
  profile.getValueByTime = function getValueByTime (time, valueType) {
    ...
    var valueContainer = profile.getCurrentProfile(time)[valueType];
  ...
  profile.getCurrentProfile = function getCurrentProfile(time) {
  ...
  profile.updateTreatments = function updateTreatments (profiletreatments, tempbasaltreatments) {
  ...
  profile.getTempBasal = function getTempBasal(time) {

    var cacheKey = 'basal' + time + profile.tempbasaltreatments_hash;
    var returnValue = profile.timeValueCache[cacheKey];
    ...
    var basal = profile.getBasal(time);
    var tempbasal = basal;
  ...
  profile.currentBasalProfile = function currentBasalProfile () {
    return profile.currentProfile;
  };
  
  profile.listBasalProfiles = function listBasalProfiles () {
    var profiles = [ profile.currentProfile ];
    profiles.push('AAAAA');
    profiles.push('BBBBB');
    profiles.push('CCCCC');
    return profiles;
