...
function init(profileData) {
  ...
  profile.getTempBasal = function getTempBasal(time, spec_profile) {
    ...
    var cacheKey = 'basal' + time + profile.tempbasaltreatments_hash + profile.profiletreatments_hash + spec_profile;
    var returnValue = profile.timeValueCache[cacheKey];
    ...
    } else if (treatment && treatment.relative) {
      tempbasal = basal + treatment.relative;
    }
    returnValue = {
      basal: basal
      , treatment: treatment
