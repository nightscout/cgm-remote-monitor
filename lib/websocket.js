...
function update() {
    ...
    //TODO: use async.parallel for these nested list calls
    entries.list(q, function (err, results) {
      ...
      var tq = { find: {"created_at": {"$gte": new Date(earliest_data - (ONE_DAY / 2)).toISOString()}} };
      treatments.list(tq, function (err, results) {
          ...
          profiles.list(function (err, results) {
              ...
              // There should be only one document in the profile collection with a DIA.
              // If there are multiple, use the one with latest start date.
              // If no profile is marked with a date, use the last profile found.
              // TODO: send multiple profiles and use expired profiles for historic treatments
              ...
              results.forEach(function(element, index, array) {
                  if (element) {
                    if (element.dia) {
                    	...
                    	if (!lastDate) profileData[0] = element;
                    }
                  }
              });
...
function loadData() {
    ...
    if (actualLength > 1) {
        ...
        // show predict_hr hours worth of prediction dots (see also predict_hr in client.js)
        var predict_hr = 3;
        var n = Math.ceil(12 * (predict_hr + (now - lastValidReadingTime) / ONE_HOUR));
        var AR = [-0.723, 1.716];
