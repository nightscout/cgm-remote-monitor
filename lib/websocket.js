...
function init (env, ctx, server) {
  ...
  var levels = ctx.notifications.levels;
  ...
  websocket.update = function update ( ) {
    ...
    var lastSGV = ctx.data.sgvs.length > 0 ? ctx.data.sgvs[ctx.data.sgvs.length - 1].mgdl : null;
    if (lastSGV) {
      if (lastData.sgvs) {
  ...
  websocket.emitNotification = function emitNotification (notify) {
    ...
    } else if (notify.level === levels.INFO) {
      //client doesn't know how to display info notifications yet, ignoring
