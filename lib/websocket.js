...
function loadData() {
    ...
    if (actualLength > 1) {
        ...
        for (var i = 0; i <= n; i++) {
            ...
            predictedCI[i * 2] = {
                x: dt + 1200,
                y: Math.max(BG_MIN, Math.min(BG_MAX, Math.round(BG_REF * Math.exp((y[1]-CONE[i])))))};
            predictedCI[i * 2 + 1] = {
                x: dt + 3000,
                y: Math.max(BG_MIN, Math.min(BG_MAX, Math.round(BG_REF * Math.exp((y[1]+CONE[i])))))};
        }
        ...
        var shouldEmit = is_different(actual, predictedCI, mbg, treatment);
        patientData = [actual, predictedCI, mbg, treatment];
