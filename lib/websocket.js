    ...
    entries.list(q, function (err, results) {
      ...
      treatments.list(function (err, results) {
          ...
          profiles.list(function (err, results) {
              // There should be only one document in the profile collection for now.  If there are multiple, use the last one.
              results.forEach(function(element, index, array) {
                  if (element) {
...
function loadData() {
    ...
    if (actualLength > 1) {
        ...
        // show predict_hr hours worth of prediction dots (should be larger than predict_hr in client.js)
        var predict_hr = 4;
        ...
        var shouldEmit = is_different(actual, predicted, mbg, treatment, errorCode, profile);
        patientData = [actual, predicted, mbg, treatment, errorCode, profile];
