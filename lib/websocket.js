...
function init (env, ctx, server) {
  ...
  function emitData (delta) {
    if (lastData.cals) {
      console.log('running websocket.emitData', ctx.ddata.lastUpdated);
      ...
      socket.on('authorize', function authorize(message, callback) {
        ...
        if (socketAuthorization.read) {
          ...
          if (lastData && lastData.split) {
            ...
            console.info('>>>>split.first.sgvs.length', split.first.sgvs.length);
            console.info('>>>>split.rest.sgvs.length', split.rest.sgvs.length);
