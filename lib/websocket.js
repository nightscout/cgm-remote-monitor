  ...
  function verifyAuthorization (message, callback) {
    ctx.authorization.resolve({ api_secret: message.secret, token: message.token }, function resolved (err, result) {
      ...
      return callback(null, {
        read: result.shiro.check('api:*:read')
        , write: result.shiro.check('api:*:create,update,delete')
        , write_treatment: result.shiro.check('api:treatments:create,update,delete')
      });
    });
  }
