...
function init (env, ctx, server) {
  ...
  var LOG_IO = log_yellow + 'IO: ' + log_reset;
  ...
  function emitData (delta) {
    if (lastData.cals) {
      console.log(LOG_IO + 'running websocket.emitData', ctx.ddata.lastUpdated);
  ...
  function listeners ( ) {
    io.sockets.on('connection', function (socket) {
      ...
      console.log(LOG_IO + 'Connection from client ID: ', socket.client.id, ' IP: ', remoteIP);
      ...
      socket.on('disconnect', function () {
        io.emit('clients', --watchers);
        console.log(LOG_IO + 'Disconnected client ID: ',socket.client.id);
      });
      ...
      socket.on('dbUpdate', function dbUpdate(data, callback) {
       console.log(LOG_IO + 'dbUpdate client ID: ', socket.client.id, ' data: ', data);
       var collection = supportedCollections[data.collection];
      ...
      socket.on('dbAdd', function dbAdd(data, callback) {
       console.log(LOG_IO + 'dbAdd client ID: ', socket.client.id, ' data: ', data);
        var collection = supportedCollections[data.collection];
        ...
        var result = ctx.store.db.collection(collection).insert(data.data, function insertResult(err, doc) {
          if (callback) {
            callback(doc.ops);
          }
          ctx.bus.emit('data-received');
      ...
      socket.on('authorize', function authorize(message, callback) {
        ...
        if (socketAuthorization.read) {
          ...
          if (lastData && lastData.split) {
            var split = lastData.split(Date.now(), times.hours(3).msecs);
        ...
        console.log(LOG_IO + 'Authetication request from client ID: ',socket.client.id, ' client: ', clientType);
        if (callback) {
      ...
      socket.on('ping', function ping (message, callback) {
        ...
        console.log(LOG_IO + 'Ping from client ID: ',socket.client.id, ' client: ', clientType, ' timeDiff: ', (timeDiff/1000).toFixed(1) + 'sec');
        if (callback) {
  ...
  websocket.update = function update ( ) {
    console.log(LOG_IO + 'running websocket.update');
    if (lastData.sgvs) {
  ...
  websocket.emitNotification = function emitNotification (notify) {
    if (notify.clear) {
      ...
      console.info(LOG_IO + 'emitted clear_alarm to all clients');
    } else if (notify.level === levels.WARN) {
      io.emit('alarm', notify);
      console.info(LOG_IO + 'emitted alarm to all clients');
    } else if (notify.level === levels.URGENT) {
      io.emit('urgent_alarm', notify);
      console.info(LOG_IO + 'emitted urgent_alarm to all clients');
    } else if (notify.isAnnouncement) {
      io.emit('announcement', notify);
      console.info(LOG_IO + 'emitted announcement to all clients');
    }
  };
