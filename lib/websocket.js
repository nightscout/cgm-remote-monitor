...
var dir2Char = {
  ...
  function start ( ) {
    io = require('socket.io').listen(core.server);
  }
  ...
  function listeners ( ) {
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('ack', function(alarmType, _silenceTime) {
            ...
            console.log("alarm cleared", arguments, alarms);
        });
        inputs.write({type: 'websocket/connection', watchers: watchers, now: now, date: new Date( ) });
