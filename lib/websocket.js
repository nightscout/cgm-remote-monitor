...
function init(env, ctx, server) {
            ...
            socket.on('dbAdd', function dbAdd(data, callback) {
                ...
                if (data.collection === 'treatments') {
                    ...
                    if (data.data.NSCLIENT_ID) {
                        query = {
                            NSCLIENT_ID: data.data.NSCLIENT_ID
                        };
                    } else {
                        query = {
    ...
    websocket.update = function update() {
        ...
        // websocket only needs the raw data, remove processed data
        
        var ddataClone = ctx.ddata.clone();
        delete ddataClone['profileTreatments'];
        delete ddataClone['tempbasalTreatments'];
        delete ddataClone['tempTargetTreatments'];

        if (lastData.sgvs) {

            var delta = calcData(lastData, ddataClone);
            if (delta.delta) {
                console.log('lastData full size', JSON.stringify(lastData).length, 'bytes');
                if (delta.sgvs) {
                    console.log('patientData update size', JSON.stringify(delta).length, 'bytes');
                }
                emitData(delta);
            } else {
                console.log('delta calculation indicates no new data is present');
            }
        }
        lastData = ddataClone;
    };
