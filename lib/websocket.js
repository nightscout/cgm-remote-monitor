...
// list of alarms with their thresholds - disable by standard stupidly high - was 0.05 - keep urgent alarm same as that should still trigger
var alarms = {
    "alarm" : new Alarm("Regular", 0.99),
...
function loadData() {
	...
	if (actual[actualLength] > 100) emitAlarm('alarm');
    if (actualLength > 1) {
		...
		var lowAlarm = localStorage["lowAlarm"];
		if (lowAlarm == null) lowAlarm = 90;
		var highAlarm = localStorage["highAlarm"];
		if (highAlarm == null) highAlarm = 100;
		...
		else if (cBg > highAlarm) {
			emitAlarm('alarm');
		}		
		else if (cBg < lowAlarm) {
			emitAlarm('alarm');
		}		
