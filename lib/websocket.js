...
var currentProfile = require('./currentProfile')();

function websocket (env, server, entries, treatments, devicestatus) {
"use strict";
// CONSTANTS
...
var dir2Char = {
  ...
  function emitProfile (newprofile ) {
    console.log('running emitProfile', now);
    io.sockets.emit("profile", newprofile);
  }
  ...
  function listeners ( ) {
    io.sockets.on('connection', function (socket) {
        io.sockets.emit("profile", currentProfile.data());
        io.sockets.emit("sgv", patientData);
    ...
    async.parallel({
        ...
        , treatments: function(callback) {
            ...
            treatments.list(tq, function (err, results) {
                treatmentData = results ? results.map(function (treatment) {
...
function loadData() {
  ...
  currentProfile.registerUpdateHandler(emitProfile);
