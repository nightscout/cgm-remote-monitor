...
function getData(environment,time)
{
	return this.cobTotal(environment.treatments,time);	
}
...
function updateVisualisation() {
	...
	var pill = this.currentDetails.find('span.pill.cob');

	if (!pill || pill.length == 0) {
		pill = $('<span class="pill cob"><label>COB</label><em></em></span>');
		this.currentDetails.append(pill);
	}
	
	var displayCob = Math.round(this.env.cob.cob * 10) / 10;
		
	pill.find('em').text(displayCob + " g");
