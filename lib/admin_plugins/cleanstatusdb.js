...
cleanstatusdb.actions = [
    ...
    , {
        name: 'Delete all documents from devicestatus collection older than 1 month'
      , description: 'This task removes all documents from devicestatus collection that are older than 1 month. Useful when uploader battery status is not properly updated.'
...
cleanstatusdb.actions[1].init = function init(client, callback) {
  ...
  var numDays =  '<label for="devicestatus_days">'
      + client.translate('Days to Delete')
      + '          <input id="devicestatus_days"/>'
      + '      </label>';

  $('#admin_' + cleanstatusdb.name + '_1_html').append(numDays);
...
cleanstatusdb.actions[1].code =  function deleteOldRecords(client, callback) {
  var translate = client.translate;
  var $status = $('#admin_' + cleanstatusdb.name + '_1_status');
  var endDate = moment().subtract(1, 'months');
  ...
  $.ajax({
      method: 'DELETE'
    , url: '/api/v1/devicestatus_delete/' + dateStr
    , headers: client.headers()
    , success: function (retVal) {
      $status.hide().text(translate('%1 records deleted',{ params: [retVal.count] })).fadeIn('slow');
    }
  }).done(function success () {
    if (callback) {
      callback();
