...
cleanstatusdb.actions = [
    {
      ...
      , description: 'This task removes all documents from devicestatus collection. Usefull when uploader battery status is not properly updated'
...
cleanstatusdb.actions[0].init = function init(client, callback) {
  ...
  $.ajax('/api/v1/devicestatus.json?count=500000', {
    success: function (records) {
      $status.hide().text(translate('Database contains %1 records',{ params: [records.length] })).fadeIn('slow');
    },
    error: function () {
