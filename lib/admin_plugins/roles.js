    ...
    , buttons: [
      {
        ...
        , click: function() {
          ...
          role.permissions = $('#edrole_permissions').val().toLowerCase()
            .split(/[;, ]/)
            .map(s => s.trim())
            .filter(s => s.length > 0)
            .sort();
          ...
          createOrSaveRole(role, client, function callback (err) {
            if (!err) {
              $(self).dialog('close');
            }
          });
...
function showRole (role, table, client) {
  ...
  table.append($('<tr>').css('background-color', '#0f0f0f')
    .append($('<td>').attr('width', '20%').append(editIcon).append(deleteIcon).append($('<span>').text(toTextContent(role.name))))
    .append($('<td>').attr('width', '20%').text(toTextContent(!role.permissions || role.permissions.length === 0 ? '[none]' : role.permissions.join(' '))))
    .append($('<td>').attr('width', '10%').text(toTextContent(role._id ? (role.notes ? role.notes : '') : '[system default]')))
  );
}
