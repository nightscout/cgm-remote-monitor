...
roles.actions = [{
  description: 'Each role will have a 1 or more permissions.  The <em>*</em> permission is a wildcard, permisions are a hierarchy using <em>:</em> as a seperator.'
...
function showRole (role, table, client) {
  ...
  table.append($('<tr>').css('background-color','#0f0f0f')
      ...
      .append($('<td>').attr('width','20%').append(role.permissions ? role.permissions.join(' ') : '[none]'))
      .append($('<td>').attr('width','10%').append(role.autoGenerated ? '[auto generated]' : (role.notes ? role.notes : '')))
  );
}
