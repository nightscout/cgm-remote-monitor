...
const messages = {
  ...
  provider_unavailable: 'CareLink is temporarily unavailable. Please try again shortly.',
...
module.exports = function connect() {
  let client, root, country, status, details, connectButton, disconnectButton, replace, panel, image,
    ...
    pendingInput = Promise.resolve(), pendingCount = 0, pageWindow;
  ...
  function renderSession(value) {
    ...
    if (!active) {
      ...
      if (value?.state === 'expired') status.text('The login timed out. Start again when you are ready.');
      if (value?.state === 'failed') status.text(messages[value.error] || 'Sign-in failed. Please try again.');
      return;
    }
    const words = { starting: 'Opening the secure CareLink login…', waiting: 'Sign in to CareLink below. Complete any verification on the page.',
      exchanging: 'Checking your CareLink connection…', selecting: 'Choose whose data Nightscout should display.', saving: 'Saving your connection…' };
    status.text(words[value.state]);
  ...
  function init(c) {
    ...
    panel.append(button('Cancel sign-in', async () => {
