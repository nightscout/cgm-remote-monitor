...
var futuretreatments = {
  name: 'futuretreatments'
  , label: 'Remove future treatments from mongo database'
  , pluginType: 'admin'
};

function init() {
  return futuretreatments;
}

module.exports = init;

futuretreatments.actions = [
    {
        name: 'Find and remove treatments in the future'
      , description: 'This task find and remove treatments in the future.'
      , buttonLabel: 'Remove treatmens in the future'
    }
  ];

futuretreatments.actions[0].init = function init(client, callback) {
  var translate = client.translate;
  var $status = $('#admin_' + futuretreatments.name + '_0_status');
  ...
  function showTreatments(treatments, table) {
    ...
    for (var t=0; t<treatments.length; t++) {
      ...
      table.append($('<tr>').css('background-color','#0f0f0f')
        ...
        .append($('<td>').attr('width','10%').attr('align','center').append(tr.insulin ? tr.insulin : ''))
        .append($('<td>').attr('width','10%').attr('align','center').append(tr.carbs ? tr.carbs : ''))
        .append($('<td>').attr('width','10%').append(tr.enteredBy ? tr.enteredBy : ''))
        .append($('<td>').attr('width','20%').append(tr.notes ? tr.notes : ''))
      );
    }
  };
  ...
  $.ajax('/api/v1/treatments.json?&find[created_at][$gte]=' + nowiso, {
    success: function (records) {
      futuretreatments.records = records;
      ...
      $('#admin_' + futuretreatments.name + '_0_html').append(table);
      showTreatments(records, table);
      futuretreatments.actions[0].confirmText = translate('Remove %1 selected records?', { params: [records.length] });
    },
    error: function () {
      $status.hide().text(translate('Error loading database')).fadeIn('slow');
      futuretreatments.records = [];
    }
  }).done(function () { if (callback) { callback(); } });
};

futuretreatments.actions[0].code =  function deleteRecords(client, callback) {
  var translate = client.translate;
  var $status = $('#admin_' + futuretreatments.name + '_0_status');
  ...
  for (var i = 0; i < futuretreatments.records.length; i++) {
    var _id = futuretreatments.records[i]._id;
    $.ajax({
        method: 'DELETE'
  ...
  $('#admin_' + futuretreatments.name + '_0_html').html('');
  if (callback) {
    callback();
  }
};
