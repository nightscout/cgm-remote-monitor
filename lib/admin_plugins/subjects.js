...
subjects.actions[0].init = function init(client, callback) {
  ...
  function reload ( ) {
    $.ajax({
      ...
      $('#admin_' + subjects.name + '_0_html').empty().append(table).append(genDialog());
  ...
  function showSubject (subject, table) {
    ...
    deleteIcon.click(function deleteSubject ( ) {
      var ok = window.confirm(translate('Are you sure you want to delete: ') + subject.name);
      if (ok) {
        $.ajax({
          method: 'DELETE'
          ...
          window.alert(translate('Unable to delete Subject'));
        });
      }
    });
