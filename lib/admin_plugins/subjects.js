...
function createOrSaveSubject (subject, client) {

  var method = _.isEmpty(subject._id) ? 'POST' : 'PUT';

  console.info('>>>createOrSaveSubject', method, subject);
...
function deleteSubject (subject, client) {
  $.ajax({
    method: 'DELETE'
...
function reload (client, callback) {
  $.ajax({
    ...
    $('#admin_' + subjects.name + '_0_html').empty().append(table).append(genDialog(client));
    showSubjects(records, table, client);
    if (callback) {
      callback();
    }
  }).fail(function fail(err) {
    ...
    ,  buttons: [
      { text: client.translate('Save'),
        ...
        click: function() {
          ...
          createOrSaveSubject(subject, client);
...
function showSubjects(subjects, table, client) {
