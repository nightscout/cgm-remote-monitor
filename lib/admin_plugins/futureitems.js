...
futureitems.actions[0].code =  function deleteRecords(client, callback) {
  ...
  for (var i = 0; i < futureitems.treatmentrecords.length; i++) {
    var _id = futureitems.treatmentrecords[i]._id;
    $.ajax({
        method: 'DELETE'
...
futureitems.actions[1].code =  function deleteRecords(client, callback) {
  ...
  $.ajax({
      method: 'DELETE'
    , url: '/api/v1/entries/*.json&find[date][$gte]=' + new Date().getTime()
    , headers: {
      'api-secret': client.hashauth.hash()
    }
  }).done(function success () {
    $status.text(translate('Records removed ...'));
  }).fail(function fail() {
    $status.text(translate('Error removing records'));
  });
  if (callback) {
    callback();
  }
