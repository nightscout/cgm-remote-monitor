...
function init() {
  ...
  var translations = {
    ...
    ,'Wrong profile setting.\nNo profile defined to displayed time.\nRedirecting to profile editor to create new profile.' : {
      ...
      ,ko: '잘못된 프로파일 설정. \n출력된 시간으로 정의된 프로파일 없음. 새 프로파일을 생성하기 위해 프로파일 편집으로 리다이렉팅'
    ...
    ,'Each role will have a 1 or more permissions.  The <em>*</em> permission is a wildcard, permissions are a hierarchy using <em>:</em> as a separator.' : {
      ...
      ,ko: '각각은 1 또는 그 이상의 퍼미션을 가지고 있습니다. <em>*</em> 퍼미션은 구분자에 의한 와일드 카드와 계층이 <em>:</em> 있다.'
    ...
    ,'Each subject will have a unique access token and 1 or more roles.  Click on the access token to open a new view with the selected subject, this secret link can then be shared.' : {
      ...
      ,ko: '각 subject는 단일의 access token을 가지고 1 이상의 역할을 가질 것이다. 선택된 subject로 새로운 뷰를 열기 위해 access token을 클릭하세요. 그러면 이 비밀 링크는 공유되어질 수 있습니다.'
