...
function init() {
  ...
  // TODO: the whole translation dict can be removed once the translation dict can be removed once
  // we will use the ../static/translations/language-LANGCODE-resource.json files
  // This saves ~534Kb in page load
  var translations = {
    // Server
    'Listening on port' : {
  ...
  language.set = function set(newlang) {
    ...
    //TODO: how to get the URL of Nightscout in this context. I noticed BASE_URL and baseUrl are not defined
    // anymore. Suggestions?
    var url="http://localhost:1377";
    // fetch translations  TODO: make secure and filter newlang and url
    fetch(url+'/translations/language-'+newlang+'-resource.json')
      .then(function(response) {
        return response.json();
      })
      .then(function(translationJson) {
         console.log(JSON.stringify(translationJson));
         language.translations = translationJson ;
      });
  ...
  /*
  function migrate() {
    // one time conversion of translations to json format
    // TODO: remove after conversion has been done
    const fs = require('fs');
    _.forEach(language.languages, function (l) {
      ...
      for ( var k in language.translations ) {
        ...
        o = language.translations[ k ];
        if ( k == o[ l.code ] ) { // if foreign translation is identical to english, set it to empty
          dict [ k ] = '';
        } else {
          dict[ k ] = o[ l.code ];
        }
