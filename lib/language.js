...
function init() {
  ...
  language.translateCS = function translateCaseSensitive(text) {
    if (translations[String(text)] && translations[String(text)][language.lang]) {
      return translations[String(text)][language.lang];
    }
    return text;
  };
  ...
  language.translate = function translate(text, options) {
    ...
    if (options && options.params) {
      for (var i = 0; i < options.params.length; i++) {
        ...
        translated = translated.replace(r, options.params[parseInt(i)]);
      }
    }
    return translated;
  };
