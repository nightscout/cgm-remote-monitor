...
function init (ctx) {
  ...
  guardianconnect.getPrefs = function getPrefs (sbx) {
    var prefs = {
      ...
      , warnGCLastData: sbx.extendedSettings.warnGCBat || 15
      , urgentGCLastData: sbx.extendedSettings.urgentGCBat || 30
  ...
  guardianconnect.getStateString = function findLatestState (sbx) {
    ...
    if (sensorInfo && sensorInfo.connect) {
      ...
      if ((moment().diff(sensorInfo.mills, 'minutes') > warnGCLastData) && (moment().diff(sensorInfo.mills, 'minutes') < urgentGCLastData)) {
        result.level = levels.WARN;
        result.lastDataClass = 'warnBackground';
      } else if (moment().diff(sensorInfo.mills, 'minutes') >= urgentGCLastData) {
        result.level = levels.URGENT;
  ...
  guardianconnect.updateVisualisation = function updateVisualisation (sbx) {
    ...
    if (sensor) {
      ...
      info.push({
        ...
        , class: 'black-text ' + sensor.sensonDurationDaysStatusClass
      });
