...
function init () {
  ...
  alexa.getRollup = function(rollupGroup, sbx, slots, locale, callback) {
    var handlers = rollup[rollupGroup].map(item => item.handler);
    console.log('Rollup array for ', rollupGroup);
    console.log(rollup[rollupGroup]);
    var nHandlers = [];
    handlers?.forEach(function (handler) {
      nHandlers.push(handler.bind(null, slots, sbx));
    });    tasks.parallelLimit(nHandlers, 10, function(err, results) {
      if (err) {
        console.error('Error: ', err);
      }
      callback(results
        .filter(Boolean)
        .sort((a, b) => (a.priority || 0) - (b.priority || 0)) // Sort ascending by priority
