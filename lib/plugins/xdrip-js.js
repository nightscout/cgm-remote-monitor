...
function init(ctx) {
  ...
  sensorState.getPrefs = function getPrefs(sbx) {
    return {
      ...
      , stateNotifyIntrvl: sbx.extendedSettings.stateNotifyIntrvl
    };
  };
  ...
  sensorState.getStateString = function findLatestState(sbx) {
    ...
    var previous = sbx.properties.sensorState;
    ...
    if (sensorInfo && sensorInfo.xdripjs) {
      ...
      if (sensorInfo.xdripjs.state != 0x6) {
        ...
        if (!previous || !previous.lastStateNotification || !prefs.stateNotifyIntrvl || (moment().diff(previous.lastStateNotification, 'minutes') > (prefs.stateNotifyIntrvl*60))) {
          sendNotification = true;
          result.lastStateNotification = moment();
        }
        console.log('send state notification: ' + sendNotification + ': ', previous);
