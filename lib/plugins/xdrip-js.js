  ...
  sensorState.getStateString = function findLatestState(sbx) {
    ...
    if (sensorInfo && sensorInfo.xdripjs) {
      ...
      if (sensorInfo.xdripjs.state != 0x6) {
        ...
        if (!lastStateNotification || !prefs.stateNotifyIntrvl || (moment().diff(lastStateNotification, 'minutes') > (prefs.stateNotifyIntrvl*60))) {
          sendNotification = true;
          lastStateNotification = moment();
        }
        console.log('send state notification: ' + sendNotification + ' - ' + moment().diff(lastStateNotification, 'minutes'));
