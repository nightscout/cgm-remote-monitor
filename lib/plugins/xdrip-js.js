...
function init(ctx) {
  ...
  sensorState.getStateString = function findLatestState(sbx) {
    ...
    var previous = (sbx.properties.sensorState && sbx.properties.sensorState.latest) || null;
    ...
    if (sensorInfo && sensorInfo.xdripjs) {
      ...
      if (sensorInfo.xdripjs.state != 0x6) {
        ...
        // but only send notification once when the state transitions
        if (previous && previous.lastState != sensorInfo.xdripjs.state) {
      ...
      if (sensorInfo.xdripjs.state == 0x7) {
        ...
        // only send notification once when the state transitions
        if (previous && previous.lastState != sensorInfo.xdripjs.state) {
          sendNotification = true;
        }
        message = 'G5 Sensor state: ' + sensorInfo.xdripjs.stateString;
