...
function init(ctx) {
  ...
  sensorState.getStateString = function findLatestState(sbx) {
    ...
    if (sensorInfo && sensorInfo.xdripjs) {
      ...
      result.lastSlope = sensorInfo.xdripjs.slope;
      result.lastIntercept = sensorInfo.xdripjs.intercept;
  ...
  sensorState.updateVisualisation = function updateVisualisation (sbx) {
    ...
    info.push( { label: 'State Time: ', value: (sensor && sensor.lastStateTime && sensor.lastStateTime.diff(moment(), 'minutes') + ' minutes ago') || 'Unknown' } );
    ...
    if (sensor) {
      if (sensor.lastTxActivation) {
        info.push( { label: 'Tx Age: ', value: moment(sensor.lastTxActivation).diff(moment(), 'days') + ' days ago' } );
      }
      ...
      if (sensor.lastCalibrationDate) {
        info.push( { label: 'Calibration: ', value: moment(sensor.lastCalibrationDate).diff(moment(), 'hours') + ' hours ago' } );
      }
      ...
      if (sensor.lastBatteryTimestamp) {
        info.push( { label: 'Battery: ', value: moment(sensor.lastBatteryTimestamp).diff(moment(), 'minutes') + ' minutes ago' } );
      }
