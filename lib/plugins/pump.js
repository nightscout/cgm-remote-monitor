...
function init (ctx) {
  ...
  function googleHomeResevoirHandler (result, next, sbx) {
    var response = result.parameters && result.parameters.givenName ? result.parameters.givenName + ' has ' : 'You have ';
    response += sbx.properties.pump.pump.reservoir + ' units remaining';
    next(response);
  }

  function googleHomeBatteryHandler (result, next, sbx) {
    var battery = _.get(sbx, 'properties.pump.data.battery');
    if (battery) {
      var response = result.parameters && result.parameters.givenName ? result.parameters.givenName + '\'s ' : 'Your ';
      response += 'battery is at ' + battery.value + ' ' + battery.unit;
      next(response);
    } else {
      next();
    }
  ...
  pump.googleHome = {
    intentHandlers: [{
      intent: 'InsulinRemaining'
      , intentHandler: googleHomeResevoirHandler
    }, {
      intent: 'PumpBattery'
      , intentHandler: googleHomeBatteryHandler
    }]
  };
