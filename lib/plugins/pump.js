...
function init (ctx) {
  ...
  pump.getPrefs = function getPrefs (sbx) {
    ...
    return {
      ...
      , warnBattQuietNight: sbx.extendedSettings.warnBattQuietNight || false
  ...
  function prepareData (prop, prefs, sbx) {
    ...
    var time = new Date(sbx.time);
    var now = time.getHours() + time.getMinutes() / 60.0 + time.getSeconds() / 3600.0;
