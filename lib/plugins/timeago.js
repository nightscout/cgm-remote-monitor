...
function init(ctx) {
  ...
  timeago.checkNotifications = function checkNotifications(sbx) {
    ...
    var enableAlerts = sbx.extendedSettings.enableAlerts || false;
  ...
  timeago.checkStatus = function checkStatus(sbx) {
    ...
    let hibernationTimer = 15 * 1000;

    if (sbx.runtimeEnvironment == 'server') {
      hibernationTimer = ctx.settings.heartbeat * 2 * 1000;
    }

    if (delta > hibernationTimer) { // Looks like we've been hibernating
      lastSuspendTime = now;
    }
