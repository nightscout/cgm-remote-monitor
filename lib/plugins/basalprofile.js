...
function init (ctx) {
  ...
  function alexaRollupCurrentBasalHandler (slots, sbx, callback) {
    var pwd = (slots && slots.pwd && slots.pwd.value) ? slots.pwd.value : null;
    callback(null, {results: basalMessage(pwd, sbx), priority: 1});
  }

  function alexaCurrentBasalhandler (next, slots, sbx) {
    var pwd = (slots && slots.pwd && slots.pwd.value) ? slots.pwd.value : null;
    next('Current Basal', basalMessage(pwd, sbx));
  }
