...
function init (ctx) {
  ...
  basal.setProperties = function setProperties (sbx) {
    if (hasRequiredInfo(sbx)) {
      ...
      sbx.offerProperty('basal', function setBasal() {
        return {
          display: tempMark + current.totalbasal + 'U'
          , current: current
        };
      });
    }
  ...
  basal.updateVisualisation = function updateVisualisation (sbx) {
    ...
    var extraBasalFiveMin= Math.round(100*(basalValue.tempbasal-basalValue.basal)/12)/100;
    var extraBasalFiveMinText = extraBasalFiveMin + "U/5min";
    if(extraBasalFiveMin>0){
      extraBasalFiveMinText = '+' + extraBasalFiveMinText;
    }
    sbx.pluginBase.updatePillText(basal, {
      value: basalValue.tempbasal + 'U (' + Math.round(100*basalValue.basal)/100 + ') -> ' + extraBasalFiveMinText
