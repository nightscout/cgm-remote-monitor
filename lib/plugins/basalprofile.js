...
function init (ctx) {
  ...
  var translatePlaceholders = ctx.language.translatePlaceholders;
  ...
  function basalMessage(slots, sbx) {
    ...
    if (basalValue.treatment) {
      ...
      var map = new Map();
      map.set('preamble',  preamble);
      map.set('basal', basalValue.totalbasal);
      map.set('left', minutesLeft);
      response = translatePlaceholders('alexaBasalTemp', map);
      ...
      map.set('preamble', preamble);
      map.set('basal', basalValue.totalbasal);
      response = translatePlaceholders('alexaBasal', map);
