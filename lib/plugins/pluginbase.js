...
function init (majorPills, minorPills, statusPills, bgStatus, tooltip, storage) {
  ...
  function findOrCreatePill (plugin) {
    ...
    if (!pill || pill.length === 0) {
      ...
      if (plugin.pillFlip) {
        ...
        pill.append(pillLabel);
        if (plugin.additionalHtml) {
          pill.append(plugin.additionalHtml);
        }
        pill.append(pillValue);
      }

      container.append(pill);
      if (plugin.htmlInitCode) {
        plugin.htmlInitCode(pluginBase);
      }
    } else {
  ...
  pluginBase.updatePillText = function updatePillText (plugin, options) {
    ...
    if (options.info  && options.info.length) {
      ...
      pill.find('em').mouseover(function pillMouseover (event) {
      ...
      pill.find('em').mouseout(function pillMouseout ( ) {
        tooltip.transition()
          .duration(200)
          .style('opacity', 0);
      });
  ...
  pluginBase.getFromStorage = function getFromStorage (name, defaultValue) {
    var stored = storage.get(name);
    return stored === undefined ? defaultValue : stored;
  };

  pluginBase.setInStorage = function setInStorage (name, value) {
