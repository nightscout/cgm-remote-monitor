...
function init (majorPills, minorPills, statusPills, bgStatus, tooltip, storage, translate) {

  var pluginBase = { };
  var tooltipPlugin = null;
  ...
  pluginBase.updatePillText = function updatePillText (plugin, options) {
    ...
    // Replace only our handlers, releasing the previous update's options.
    pill.off('.pillTooltip');
    if (options.info  && options.info.length) {
      pill.removeAttr('title');
      pill.on('mouseover.pillTooltip', function pillMouseover (event) {
        tooltipPlugin = plugin.name;
      ...
      pill.on('mouseout.pillTooltip', function pillMouseout ( ) {
        tooltipPlugin = null;
        tooltip.style('display', 'none');
      });
    } else {
      pill.attr('title', toTextContent(translate(plugin.label || plugin.name)));
      if (tooltipPlugin === plugin.name) {
        tooltip.style('display', 'none');
        tooltipPlugin = null;
      }
    }
