...
function pushTempBasalAdjustments(prop, info, sbx) {
  if (prop && prop.tempBasalAdjustment) {
    var carbsOrBolusMessage = 'basal adjustment out of range, give carbs?';
    var sign = '';
    if (prop.tempBasalAdjustment.thirtymin > 100) {
      carbsOrBolusMessage = 'basal adjustment out of range, give bolus?';
      sign = '+';
    }

    info.push({label: '---------', value: ''});
    if (prop.aimTarget) {
      info.push({label: 'Projected BG ' + prop.aimTargetString +' target', value: 'aiming at ' + prop.aimTarget + ' ' + sbx.settings.units});
    }
    
    if (prop.bolusEstimate > 0) {
      info.push({label: 'Bolus ' + prop.bolusEstimateDisplay + ' units', value: 'or adjust basal'});
      info.push({label: 'Check BG using glucometer before correcting!', value: ''});
      info.push({label: '---------', value: ''});
    } else {
      info.push({label: 'Basal reduction to account ' + prop.bolusEstimateDisplay + ' units:', value: ''});
    }
