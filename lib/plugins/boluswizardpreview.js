...
function init() {
  ...
  bwp.updateVisualisation = function updateVisualisation (sbx) {
    ...
    var info = [
      ...
      , {label: 'Expected effect', value: results.displayIOB + ' x -' + sbx.data.profile.getSensitivity(sbx.time) + ' = -' + results.effectDisplay + ' ' + sbx.units}
      , {label: 'Expected outcome', value: sbx.lastScaledSGV() + '-' + results.effectDisplay + ' = ' + results.outcomeDisplay + ' ' + sbx.units}
    ];
    ...
    if (results.tempBasalAdjustment) {
      ...
      var sign = '';
      if (results.tempBasalAdjustment.thirtymin > 100) { carbsOrBolusMessage = 'too large adjustment needed, give bolus?'; sign = '+';}

      info.push({label: '---------', value: ''});      
      info.push({label: 'Current basal', value: sbx.data.profile.getBasal(sbx.time)});
      info.push({label: 'Basal change to account for ' + results.bolusEstimateDisplay + ':', value: ''});
      
      if (results.tempBasalAdjustment.thirtymin > 0 && results.tempBasalAdjustment.thirtymin < 200) {
        info.push({label: '30m temp basal', value: ''+results.tempBasalAdjustment.thirtymin + '% (' + sign + (results.tempBasalAdjustment.thirtymin-100) + '%)'});
      } else {
        info.push({label: '30m temp basal', value: carbsOrBolusMessage});
      }
      if (results.tempBasalAdjustment.onehour > 0 && results.tempBasalAdjustment.onehour < 200) {
        info.push({label: '1h temp basal', value: ''+results.tempBasalAdjustment.onehour + '% (' + sign + (results.tempBasalAdjustment.onehour-100) + '%)'});
      } else {
