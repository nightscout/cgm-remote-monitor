...
function init() {
  ...
  bwp.highSnoozedByIOB = function highSnoozedByIOB(prop, settings, sbx) {
    ...
    var high = ar2EventType === 'high' || prop.scaledSGV >= sbx.scaleMgdl(sbx.thresholds.bg_target_top);
...
function isSGVOk (limit, sbx) {
  var lastSGVEntry = sbx.lastSGVEntry();
  return lastSGVEntry && lastSGVEntry.mgdl >= 39 && sbx.time - lastSGVEntry.mills < limit;
}
...
function pushInfo(prop, info, sbx) {
  if (prop && prop.errors) {
    ...
    if (prop.bolusEstimateDisplay < 0) {
      ...
      info.unshift({label: 'Carb Equivalent', value: prop.bolusEstimateDisplay + 'U * ' + sbx.data.profile.getCarbRatio() + ' = ' + carbEquivalent + ' grams'});
