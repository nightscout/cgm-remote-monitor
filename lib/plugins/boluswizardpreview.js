...
function init() {
  ...
  bwp.calc = function calc (sbx) {
    ...
    var lastMeal = _.findLast(sbx.data.treatments, function eachTreatment (treatment) {
      console.info('>>>findLast', treatment, new Date(sbx.time));
      return treatment.mills <= sbx.time && treatment.carbs > 0;
    });

    results.timeSinceLastMeal = lastMeal ? sbx.time - lastMeal.mills : 1000000000;
    results.mealEatenRecently = results.timeSinceLastMeal < times.mins(60).msecs;

    console.info('>>>>>lastMeal', lastMeal, results);
...
function pushInfo(prop, info, sbx) {
  if (prop && prop.errors) {
    ...
    if (prop.bolusEstimateDisplay < 0) {
      ...
      if (prop.mealEatenRecently) {
        info.unshift({label: 'Meal eaten recently', value: 'excess IOB is probably your meal bolus?'});
      }
