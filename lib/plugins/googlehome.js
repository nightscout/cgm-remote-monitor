...
function init (env, ctx) {
  ...
  // This function retrieves a handler based on the intent name and parameters requested.
  googleHome.getIntentHandler = function getIntentHandler(intentName, parameters) {
    if (intentName && intentHandlers[intentName]) {
      if (parameters) {
        var parameterKeys = Object.keys(parameters);
        for (var i = 0, len = parameterKeys.length; i < len; i++) {
          if (intentHandlers[intentName][parameterKeys[i]] && parameters[parameterKeys[i]] &&
            intentHandlers[intentName][parameterKeys[i]][parameters[parameterKeys[i]]] &&
            intentHandlers[intentName][parameterKeys[i]][parameters[parameterKeys[i]]].handler) {

            return intentHandlers[intentName][parameterKeys[i]][parameters[parameterKeys[i]]].handler;
          }
        }
      }
