...
function init() {
  ...
  iob.calcTotal = function calcTotal(treatments, devicestatus, profile, time, spec_profile) {
    ...
    var treatmentResult = (treatments !== undefined) ? iob.fromTreatments(treatments, profile, time, spec_profile) : {};
  ...
  iob.updateVisualisation = function updateVisualisation(sbx) {
    ...
    if (prop.treatmentIob !== undefined) {
      ...
      info.push({ label: 'Treatments IOB', value: prop.treatmentIob.toFixed(2) });
    }
