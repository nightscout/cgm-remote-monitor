...
function init() {
  ...
  function usePumpIOB (sbx) {
    return sbx.extendedSettings.source === 'pump';
  }

  iob.calc = function calc (sbx, time) {
    // Choose calculation mode at runtime, when we can get extended settings from the sandbox
    if (usePumpIOB(sbx)) {
      return iob.getPumpIOB(sbx.data.pumpStatuses, sbx.extendedSettings, time);
    } else {
      return iob.calcTotal(sbx.data.treatments, sbx.data.profile, time);
    }
  };
