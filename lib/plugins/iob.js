...
function init() {
  ...
  iob.lastIOBDeviceStatus = function lastIOBDeviceStatus(devicestatus, time) {
    ...
    return _.chain(devicestatus)
      .filter(function (devicestatusEntry) {
        return devicestatusEntry.mills <= time && devicestatusEntry.mills >= recentMills;
      })
  ...
  iob.fromDeviceStatus = function fromDeviceStatus(devicestatusEntry) {
    ...
    if (_.get(devicestatusEntry, 'openaps.iob.iob') !== undefined) {
      ...
      iob = _.isArray(devicestatusEntry.openaps.iob) ? devicestatusEntry.openaps.iob[0] : devicestatusEntry.openaps.iob;
      if (iob.time) {
      ...
      return {
        ...
        , mills: moment(iob.timestamp).valueOf( )
      };
    } else if (_.get(devicestatusEntry, 'pump.iob') !== undefined) {
      iob = devicestatusEntry.pump.iob.iob;
