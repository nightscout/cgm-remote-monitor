...
function init(ctx) {
  ...
  iob.fromDeviceStatus = function fromDeviceStatus(devicestatusEntry) {
    var iobOpenAPS = __get(devicestatusEntry, 'openaps.iob');
    var iobLoop = __get(devicestatusEntry, 'loop.iob');
    var iobPump = __get(devicestatusEntry, 'pump.iob');
  ...
  function getIob(sbx) {
    var iob = __get(sbx, 'properties.iob.iob');
    if (iob !== 0) {
