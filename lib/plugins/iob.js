...
function init() {
  ...
  iob.updateVisualisation = function updateVisualisation(sbx) {
    ...
    if (prop && prop.lastBolus) {
      ...
      var amount = sbx.pluginBase.roundInsulinForDisplayFormat(Number(prop.lastBolus.insulin), sbx) + 'U';
      info = [{label: 'Last Bolus', value: amount + ' @ ' + when }]
    }

    sbx.pluginBase.updatePillText(iob, sbx.pluginBase.roundInsulinForDisplayFormat(prop.display, sbx) + 'U', 'IOB', info);
