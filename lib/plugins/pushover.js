...
function init (env) {
  ...
  function selectKeys (notify) {
    ...
    } else if (env.levels.isAlarm(notify.level)) {
  ...
  pushover.send = function wrapSend (notify, callback) {
    ...
    function prepareMessage() {
      var msg = {
        ...
        , priority: notify.level >= env.levels.WARN ? pushover.PRIORITY_EMERGENCY : pushover.PRIORITY_NORMAL
      };

      if (env.levels.isAlarm(notify.level)) {
        //ADJUST RETRY TIME based on WARN or URGENT
        msg.retry = notify.level === env.levels.URGENT ? times.mins(2).secs : times.mins(15).secs;
