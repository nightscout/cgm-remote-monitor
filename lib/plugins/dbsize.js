...
function init (ctx) {
  ...
  dbsize.getPrefs = function getPrefs (sbx) {
    return {
      warn: sbx.extendedSettings.warn ? sbx.extendedSettings.warn : 300
      , urgent: sbx.extendedSettings.urgent ? sbx.extendedSettings.urgent : 370
  ...
  dbsize.analyzeData = function analyzeData (sbx) {
    ...
    if (totalDataSize >= prefs.warn && totalDataSize < prefs.urgent) {
      result.notificationLevel = levels.WARN;
    } else if (totalDataSize >= prefs.urgent) {
  ...
  dbsize.updateVisualisation = function updateVisualisation (sbx) {
    ...
    sbx.pluginBase.updatePillText(dbsize, {
      ...
      , info: infos
      , hide: !(prop && prop.dataPercentage && prop.dataPercentage >= 0)
    });
  };
