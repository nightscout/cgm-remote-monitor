...
function init (ctx) {
  ...
  function virtAsstDatabaseSizeHandler (next, slots, sbx) {
    var display = __get(sbx, 'properties.dbsize.display');
    if (display) {
      var dataSize = __get(sbx, 'properties.dbsize.details.dataSize');
      var dataPercentage = __get(sbx, 'properties.dbsize.dataPercentage');
