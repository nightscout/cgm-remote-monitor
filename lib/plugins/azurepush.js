  ...
  azurepush.register = function register(request, callback) {
    if (request && request.platform) {
      if (!request.installationId) {
        // New registration, create installationId
        // TODO: Store and increment
        request.installationId = env.settings.azureTag + '-1';
      }

      // More info on request body: https://msdn.microsoft.com/en-us/library/mt621153.aspx
      // TODO: Split out tags
      var installation = {
        installationId: request.deviceToken
      ...
      switch (request.platform) {
        case 'iOS':
          ...
          installation.templates.nightscout.body = "{\"aps\":{\"alert\":\"$(message)\",\"sound\":\"$(sound)\"},\"eventName\":\"$(eventName)\",\"group\":\"$(group)\",\"key\":\"$(key)\",\"level\":\"$(level)\",\"title\":\"$(title)\"}";
          break;
        case 'Android':
