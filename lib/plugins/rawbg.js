  ...
  rawbg.setProperties = function setProperties (sbx) {
    sbx.offerProperty('rawbg', function setRawBG ( ) {
      ...
      if (currentSGV && currentCal) {
        ...
        result.displayLine = ['Raw BG:', sbx.scaleBg(result.mgdl), sbx.unitsLabel, result.noiseLabel].join(' ');
      }
  ...
  rawbg.showRawBGs = function showRawBGs(mgdl, noise, cal, sbx) {
    return cal
      ...
      && (sbx.defaults.showRawbg === 'always'
           || (sbx.defaults.showRawbg === 'noise' && (noise >= 2 || mgdl < 40))
         );
  };
