...
function init (ctx) {
  ...
  function googleHomeRawBGHandler(result, next, sbx) {
    ...
    if (result.parameters && results.parameters.givenName) {
      response += parameters.givenName + '\'s ';
    } else {
      response += 'Your ';
    }
    if (result.parameters && result.parameters.readingType) {
      response += parameters.readingType;
    } else {
      response += 'raw blood glucose';
    }
    response += ' is ' + sbx.properties.rawbg.mgdl;
    next(response);
  }
  ...
  rawbg.googleHome = {
    ...
    , {
      intentName: 'current_raw_bg_given_name'
      , intentHandler: googleHomeRawBGHandler
    }]
  };
