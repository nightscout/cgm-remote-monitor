...
function init (ctx) {
  ...
  function googleHomeRawBGHandler(result, next, sbx) {
    var response = buildPreamble(result.parameters) + sbx.properties.rawbg.mgdl;
    next(response);
  }
  ...
  rawbg.googleHome = {
    intentHandlers: [{
      ...
      , routableSlot: 'metric'
      , slots: ['raw bg', 'raw blood glucose', 'raw blood sugar']
      , intentHandler: googleHomeRawBGHandler
    }]
  };
  ...
  function buildPreamble(parameters) {
    ...
    if (parameters && parameters.metric) {
      preamble += parameters.metric + ' is ';
    } else {
      preamble += 'raw blood glucose is ';
    }
    return preamble;
  }
