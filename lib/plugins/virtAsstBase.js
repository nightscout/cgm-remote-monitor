...
function init(env, ctx) {
  ...
  // Use the configured translation locale, including aliases such as dk/da_DK
  // and br/pt_BR. Localize each Moment instance; never change the global locale.
  var language = ctx.language.get(ctx.language.lang);
  var timeLocale = language ? language.file.replace('_', '-') : 'en';
      ...
      entries.list({count: 1}, function (err, records) {
        ...
        var status = translate('virtAsstStatus', {
          params: [
            ...
            direction,
            moment(records[0].date).locale(timeLocale).from(moment(sbx.time))
          ]
        });
