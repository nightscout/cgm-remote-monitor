...
function init(env, ctx) {
  ...
  virtAsstBase.setupVirtAsstHandlers = function (configuredPlugin) {
    function checkAndHandlePlugin(plugin) {
      if (plugin.virtAsst) {
    ...
    ctx.plugins.eachEnabledPlugin(checkAndHandlePlugin);

    // check specific plugins that are enabled by default, because they don't seem to be captured by the eachEnabledPlugin() call
    _each(['dbsize'], function (pluginName) {
      if (env.settings.isEnabled(pluginName)) {
        var plugin = ctx.plugins.getPlugin(pluginName);
        console.log(pluginName+' is enabled');
        console.log(plugin);
        if (plugin) {
          console.log(plugin.name);
          console.log(plugin.virtAsst);
        }
        checkAndHandlePlugin(plugin);
      } else {
