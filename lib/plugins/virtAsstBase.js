...
function init(env, ctx) {
      ...
      if (plugin.virtAsst) {
        if (plugin.virtAsst.intentHandlers) {
          ...
          _each(plugin.virtAsst.intentHandlers, function (route) {
            if (route) {
              route.intentHandler.createAccess = !!route.createAccess;
              route.intentHandler.alexaSettings = route.alexaSettings; // some settings that are specific to alexa
  ...
  virtAsstBase.initSandbox = function (configuredPlugin) {
