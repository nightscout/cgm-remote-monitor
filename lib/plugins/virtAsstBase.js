  ...
  virtAsstBase.setupMutualIntents = function (configuredPlugin) {
    ...
    configuredPlugin.configureIntentHandler('MetricNow', function (callback, slots, sbx) {
      if (sbx.properties.delta && sbx.properties.delta.display) {
        entries.list({count: 10}, function(err, records) {
          // make sure we're referencing distinct entries when the user may have multiple uploaders
          var last = moment(records[0].date);
          var previous = moment(records[1].date);
          var position = 1;
          while (Math.abs(last.diff(previous, 'second')) < 60 && position < records.length) { // at least one minute apart
            position++;
            previous = moment(records[position].date);
          }
          ...
          callback(
            ...
            translate('virtAsstDelta', {
              params: [
                ...
                last.from(moment(sbx.time)),
                previous.from(moment(sbx.time))
              ]
            })
          );
        });
