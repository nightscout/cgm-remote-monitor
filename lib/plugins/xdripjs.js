...
function init(ctx) {
  ...
  sensorState.getStateString = function findLatestState(sbx) {
    ...
    function getDevice(status) {
      ...
      var device = result.seenDevices[String(uri)];
      ...
      if (!device) {
        ...
        result.seenDevices[String(uri)] = device;
      }
      return device;
    }
  ...
  function virtAsstGenericCGMHandler(translateItem, field, next, sbx) {
    var response;
    if (sbx.properties.sensorState && sbx.properties.sensorState[String(field)]) {
      response = translate('virtAsstCGM'+translateItem, {
        params:[
          sbx.properties.sensorState[String(field)]
