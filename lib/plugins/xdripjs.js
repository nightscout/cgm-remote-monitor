...
function init(ctx) {
  ...
  function virtAsstGenericCGMHandler(translateItem, field, next, sbx) {
    var response;
    var state = __get(sbx, 'properties.sensorState.'+field);
    if (state) {
    ...
    intentHandlers: [
      ...
      , {
        ...
        , intentHandler: function(next, slots, sbx){
          var response;
          var lastSessionStart = __get(sbx, 'properties.sensorState.lastSessionStart');
          // session start is only valid if in a session
          if (lastSessionStart) {
            if (__get(sbx, 'properties.sensorState.lastState') != 0x1) {
      ...
      , {
        ...
        , metrics: ['cgm tx age']
        , intentHandler: function(next, slots, sbx){
          var lastTxActivation = __get(sbx, 'properties.sensorState.lastTxActivation');
          next(
      ...
      , {
        ...
        , intentHandler: function(next, slots, sbx){
          var response;
          var lastVoltageA = __get(sbx, 'properties.sensorState.lastVoltageA');
          var lastVoltageB = __get(sbx, 'properties.sensorState.lastVoltageB');
          var lastBatteryTimestamp = __get(sbx, 'properties.sensorState.lastBatteryTimestamp');
