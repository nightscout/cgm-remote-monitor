  ...
  bgnow.updateVisualisation = function updateVisualisation (sbx) {
    ...
    if (prop.sgvs) {
      _.forEach(prop.sgvs, function deviceAndValue(entry) {
        var device = deviceName(entry.device);
        deviceInfos[device] = {
          name: device
          , time: timeFormat(moment(entry.mills), sbx)
    ...
    if (delta.previous && delta.previous.sgvs) {
      ...
      if (deviceInfos.length > 0) {
        _.forEach(deviceInfos, function addInfo (deviceInfo) {
          ...
          info.push({label: deviceInfo.name, value: display});
        });
      }
    }
