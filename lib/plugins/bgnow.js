...
//    });
  ...
  bgnow.calcDelta = function calcDelta (buckets, sbx) {
    ...
    var first = _.find(buckets, function notEmpty (bucket) {
      return bucket && !bucket.isEmpty
    });

    if (_.isEmpty(first)) {
      console.info('>>>all buckets are empty', buckets);
      return null;
    }

    var next = _.chain(buckets).find(function afterFirstNotEmpty (bucket) {
      return bucket.mills < first.mills && !bucket.isEmpty;
    }).value();

    if (_.isEmpty(next)) {
      console.info('>>>next bucket not found', buckets);
      return null;
    }

    console.info('found buckets, first:', first, 'next:', next);
