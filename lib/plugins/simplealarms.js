  ...
  simplealarms.checkNotifications = function checkNotifications(sbx) {
    ...
    if (lastSGV) {
      if (lastSGV > sbx.thresholds.bg_high) {
        ...
        console.info(title + ': ' + (lastSGV + ' > ' + sbx.thresholds.bg_high));
      } else if (lastSGV > sbx.thresholds.bg_target_top) {
        trigger = true;
        level = 1;
        title = 'High warning';
        pushoverSound = 'climb';
        console.info(title + ': ' + (lastSGV + ' > ' + sbx.thresholds.bg_target_top));
      } else if (lastSGV < sbx.thresholds.bg_low) {
        trigger = true;
        level = 2;
        title = 'Urgent LOW';
        pushoverSound = 'persistent';
        console.info(title + ': ' + (lastSGV + ' < ' + sbx.thresholds.bg_low));
      } else if (lastSGV < sbx.thresholds.bg_target_bottom) {
        trigger = true;
        level = 1;
        title = 'Low warning';
        pushoverSound = 'falling';
        console.info(title + ': ' + (lastSGV + ' < ' + sbx.thresholds.bg_target_bottom));
