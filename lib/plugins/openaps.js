...
function init() {
  ...
  openaps.setProperties = function setProperties (sbx) {
    sbx.offerProperty('openaps', function setopenaps ( ) {
      ...
      console.info('>>>>openapsStatus', openapsStatus);
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    var device = prop.device || 'device';

    if (device.indexOf('openaps://') == 0) {
      device = device.substring('openaps://'.length);
    }

    info.push({label: timeFormat(prop.status.when, sbx), value: 'update from ' + device});
