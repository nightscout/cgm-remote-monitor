...
function init() {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
	...
	if (prop.lastSuggested.mealAssist && _.includes(selectedFields, 'meal-assist'))
	{
		events.push({time: prop.lastSuggested.moment, label: '&emsp;Meal Assist', value: prop.lastSuggested.mealAssist});
	}

        if (_.includes(selectedFields, 'iob')) {
          valueParts = concatIOB(valueParts);
          events.push({time: prop.lastSuggested.moment, label: '&emsp;IOB',  value: getIOB()});

        }

	events.push({time: prop.lastSuggested.moment, label: '&emsp;BG', value: '' + prop.lastSuggested.bg + ''});
	events.push({time: prop.lastSuggested.moment, label: '&emsp;Reason', value: prop.lastSuggested.reason});
	if (typeof prop.lastSuggested.duration !== 'undefined')
	{
		events.push({time: prop.lastSuggested.moment, value: '<b>Suggested Basal</b> ' + prop.lastSuggested.duration + 'm@' + prop.lastSuggested.rate + 'U/hr'});
	}
	else
	{
		events.push({time: prop.lastSuggested.moment, value: '<b>Suggested Basal</b> No action'});
	}
	...
	if (prop.lastIOB) {
		IOBString += sbx.roundInsulinForDisplayFormat(prop.lastIOB.iob) + 'U';
		IOBString += ' ' + prop.lastIOB.basaliob ? ', Basal IOB ' + sbx.roundInsulinForDisplayFormat(prop.lastIOB.basaliob) + 'U' : '';
		IOBString +=  bolussnooze ? ', Bolus Snooze ' + sbx.roundInsulinForDisplayFormat(bolussnooze) + 'U' : '';
	}
    ...
    function concatIOB (valueParts) {
      if (prop.lastIOB) {
        ...
        valueParts = valueParts.concat([
          '&emsp;IOB ' + sbx.roundInsulinForDisplayFormat(prop.lastIOB.iob) + 'U'
    ...
    if ('enacted' === prop.status.code) {
      ...
      events.push({time: prop.lastEnacted.moment, label: 'BG', value: prop.lastEnacted.bg});
      canceled ? '' : events.push({time: prop.lastEnacted.moment, label: 'Rate', value: prop.lastEnacted.rate + ' for ' + prop.lastEnacted.duration + 'm'});

      var valueParts = [', <b>Temp Basal' + (canceled ? ' Canceled' : ' Started') + '</b>'
	...
	events.push({time: prop.lastEnacted.moment, label: 'IOB', value: getIOB()});
        //valueParts = concatIOB(valueParts);
      }
	...
	var mmtuneString = '';
        if (best && best.length > 2 && _.includes(selectedFields, 'rssi')) {
          mmtuneString = mmtuneString.concat(best[2] + 'dB ');
//	  events.push({time: device.status.when, label: '&emsp;Signal', value: mmtuneString});
        }

        if (_.includes(selectedFields, 'freq')) {
          mmtuneString = mmtuneString.concat('@ ' + device.mmtune.setFreq + 'MHz');
        }

	deviceInfo.push(' (' + mmtuneString + ')');
      }
    ...
    var info = _.map(sorted, function eventToInfo (event) {
      return {
        label: ((typeof event.label !== 'undefined') ? event.label : timeAt(false, sbx) + timeFormat(event.time, sbx))
        , value: event.value
      };
    });
