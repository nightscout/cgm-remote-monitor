...
function init (ctx) {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    sbx.pluginBase.updatePillText(openaps, {
      value: utils.timeFormat(prop.lastLoopMoment, sbx).replace(' sedan','')  + 
      (prop.lastSuggested.reason.split(", ").find(x => x.includes("Target")) != undefined ? ' ' + prop.lastSuggested.reason.split(", ").find(x => x.includes("Target")).replace("Target","T") : "") + 
      '   ' + prop.lastSuggested.reason.split(", ").find(x => x.includes("ISF")) +
      '   ' + prop.lastSuggested.reason.split(", ").find(x => x.includes("CR")) +
      '   sr:' + prop.lastSuggested.sensitivityRatio +
      '   act:' + (Math.round(prop.lastIOB.activity*100*10)/10)
      , label: label
      , info: info
