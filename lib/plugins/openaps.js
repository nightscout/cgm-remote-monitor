  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
	...
	if (prop.lastSuggested.mealAssist && _.includes(selectedFields, 'meal-assist'))
	{
        ...
        if (_.includes(selectedFields, 'iob')) {
          ...
          events.push({time: prop.lastSuggested.moment, label: '&emsp;IOB',  value: valueParts.join('')});
    ...
    if ('enacted' === prop.status.code) {
      ...
      events.push({time: device.status.when, label: '&emspBG', value: prop.lastEnacted.bg});
      canceled ? '' : events.push({time: device.status.when, label: Rate, value: prop.lastEnacted.rate + ' for ' + prop.lastEnacted.duration + 'm'});
