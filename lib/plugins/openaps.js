...
function init(ctx) {
  ...
  openaps.getPrefs = function getPrefs(sbx) {
    ...
    var prefs = {
      ...
      , colorizePredictions: sbx.extendedSettings.colorizePredictions ? sbx.extendedSettings.colorizePredictions : false
    };
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    function getForecastPoints ( ) {
      var points = [ ];
      let colorize = openaps.getPrefs(sbx).colorizePredictions;
      function toPoints (offset, forecastType, fcastcolor) {
        return  function toPoint (value, index) {
          return {
            mgdl: value
            , color: colorize ? fcastcolor : '#ff00ff'
      ...
      // IOB, COB, UAM and ZT colors are synchronised with AndroidAPS,
      // see app/src/main/res/values/colors.xml in AndroidAPS
      if (prop.lastPredBGs) {
        if (prop.lastPredBGs.values) {
          points = points.concat(_.map(prop.lastPredBGs.values, toPoints(0, "Values", 'white')));
        }
        if (prop.lastPredBGs.IOB) {
          points = points.concat(_.map(prop.lastPredBGs.IOB, toPoints(3333, "IOB", '#1e88e5')));
        }
        if (prop.lastPredBGs.ZT) {
          points = points.concat(_.map(prop.lastPredBGs.ZT, toPoints(4444, "Zero-Temp", '#b601b6')));
        }
        if (prop.lastPredBGs.aCOB) {
          points = points.concat(_.map(prop.lastPredBGs.aCOB, toPoints(5555, "Accel-COB", 'white')));
        }
        if (prop.lastPredBGs.COB) {
          points = points.concat(_.map(prop.lastPredBGs.COB, toPoints(7777, "COB", '#fb8c00')));
        }
        if (prop.lastPredBGs.UAM) {
          points = points.concat(_.map(prop.lastPredBGs.UAM, toPoints(9999, "UAM", '#ff89ff')));
        }
      }
