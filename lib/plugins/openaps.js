...
function init (ctx) {
  ...
  openaps.updateVisualisation = function updateVisualisation (sbx) {
    ...
    let pillValue = utils.timeFormat(prop.lastLoopMoment, sbx);
    pillValue += ' IOB: ' + Math.round(prop.lastSuggested.IOB * 10) / 10;
    pillValue += prop.lastSuggested.COB ? ' COB: ' + Math.round(prop.lastSuggested.COB) : '';
    pillValue += ' ' + prop.lastSuggested.reason.split(', ').find(x => x.includes("ISF"));
    pillValue +=  prop.lastSuggested.sensitivityRatio !== 1 ? ' Sens: ' + prop.lastSuggested.sensitivityRatio : '';
