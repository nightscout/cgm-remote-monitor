...
function init() {
  ...
  delta.calc = function calc(prev, current, sbx) {
    ...
    if (!prev || !current) { return result; }

    if (current.mgdl < 40 || prev.mgdl < 40) { return result; }
    if (current.mgdl > 400 || prev.mgdl > 400) { return result; }

    result.value = sbx.roundBGToDisplayFormat(sbx.scaleEntry(current) - sbx.scaleEntry(prev));
