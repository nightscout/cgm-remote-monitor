...
function init() {
  ...
  delta.calc = function calc(prev, current, sbx) {
    ...
    result.abs = current.mgdl - prev.mgdl;
    result.elapsedMins = (current.mills - prev.mills) / ONE_MINUTE;
    result.mgdl5MinsAgo = mgdl5MinsAgo(current, prev, result);
...
function mgdl5MinsAgo (current, prev, result) {
  var mgdlAgo;
  if (result.elapsedMins > 5.1) {
    mgdlAgo = current.mgdl - result.abs / result.elapsedMins * 5;
  } else {
    mgdlAgo = prev.mgdl;
  }
  return mgdlAgo
