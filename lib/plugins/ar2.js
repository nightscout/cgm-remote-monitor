...
function init() {
  ...
  ar2.checkNotifications = function checkNotifications(sbx) {
    ...
    if (trigger) {
      ...
      var useRaw = rawbgProp && sbx.extendedSettings.useRaw;
      ...
      if (useRaw) {
        ...
        lines.push(['BG Now:', sbx.scaleBg(sbx.data.lastSGV()), sbx.unitsLabel].join(' '));
      }

      lines.push([useRaw ? 'Raw BG' : 'BG Now', '15m:', sbx.scaleBg(predicted[2]), sbx.unitsLabel].join(' '));

      if (rawbgProp && !useRaw) { //If we're using raw for the forecast, don't add dupe line to the message
        lines.push(['Raw BG:', sbx.scaleBg(rawbgProp.value), sbx.unitsLabel, rawbgProp.noiseLabel].join(' '));
      }
  ...
  ar2.forecast = function forecast(sgvs, sbx) {
    ...
    var useRaw = sbx && sbx.extendedSettings.useRaw;

    if (useRaw) {
      var cal = _.last(sbx.data.cals);
      var currentRaw = rawbg.calc(sgvs[lastIndex], cal);
      if (currentRaw) {
        var prevRaw = rawbg.calc(sgvs[lastIndex - 1], cal);
        if (prevRaw) {
          current = currentRaw;
          prev = prevRaw;
          console.info('Using raw value for forecasting :)', prev, current);
        }
      }
    }
