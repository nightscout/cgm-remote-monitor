...
function init() {
  ...
  ar2.forecast = function forecast (sgvs, sbx) {
    ...
    if (current >= BG_MIN && prev >= BG_MIN) {
      //fold left over point, each time update the accumulator
...
function selectEventType (prop, sbx) {
  ...
  if (in20mins !== undefined) {
    if (in20mins > sbx.thresholds.bg_target_top) {
      eventName = 'high';
    } else if (in20mins < sbx.thresholds.bg_target_bottom) {
      eventName = 'low';
    }
  }
...
function incrementAR2 (result, sbx) {
  return {
    forecastTime: result.forecastTime ? result.forecastTime + FIVE_MINUTES : sbx.lastSGVMills()
