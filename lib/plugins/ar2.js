...
function init() {
  ...
  ar2.updateVisualisation = function updateVisualisation(sbx) {
    var forecast = sbx.properties.ar2 && sbx.properties.ar2.forecast.predicted;
    if (forecast) {
      sbx.pluginBase.addForecastPoints(ar2.forecastCone(forecast, false, sbx));
    }
  };

  ar2.forecastCone = function forecastCone (forecast, usingRaw, sbx) {

    var conePoints = [];
    var sgvs = sbx.data.sgvs;

    if (!forecast || sgvs.length < 2) {
      return conePoints;
    }

    var CONE = [0.020, 0.041, 0.061, 0.081, 0.099, 0.116, 0.132, 0.146, 0.159, 0.171, 0.182, 0.192, 0.201];

    var coneFactor = Number(sbx.extendedSettings.coneFactor);
    if (!sbx.extendedSettings.coneFactor || coneFactor < 0) {
      coneFactor = 2;
    }
    ...
    var y = [Math.log(fiveMinAgoSgv / BG_REF), Math.log(sgvs[sgvs.length-1].y / BG_REF)];

    var forecastTime = sbx.lastSGVMills();
    ...
    for (var i = 0; i < CONE.length; i++) {
      ...
      //if coneFactor = 0, then we'll only add 1 point per period instead of 2
      conePoints[coneFactor === 0 ? i : i * 2] = {
        mills: forecastTime + 2000
        , sgv: Math.max(BG_MIN, Math.min(BG_MAX, sbx.scaleBg(BG_REF * Math.exp((y[1] - coneFactor * CONE[i])))))
