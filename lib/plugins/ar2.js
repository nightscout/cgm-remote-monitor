...
function init() {
  ...
  ar2.checkNotifications = function checkNotifications(sbx) {
    ...
    var result = checkForecast(prop.forecast) || checkForecast(prop.rawForecast, true);

    if (result) {
      var eventName = selectEventType(result, sbx);
      var rangeLabel = eventName ? eventName.toUpperCase() : 'Check BG';

      if (eventName && !result.pushoverSound) {
        result.pushoverSound = eventName == 'high' ? 'climb' : 'falling';
      }

      sbx.notifications.requestNotify({
        level: result.level
        , title: buildTitle(sbx, result, rangeLabel, lastSGVEntry)
        , message: sbx.buildDefaultMessage(sbx.data.lastSGV(), result.usingRaw)
        , eventName: eventName
...
function checkForecast(forecast, usingRaw) {
...
function buildTitle(sbx, result, rangeLabel, lastSGVEntry) {
