...
function init() {
  ...
  function useRaw (sbx) {
    return sbx.properties.rawbg && (sbx.extendedSettings.useRaw === true || sbx.extendedSettings.useRaw.toLowerCase() === 'true')
  }
...
function selectEventType (prop, sbx) {
  ...
  if (in20mins !== undefined) {
    if (in20mins > sbx.scaleMgdl(sbx.thresholds.bg_target_top)) {
      eventName = 'high';
...
function buildTitle(prop, sbx) {
  ...
  if (sgv > sbx.scaleMgdl(sbx.thresholds.bg_target_bottom) && sgv < sbx.scaleMgdl(sbx.thresholds.bg_target_top)) {
    title += ' predicted';
  }
  if (prop.usingRaw) {
    title += ' w/raw';
