...
function init (ctx) {
  ...
  ar2.forecast = function forecast (sbx) {
    ...
    for (var j = 0; j <= size; j++) {
      result.avgLoss += 1 / size * Math.pow(log10(result.predicted[parseInt(j)].mgdl / 120), 2);
    }
  ...
  function virtAsstAr2Handler (next, slots, sbx) {
    if (sbx.properties.ar2.forecast.predicted) {
      ...
      for (var i = 1, len = forecast.length; i < len; i++) {
        if (forecast[parseInt(i)].mgdl > max) {
          max = forecast[parseInt(i)].mgdl;
        }
        if (forecast[parseInt(i)].mgdl < min) {
          min = forecast[parseInt(i)].mgdl;
        }
        if (forecast[parseInt(i)].mills > maxForecastMills) {
          maxForecastMills = forecast[parseInt(i)].mills;
        }
      }
      var response = '';
      if (min === max) {
