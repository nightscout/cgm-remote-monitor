...
function init() {
  ...
  ar2.setProperties = function setProperties (sbx) {
    sbx.offerProperty('ar2', function setAR2 ( ) {
      ...
      var scaled = predicted && _.map(predicted, function(p) { return sbx.scaleBg(p.mgdl) } );
  ...
  ar2.forecast = function forecast(sgvs) {
    ...
    var current = sgvs[lastIndex].mgdl;
    var prev = sgvs[lastIndex - 1].mgdl;

    if (current >= BG_MIN && sgvs[lastIndex - 1].mgdl >= BG_MIN) {
      ...
      for (var i = 0; i <= n; i++) {
        ...
        result.predicted[i] = {
          mills: dt,
          mgdl: Math.max(BG_MIN, Math.min(BG_MAX, Math.round(BG_REF * Math.exp(y[1]))))
        };
      }
...
function selectEventType (prop, sbx) {
  var predicted = prop.forecast && _.map(prop.forecast.predicted, function(p) { return sbx.scaleBg(p.mgdl) } );
...
function initConeAccumulator (sgvs, sbx) {
  ...
  var delta = sgvs[sgvs.length-1].mgdl - sgvs[sgvs.length-2].mgdl;
  var fiveMinAgoSgv = sgvs[sgvs.length-1].mgdl - delta / elapsedMins * 5;

  return {
    forecastTime: sbx.lastSGVMills()
    , conePoints: []
    , prev: Math.log(fiveMinAgoSgv / BG_REF)
    , curr: Math.log(sgvs[sgvs.length-1].mgdl / BG_REF)
  };
}
  ...
  return prop.forecast && {
    ...
    , forecast: {
      avgLoss: prop.forecast.avgLoss
        , predicted: _.map(prop.forecast.predicted, function(p) { return sbx.scaleBg(p.mgdl) }).join(', ')
    }
    , rawForecast: prop.rawForecast && {
      avgLoss: prop.rawForecast.avgLoss
      , predicted: _.map(prop.rawForecast.predicted, function(p) { return sbx.scaleBg(p.mgdl) }).join(', ')
    }
  };
}
