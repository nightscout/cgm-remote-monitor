...
function init (ctx) {
  ...
  function alexaAr2Handler (next, slots, sbx) {
    if (sbx.properties.ar2.forecast.predicted) {
      ...
      var response = 'You are expected to be between ' + min + ' and ' + max + ' over the next ' + moment(maxForecastMills).from(moment(sbx.time), true);
  ...
  ar2.alexa = {
    intentHandlers: [{
      intent: 'MetricNow'
      , routableSlot:'metric'
      , slots:['ar2', 'ar2 forecast', 'forecast']
      , intentHandler: alexaAr2Handler
    }]
  };

  function googleHomeAr2Handler(result, next, sbx) {
    if (sbx.properties.ar2.forecast.predicted) {
      var forecast = sbx.properties.ar2.forecast.predicted;
      if (forecast && forecast.length > 0) {
        ...
        var response = buildPreamble(result.parameters) + 'expected to be between ' + min + ' and ' + max + ' over the next ' + moment(maxForecastMills).from(moment(sbx.time), true);
        next(response);
      } else {
        next('Forecast is not currently known');
      }
    } else {
      next('AR2 plugin does not seem to be enabled');
    }
  }
  ...
  function buildPreamble(parameters) {
    ...
    if (parameters && parameters.givenName) {
      preamble = parameters.givenName + ' is ';
    } else {
      preamble = 'You are ';
    }
    return preamble;
  ...
  ar2.googleHome = {
    intentHandlers: [{
      ...
      , routableSlot:'metric'
      , slots:['ar2', 'ar2 forecast', 'forecast']
      , intentHandler: googleHomeAr2Handler
    }]
  };
