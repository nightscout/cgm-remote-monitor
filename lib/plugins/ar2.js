...
function okToForecast(sbx) {
  ...
  if (!bgnow || !bgnow.buckets || bgnow.buckets.length < 2) {
    return false;
  }

  var current = bgnow.buckets[0];

  return current && current.mean >= BG_MIN && bgnow.delta;
}

function initAR2 (sbx) {
  var bgnow = sbx.properties.bgnow;
  var current = _.first(bgnow.buckets);
  var delta = bgnow && bgnow.delta;
  ...
  return {
    ...
    , curr: Math.log(current.mean / BG_REF)
  };
}
