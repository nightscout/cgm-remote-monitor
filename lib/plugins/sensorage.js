...
function init(ctx) {
  ...
  sage.findLatestTimeChange = function findLatestTimeChange(sbx) {
    ...
    _.each(sbx.data.sensorTreatments, function eachTreatment (treatment) {
      ['Sensor Start', 'Sensor Change'].forEach( function eachEvent(event) {
        ...
        if (treatment.eventType === event && treatmentDate > prevDate[String(event)] && treatmentDate <= sbx.time) {

          prevDate[String(event)] = treatmentDate;
          ...
          var eventValue = returnValue[String(event)];
  ...
  sage.updateVisualisation = function updateVisualisation (sbx) {
    ...
    ['Sensor Change', 'Sensor Start'].forEach( function eachEvent(event) {
      if (latest[String(event)].found) {
        var label = event === 'Sensor Change' ? 'Sensor Insert' : event;
        info.push( { label: translate(label), value: new Date(latest[String(event)].treatmentDate).toLocaleString() } );
        info.push( { label: translate('Duration'), value: latest[String(event)].displayLong } );
        if (!_.isEmpty(latest[String(event)].notes)) {
          info.push({label: translate('Notes'), value: latest[String(event)].notes});
        }
      }
    });
