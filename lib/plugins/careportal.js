...
function init(ctx) {
  ...
  function virtAsstEating (next, slots, sbx) {
    if (slots.action_unit !== 'carbs') {
      next(translate('virtAsstUnknownUnitTitle'), translate('virtAsstUnknownUnitTitle', {params: [slots.action_unit]}));
      return;
    ...
    sbx.virtAsstCtx.treatments.create(treatments, function(err, created) {
      if (err) {
        ...
        next(translate('virtAsstDbErrTitle'), translate('virtAsstDbErrTitle', {params: [err]}));
      } else {
