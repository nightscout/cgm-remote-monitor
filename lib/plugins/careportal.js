...
function init(ctx) {
  ...
  function virtAsstOffline(next, slots, sbx) {
    var duration = sbx.virtAsst.durationToMinutes(slots);
    var createdAt = sbx.virtAsst.getCreatedAt(slots);

    if (!duration) {
      next(translate('virtAsstMissingDurationTitle'), translate('virtAsstMissingOfflineText'));
      return;
    }
    ...
    sbx.virtAsstCtx.treatments.create(treatments, function(err, created) {
      if (err) {
        ...
        console.log('Virtual assistant treatment created', created);
        next(translate('virtAsstSavedTitle'), translate('virtAsstOffline', {params: [duration]}));
      }
    });
  }

  function virtAsstOfflineConfirm(slots, sbx) {
    var duration = sbx.virtAsst.durationToMinutes(slots)
      , hasRelTime = sbx.virtAsst.hasRelTime(slots);
    ...
    if (hasRelTime) {
      ...
      return translate('virtAsstOfflineConfirmDate', {params: [duration, timeAgo]});
    } else {
      return translate('virtAsstOfflineConfirm', {params: [duration]});
    }
  }
  ...
  function virtAsstEatingSoon(next, slots, sbx) {
    ...
    sbx.virtAsstCtx.treatments.create(treatments, function(err, created) {
      if (err) {
        ...
        console.log('Virtual assistant treatment created', created);
        next(translate('virtAsstSavedTitle'), translate('virtAsstEatingSoon', {params: [duration]}));
      }
    });
  }
  ...
  careportal.virtAsst = {
    intentHandlers: [
      ...
      , {
        intent: 'MetricReportOffline'
        , createAccess: true
        , intentHandler: virtAsstOffline
        , alexaSettings: {
          requiredFields: [
            'duration'
          ]
          , intentConfirmer: virtAsstOfflineConfirm
        }
      }
      , {
