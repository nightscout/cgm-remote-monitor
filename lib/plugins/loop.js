...
function init (ctx) {
  ...
  loop.getEventTypes = function getEventTypes (sbx) {
    ...
    var postLoopNotification = function (client, data, callback) {
      //TODO: This is the client-side Javascript that performs an ajax call to /api/v2/notifications/loop
      //to send notifications.
      //We could instead call the new remotecommands api which would queue and post the push notification.
      //We may not do this though if we want to require additional security data, such as an HMAC symmetric key 
      //from the Caregiver client which would not be available on the Nightscout client.
      $.ajax({
        method: "POST"
