...
function init(ctx) {
  ...
  function alexaForecastHandler (next, slots, sbx) {
    if (sbx.properties.loop && sbx.properties.loop.lastLoop && sbx.properties.loop.lastLoop.predicted) {
      ...
      if (endPrediction.valueOf() < sbx.time) {
        ...
        var response = translate('alexaLoop', {
          params: [
            value,
      ...
      next('Loop forecast', 'Loop plugin does not seem to be enabled or doesn\' have any recent data.');
    }
  }
