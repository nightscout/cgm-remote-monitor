...
function init(ctx) {
  ...
  function alexaForecastHandler (next, slots, sbx) {
    if (sbx.properties.loop.lastLoop.predicted) {
      ...
      if (endPrediction.valueOf() < sbx.time) {
        ...
        var response = 'According to the loop forecast you are expected to be ' + value + ' over the next ' + moment(endPrediction).from(moment(sbx.time), true);
  ...
  function googleHomeForecastHandler(result, next, sbx) {
    if (sbx.properties.loop.lastLoop.predicted) {
      var forecast = sbx.properties.loop.lastLoop.predicted.values;
      var max = forecast[0];
      var min = forecast[0];
      ...
      var endPrediction = startPrediction.clone().add(maxForecastIndex * 5, 'minutes');
      if (endPrediction.valueOf() < sbx.time) {
        next('Unable to forecast with the data that is available');
      } else {
        for (var i = 1, len = forecast.slice(0, maxForecastIndex).length; i < len; i++) {
          if (forecast[i] > max) {
            max = forecast[i];
        ...
        var name = result.parameters && result.parameters.givenName ? result.parameters.givenName + ' is' : 'you are';
        var response = 'According to the loop forecast ' + name + ' expected to be ' + value + ' over the next ' + moment(endPrediction).from(moment(sbx.time), true);
        next(response);
      }
    } else {
      next('Loop plugin does not seem to be enabled');
    }
  }

  function googleHomeLastLoopHandler(result, next, sbx) {
    var response = 'The last successful loop was ' + moment(sbx.properties.loop.lastOkMoment).from(moment(sbx.time));
    next(response);
  }
  ...
  loop.googleHome = {
    intentHandlers: [{
      ...
      , routableSlot: 'metric'
      , slots: ['loop', 'loop forecast', 'forecast']
