  ...
  loop.analyzeData = function analyzeData (sbx) {
    ...
    _.forEach(recentData, function eachStatus (status) {
      if (status && status.loop && status.loop.timestamp) {
        var thisLoop = status.loop
        thisLoop.moment = moment(thisLoop.timestamp);

        var enacted = thisLoop.enacted;
        ...
        if (!result.lastLoop || thisLoop.moment.isAfter(result.lastLoop.moment)) {
          result.lastLoop = thisLoop
        }

        if (!thisLoop.failureReason && (!result.lastOkMoment || thisLoop.moment.isAfter(result.lastOkMoment))) {
          result.lastOkMoment = thisLoop.moment
        }
      }
