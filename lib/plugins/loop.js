...
function init() {
  ...
  loop.analyzeData = function analyzeData (sbx) {
    ...
    function getDescriptionForStatus (status)  {
      ...
      if (status.failureReason) {
        ...
        desc.code = 'enacted';
        desc.label = 'Enacted';
      } else if (moments.recommendedTempBasal && moments.recommendedTempBasal.isAfter(recent)) {
        desc.symbol = '↻';
        desc.code = 'looping';
        desc.label = 'Looping';
      } else if (moments.when && moments.when.isAfter(recent)) {
        desc.symbol = '◉';
        desc.code = 'waiting';
        desc.label = 'Waiting';
      }
    ...
    _.forEach(recentData, function eachStatus (status) {
      status.desc = getDescriptionForStatus(status);
      status.moment = moment(status.timestamp);

      var enacted = status.loop && status.loop.enacted;
      ...
      if (!result.lastLoopMoment || status.moment.isAfter(result.lastLoopMoment)) {
        result.lastLoopMoment = status.moment;
      }

      var ok = status.loop && !status.loop.failureReason;
      if (ok && (!result.lastOkMoment || status.moment.isAfter(result.lastOkMoment))) {
        result.lastOkMoment = status.moment
      }
  ...
  loop.updateVisualisation = function updateVisualisation (sbx) {
    ...
    var label = 'Loop ' + prop.status.symbol;
