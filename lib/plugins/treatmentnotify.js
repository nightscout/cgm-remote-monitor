...
var OPENAPS_NONFILTERED = ['Meal Bolus', 'Carb Correction', 'Correction Bolus'];
...
function init() {
  ...
  function filterTreatments (sbx) {
    var filterOpenAPS = sbx.extendedSettings.filter === 'openaps';
    ...
    if (filterOpenAPS) {
      treatments = _.filter(treatments, function notOpenAPS (treatment) {
        ...
        if (treatment.enteredBy && treatment.enteredBy.indexOf('openaps://') === 0) {
          ok = _.indexOf(OPENAPS_NONFILTERED, treatment.eventType) >= 0;
        }
        return ok;
      })
    }
