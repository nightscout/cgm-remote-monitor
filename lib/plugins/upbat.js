...
function init() {
  ...
  upbat.updateVisualisation = function updateVisualisation (sbx) {
    ...
    } else if (prop.min && prop.min.battery && prop.min.voltageDisplay) {
  ...
  function googleHomeUploaderBatteryHandler(result, next, sbx) {
    var response = '';
    if (result.parameters && result.parameters.givenName) {
      response = result.parameters.givenName + '\'s ';
    } else {
      response = 'Your ';
    }
    response += 'uploader battery is at ' + sbx.properties.upbat.display;
    next(response);
  }
  ...
  upbat.googleHome = {
    intentHandlers: [{
      intent: 'UploaderBattery'
      , intentHandler: googleHomeUploaderBatteryHandler
    }]
  };
