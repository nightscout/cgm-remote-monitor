...
function init(ctx) {
  ...
  upbat.analyzeData = function analyzeData (sbx) {
    ...
    var recentData = sbx.data.devicestatus?.filter(function eachStatus (status) {
    ...
    recentData?.forEach(function eachRecentStatus (status) {
      analyzeStatus(status);
      var device = getDevice(status);
      device.statuses.push(pick(status, ['uploader', 'created_at', 'mills', '_id']));
    });

    var recentLowests = [ ];
    Object.values(result.devices)?.forEach(function eachDevice (device) {
      device.statuses = device.statuses.sort((a, b) => sbx.entryMills(b) - sbx.entryMills(a));
      var first = device.statuses?.[0];
      var recent = sbx.entryMills(first) - times.mins(10).msecs;      var recentLowest = device.statuses
        .filter(function isRecent (status) {
          return sbx.entryMills(status) > recent;
        })
        .reduce((min, current) => {
          if (!min || byBattery(current) < byBattery(min)) {
            return current;
          }
          return min;
        }, null);
    ...
    var min = recentLowests.reduce((min, current) => {
      if (!min || byBattery(current) < byBattery(min)) {
        return current;
      }
      return min;
  ...
  upbat.checkNotifications = function checkNotifications(sbx) {
    ...
    if (prop.min && prop.min.notification && prop.min.notification >= ctx.levels.WARN) {
      var message = Object.values(prop.devices).map(function toMessage (device) {
        var info = [
          device.name
          , device.min.display
        ];
  ...
  upbat.updateVisualisation = function updateVisualisation (sbx) {
    ...
    var infos = null;    if (Object.values(prop.devices).length > 1) {
      infos = Object.values(prop.devices).map(function toInfo (device) {
        var info = {
          label: device.name
  ...
  function virtAsstUploaderBatteryHandler (next, slots, sbx) {
    var upBat = __get(sbx, 'properties.upbat.display');
    if (upBat) {
