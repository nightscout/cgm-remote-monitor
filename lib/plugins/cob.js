...
function init(ctx) {
  ...
  function alexaCOBHandler(next, slots, sbx) {
    var preamble = (slots && slots.pwd && slots.pwd.value) ? translate('alexaPreamble3person', {
      params: [
        slots.pwd.value
      ]
    }) : translate('alexaPreambleHaving');
    //var preamble = (slots && slots.pwd && slots.pwd.value) ? slots.pwd.value.replace('\'s', '') + ' has' : 'You have';
    var value = translate('alexaNo');
    if (sbx.properties.cob && sbx.properties.cob.cob !== 0) {
      value = sbx.properties.cob.cob;
    }
    var response = translate('alexaCOB', {
      params: [
        preamble, //preamble
        Math.round(value * 10) / 10 //cob
      ]
    })
    preamble + ' ' + value + ' carbohydrates on board';
