...
var __get = require('lodash/get')
  ...
  cob.fromDeviceStatus = function fromDeviceStatus (devicestatusEntry) {
    ...
    var cobObj;
    if (__get(devicestatusEntry, 'openaps') !== undefined) {
    ...
    } else if (__get(devicestatusEntry, 'loop.cob') !== undefined) {
  ...
  function virtAsstCOBHandler (next, slots, sbx) {
    var response = '';
    var cob = __get(sbx, 'properties.cob.cob');
    var pwd = __get(slots, 'pwd.value');
    var value = cob ? cob : 0;
    if (pwd) {
