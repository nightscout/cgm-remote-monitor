...
function init() {
  ...
  cob.setProperties = function setProperties(sbx) {
    sbx.offerProperty('cob', function setCOB ( ) {
      return cob.cobTotal(sbx, sbx.time);
    });
  };

  cob.cobTotal = function cobTotal(sbx, time, spec_profile) {

    var treatments = sbx.data.treatments
      , profile = sbx.data.profile;
    ...
    _.each(treatments, function eachTreatment(treatment) {
      if (treatment.carbs && treatment.mills < time) {
        ...
        if (decaysin_hr > -10) {
          // units: BG
          var actStart = iob.calc(sbx, lastDecayedBy, spec_profile).activity;
          var actEnd = iob.calc(sbx, cCalc.decayedBy, spec_profile).activity;
