  ...
  cob.cobTotal = function cobTotal(treatments, time) {
    ...
    _.forEach(treatments, function eachTreatment(treatment) {
      if (treatment.carbs && treatment.created_at < time) {
        ...
        console.info('cCalc', cCalc, ' --- treatment', treatment);
        ...
        if (decaysin_hr > 0) {
          ...
          console.info('COB contrib', cCalc.initialCarbs, decaysin_hr * carbs_hr);
          totalCOB += Math.min(cCalc.initialCarbs, decaysin_hr * carbs_hr);
