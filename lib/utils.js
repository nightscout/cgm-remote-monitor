...
function init() {
  ...
  utils.mergeInputTime = function mergeInputTime(timestring,datestring) {
    var eventTimeParts = timestring.split(':');
    var eventTime = new Date(datestring);
  ...
  utils.toDateInputValue = function toDateInputValue(datetime) {
    var local = new Date(datetime);
    local.setMinutes(datetime.getMinutes() - datetime.getTimezoneOffset());
    return local.toJSON().slice(0,10);
  };

  utils.addMinutes = function addMinutes(datetime,h) {    
     datetime.setTime(datetime.getTime() + (h*60*1000)); 
     return datetime;   
  };

  utils.addHours = function addHours(datetime,h) {    
     datetime.setTime(datetime.getTime() + (h*60*60*1000)); 
     return datetime;   
  };

  utils.addDays = function addDays(datetime,h) {    
     datetime.setTime(datetime.getTime() + (h*24*60*60*1000)); 
     return datetime;   
  };
