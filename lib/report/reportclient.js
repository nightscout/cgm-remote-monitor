...
var init = function init () {
  ...
  client.init(function loaded () {
    ...
    function prepareGUI () {
      ...
      $('.prevdates').click(function(event) {
        var days = $(this).attr('days');
        $('#rp_enabledate').prop('checked', true);
        return setPrevDataRange(event, days);
      });
      $('#rp_show').click(show);
    ...
    function setPrevDataRange (event, days) {
      $('#rp_to').val(moment().add(-days, 'days').format('YYYY-MM-DD'));
      $('#rp_from').val(moment().add(-2*days + 1, 'days').format('YYYY-MM-DD'));
      return maybePrevent(event);
    }
