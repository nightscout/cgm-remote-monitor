  ...
  client.init(function loaded () {
    ...
    function show (event) {
      ...
      function datefilter () {
        if ($('#rp_enabledate').is(':checked')) {
          matchesneeded++;
          var from = dayjs.tz(dayjs($('#rp_from').val()).startOf('day'), zone).startOf('day');
          var to = dayjs.tz(dayjs($('#rp_to').val()).endOf('day'), zone).endOf('day');
    ...
    function loadData (day, options, callback) {
      ...
      if (client.sbx.data.profile.getTimezone()) {
        from = dayjs(day).tz(client.sbx.data.profile.getTimezone()).startOf('day').format('x');
      } else {
