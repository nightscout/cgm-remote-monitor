...
function init ( ) {
  ...
  sbx.buildBGNowLine = function buildBGNowLine (displaySGV) {
    var line = 'BG Now: ' + displaySGV;
  ...
  sbx.buildForecastLine = function buildForecastLine (usingRaw) {
    ...
    var scaled = _.map(predicted, function(p) { return sbx.scaleBg(p.y) } );

    var line;
    if (scaled.length >= 3) {
      line = (usingRaw ? 'Raw BG' : 'BG') + ' 15m: ' + sbx.scaleBg(scaled[2]) + ' ' + sbx.unitsLabel;
    }
  ...
  sbx.appendPropertyLine = function appendPropertyLine (propertyName, lines) {
    ...
    var displayLine = sbx.properties[propertyName] && sbx.properties[propertyName].displayLine;
    if (displayLine) {
  ...
  sbx.prepareDefaultLines = function prepareDefaultLines(displaySGV, usingRaw) {
    var lines = [sbx.buildBGNowLine(displaySGV)];
    sbx.appendPropertyLine('rawbg', lines);

    var forecastLine = sbx.buildForecastLine(usingRaw);
    if (forecastLine) {
  ...
  sbx.buildDefaultMessage = function buildDefaultMessage(displaySGV, usingRaw) {
    return sbx.prepareDefaultLines(displaySGV, usingRaw).join('\n');
  };
