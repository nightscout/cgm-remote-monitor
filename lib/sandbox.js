...
function init ( ) {
  ...
  sbx.buildForecastLine = function buildForecastLine (usingRawForecast) {

    var prop = sbx.properties.ar2;
    var predicted = prop && (usingRawForecast ? prop.rawForecast && prop.rawForecast.predicted : prop.forecast && prop.forecast.predicted);
    ...
    var line;
    if (scaled.length >= 3) {
      line = (usingRawForecast ? 'Raw BG' : 'BG') + ' 15m: ' + sbx.scaleBg(scaled[2]) + ' ' + sbx.unitsLabel;
    }
  ...
  sbx.prepareDefaultLines = function prepareDefaultLines(usingRawForecast) {
    var lines = [sbx.buildBGNowLine()];
    sbx.appendPropertyLine('rawbg', lines);

    var forecastLine = sbx.buildForecastLine(usingRawForecast);
    if (forecastLine) {
