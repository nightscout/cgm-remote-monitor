...
function init(env, ctx) {
  ...
  function sendMakerEvent (notify) {
    ctx.maker.sendEvent({
      name: notify.makerEvent || notify.plugin.name
      , values: notify.makerValues
    }, function makerCallback (err, result) {
      if (err) {
        console.error('unable to send maker event', err, notify);
      } else {
        console.info('sent maker event: ', notify);
        recentlySent.ttl(notify.key, TIME_15_MINS_S);
      }
    })
  }
