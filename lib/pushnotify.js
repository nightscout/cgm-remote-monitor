...
function init(env, ctx) {
  ...
  function sendAzureEvent (notify) {
    if (ctx.azurepush && ctx.azurepush.notificationHubService)
    {
      ctx.azurepush.send(notify, function azureCallback(error, response){
        if (error) {
          console.error('Unable to send Azure push notification', notify, error);
        } else {
          console.info('Sent Azure push notification');
          recentlySent.ttl(notify.key, times.mins(15).secs);
        }
      });
    }
