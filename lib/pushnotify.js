...
function init(env, ctx) {
  ...
  pushnotify.emitNotification = function emitNotification (notify) {
    ...
    if (notify.level == ctx.notifications.levels.URGENT) {
      msg.retry = 120;
      if (env.baseUrl) {
        msg.callback = env.baseUrl + '/api/v1/notifications/pushovercallback';
      }
    } else if (notify.level == ctx.notifications.levels.WARN && env.baseUrl) {
      var now = Date.now();
    ...
    pushover.send(msg, function(err, result) {
      if (err) {
        ...
        //also hold on to the receipt/notify mapping
