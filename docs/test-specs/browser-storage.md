...
These are browser build measurements before integration with M15's page split. Recheck final integrated artifacts. No server RSS saving is established. Rollback restores the manifest/lock entry and caller imports together; the persisted browser format remains readable by the previous release.
